# Awesome best of python with stars

<!-- markdownlint-disable -->

<h1 align="center">
    Best-of Python
    <br>
</h1>

<p align="center">
    <strong>🏆&nbsp; A ranked list of awesome Python open-source libraries & tools. Updated weekly.</strong>
</p>

<p align="center">
    <a href="https://best-of.org" title="Best-of Badge"><img src="http://bit.ly/3o3EHNN"></a>
    <a href="#Contents" title="Project Count"><img src="https://img.shields.io/badge/projects-400-blue.svg?color=5ac4bf"></a>
    <a href="#Contribution" title="Contributions are welcome"><img src="https://img.shields.io/badge/contributions-welcome-green.svg"></a>
    <a href="https://github.com/ml-tooling/best-of-python/releases" title="Best-of Updates"><img src="https://img.shields.io/github/release-date/ml-tooling/best-of-python?color=green&label=updated"></a>
    <a href="https://mltooling.substack.com/subscribe" title="Subscribe to newsletter"><img src="http://bit.ly/2Md9rxM"></a>
    <a href="https://twitter.com/mltooling" title="Follow on Twitter"><img src="https://img.shields.io/twitter/follow/mltooling.svg?style=social&label=Follow"></a>
</p>

This curated list contains 400 awesome open-source projects with a total of 2.1M stars grouped into 28 categories. All projects are ranked by a project-quality score, which is calculated based on various metrics automatically collected from GitHub and different package managers. If you like to add or update projects, feel free to open an [issue](https://github.com/ml-tooling/best-of-python/issues/new/choose) ⭐ 4,604 | 🐛 49 | 📅 2026-09-17, submit a [pull request](https://github.com/ml-tooling/best-of-python/pulls) ⭐ 4,604 | 🐛 49 | 📅 2026-09-17, or directly edit the [projects.yaml](https://github.com/ml-tooling/best-of-python/edit/main/projects.yaml) ⭐ 4,604 | 🐛 49 | 📅 2026-09-17. Contributions are very welcome!

***

<p align="center">
     🧙‍♂️&nbsp; Discover other <a href="https://best-of.org">best-of lists</a> or create <a href="https://github.com/best-of-lists/best-of/blob/main/create-best-of-list.md">your own</a>.<br>
    📫&nbsp; Subscribe to our <a href="https://mltooling.substack.com/subscribe">newsletter</a> for updates and trending projects.
</p>

***

## Contents

* [Data Serialization](#data-serialization) *16 projects*
* [Data Containers & Dataframes](#data-containers--dataframes) *31 projects*
* [Data Structures](#data-structures) *15 projects*
* [Data Validation](#data-validation) *15 projects*
* [Algorithms & Design Patterns](#algorithms--design-patterns) *4 projects*
* [Date & Time Utilities](#date--time-utilities) *9 projects*
* [File & Path Utilities](#file--path-utilities) *10 projects*
* [Compatiblity](#compatiblity) *7 projects*
* [Cryptography](#cryptography) *7 projects*
* [Infrastructure & DevOps](#infrastructure--devops) *20 projects*
* [Process Utilities](#process-utilities) *4 projects*
* [Asynchronous Programming](#asynchronous-programming) *7 projects*
* [Configuration](#configuration) *9 projects*
* [CLI Development](#cli-development) *20 projects*
* [Development Tools](#development-tools) *1 projects*
* [Data Caching](#data-caching) *6 projects*
* [GUI Development](#gui-development) *10 projects*
* [Computer & Machine Vision](#computer--machine-vision) *2 projects*
* [Machine Learning & Data Engineering](#machine-learning--data-engineering) *1 projects*
* [Text Data](#text-data) *12 projects*
* [Web Development](#web-development) *1 projects*
* [Database Clients](#database-clients) *64 projects*
* [Data Loading & Extraction](#data-loading--extraction) *32 projects*
* [Data Pipelines & Streaming](#data-pipelines--streaming) *44 projects*
* [File Formats](#file-formats) *3 projects*
* [Code Inspection](#code-inspection) *4 projects*
* [General Utilities](#general-utilities) *15 projects*
* [Python Implementations](#python-implementations) *6 projects*
* [Others](#others) *21 projects*

## Explanation

* 🥇🥈🥉  Combined project-quality score
* ⭐️  Star count from GitHub
* 🐣  New project *(less than 6 months old)*
* 💤  Inactive project *(6 months no activity)*
* 💀  Dead project *(12 months no activity)*
* 📈📉  Project is trending up or down
* ➕  Project was recently added
* ❗️  Warning *(e.g. missing/risky license)*
* 👨‍💻  Contributors count from GitHub
* 🔀  Fork count from GitHub
* 📋  Issue count from GitHub
* ⏱️  Last update timestamp on package manager
* 📥  Download count from package manager
* 📦  Number of dependent projects
* <img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13">  Pandas related project

<br>

## Data Serialization

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/protocolbuffers/protobuf">protobuf</a></b> (🥇40 ·  ⭐ 72K) - Protocol Buffers - Googles data interchange format. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/protocolbuffers/protobuf) ⭐ 72,039 | 🐛 373 | 🌐 C++ | 📅 2026-09-18 (👨‍💻 1.5K · 🔀 16K · 📦 1M):

  ```
  git clone https://github.com/protocolbuffers/protobuf
  ```
* [PyPi](https://pypi.org/project/protobuf) (📥 670M / month · 📦 13K · ⏱️ 31.08.2026):
  ```
  pip install protobuf
  ```
* [Conda](https://anaconda.org/conda-forge/protobuf) (📥 36M · ⏱️ 06.09.2026):
  ```
  conda install -c conda-forge protobuf
  ```
* [npm](https://www.npmjs.com/package/google-protobuf) (📥 24M / month · 📦 3.9K · ⏱️ 17.09.2026):
  ```
  npm install google-protobuf
  ```

</details>
<details><summary><b><a href="https://github.com/google/flatbuffers">flatbuffers</a></b> (🥇34 ·  ⭐ 26K) - FlatBuffers: Memory Efficient Serialization Library. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/google/flatbuffers) ⭐ 26,483 | 🐛 304 | 🌐 C++ | 📅 2026-09-14 (👨‍💻 780 · 🔀 3.7K · 📦 210K):

  ```
  git clone https://github.com/google/flatbuffers
  ```
* [PyPi](https://pypi.org/project/flatbuffers) (📥 78M / month · 📦 700 · ⏱️ 19.12.2025):
  ```
  pip install flatbuffers
  ```
* [Conda](https://anaconda.org/conda-forge/flatbuffers) (📥 2.5M · ⏱️ 22.12.2025):
  ```
  conda install -c conda-forge flatbuffers
  ```
* [npm](https://www.npmjs.com/package/flatbuffers) (📥 33M / month · 📦 290 · ⏱️ 24.09.2025):
  ```
  npm install flatbuffers
  ```

</details>
<details><summary><b><a href="https://github.com/ijl/orjson">orjson</a></b> (🥇34 ·  ⭐ 8.2K) - Fast, correct Python JSON library supporting dataclasses, datetimes,.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/ijl/orjson) ⭐ 8,231 | 🐛 0 | 🌐 Python | 📅 2026-09-14 (👨‍💻 24 · 🔀 320 · 📦 230K):

  ```
  git clone https://github.com/ijl/orjson
  ```
* [PyPi](https://pypi.org/project/orjson) (📥 190M / month · 📦 7.3K · ⏱️ 14.08.2026):
  ```
  pip install orjson
  ```
* [Conda](https://anaconda.org/conda-forge/orjson) (📥 7.4M · ⏱️ 16.08.2026):
  ```
  conda install -c conda-forge orjson
  ```

</details>
<details><summary><b><a href="https://github.com/marshmallow-code/marshmallow">marshmallow</a></b> (🥈33 ·  ⭐ 7.2K) - A lightweight library for converting complex objects to and from.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/marshmallow-code/marshmallow) ⭐ 7,238 | 🐛 145 | 🌐 Python | 📅 2026-09-15 (👨‍💻 230 · 🔀 740 · 📦 230K):

  ```
  git clone https://github.com/marshmallow-code/marshmallow
  ```
* [PyPi](https://pypi.org/project/marshmallow) (📥 95M / month · 📦 2.9K · ⏱️ 08.08.2026):
  ```
  pip install marshmallow
  ```
* [Conda](https://anaconda.org/conda-forge/marshmallow) (📥 4.5M · ⏱️ 09.08.2026):
  ```
  conda install -c conda-forge marshmallow
  ```

</details>
<details><summary><b><a href="https://github.com/simplejson/simplejson">simplejson</a></b> (🥈32 ·  ⭐ 1.7K) - simplejson is a simple, fast, extensible JSON encoder/decoder for.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/simplejson/simplejson) ⭐ 1,710 | 🐛 13 | 🌐 Python | 📅 2026-09-15 (👨‍💻 54 · 🔀 360 · 📦 150K):

  ```
  git clone https://github.com/simplejson/simplejson
  ```
* [PyPi](https://pypi.org/project/simplejson) (📥 44M / month · 📦 3K · ⏱️ 27.08.2026):
  ```
  pip install simplejson
  ```
* [Conda](https://anaconda.org/conda-forge/simplejson) (📥 5.6M · ⏱️ 04.09.2026):
  ```
  conda install -c conda-forge simplejson
  ```

</details>
<details><summary><b><a href="https://github.com/uqfoundation/dill">dill</a></b> (🥈31 ·  ⭐ 2.4K) - serialize all of Python. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/uqfoundation/dill) ⭐ 2,449 | 🐛 209 | 🌐 Python | 📅 2026-09-05 (👨‍💻 51 · 🔀 190 · 📦 250K):

  ```
  git clone https://github.com/uqfoundation/dill
  ```
* [PyPi](https://pypi.org/project/dill) (📥 170M / month · 📦 4.2K · ⏱️ 19.01.2026):
  ```
  pip install dill
  ```
* [Conda](https://anaconda.org/conda-forge/dill) (📥 21M · ⏱️ 19.01.2026):
  ```
  conda install -c conda-forge dill
  ```

</details>
<details><summary><b><a href="https://github.com/cloudpipe/cloudpickle">cloudpickle</a></b> (🥈31 ·  ⭐ 1.9K) - Extended pickling support for Python objects. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/cloudpipe/cloudpickle) ⭐ 1,942 | 🐛 110 | 🌐 Python | 📅 2026-09-10 (👨‍💻 81 · 🔀 200 · 📦 170K):

  ```
  git clone https://github.com/cloudpipe/cloudpickle
  ```
* [PyPi](https://pypi.org/project/cloudpickle) (📥 150M / month · 📦 2.3K · ⏱️ 03.11.2025):
  ```
  pip install cloudpickle
  ```
* [Conda](https://anaconda.org/conda-forge/cloudpickle) (📥 39M · ⏱️ 09.12.2025):
  ```
  conda install -c conda-forge cloudpickle
  ```

</details>
<details><summary><b><a href="https://github.com/etingof/pyasn1">pyasn1</a></b> (🥈31 ·  ⭐ 240) - Generic ASN.1 library for Python. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/etingof/pyasn1) ⭐ 257 | 🐛 61 | 🌐 Python | 📅 2023-11-21 (👨‍💻 16 · 🔀 120 · 📦 710K):

  ```
  git clone https://github.com/etingof/pyasn1
  ```
* [PyPi](https://pypi.org/project/pyasn1) (📥 440M / month · 📦 2.4K · ⏱️ 09.07.2026):
  ```
  pip install pyasn1
  ```
* [Conda](https://anaconda.org/conda-forge/pyasn1) (📥 24M · ⏱️ 09.07.2026):
  ```
  conda install -c conda-forge pyasn1
  ```

</details>
<details><summary><b><a href="https://github.com/jsonpickle/jsonpickle">jsonpickle</a></b> (🥉29 ·  ⭐ 1.3K) - Python library for serializing any arbitrary object graph into.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/jsonpickle/jsonpickle) ⭐ 1,318 | 🐛 28 | 🌐 Python | 📅 2026-09-17 (👨‍💻 90 · 🔀 180 · 📦 51K):

  ```
  git clone https://github.com/jsonpickle/jsonpickle
  ```
* [PyPi](https://pypi.org/project/jsonpickle) (📥 15M / month · 📦 1.6K · ⏱️ 28.05.2026):
  ```
  pip install jsonpickle
  ```
* [Conda](https://anaconda.org/conda-forge/jsonpickle) (📥 2.5M · ⏱️ 28.05.2026):
  ```
  conda install -c conda-forge jsonpickle
  ```

</details>
<details><summary><b><a href="https://github.com/python-rapidjson/python-rapidjson">python-rapidjson</a></b> (🥉26 ·  ⭐ 530) - Python wrapper around rapidjson. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/python-rapidjson/python-rapidjson) ⭐ 533 | 🐛 20 | 🌐 C++ | 📅 2026-09-06 (👨‍💻 26 · 🔀 53 · 📦 8.1K):

  ```
  git clone https://github.com/python-rapidjson/python-rapidjson
  ```
* [PyPi](https://pypi.org/project/python-rapidjson) (📥 3.6M / month · 📦 300 · ⏱️ 06.09.2026):
  ```
  pip install python-rapidjson
  ```
* [Conda](https://anaconda.org/conda-forge/python-rapidjson) (📥 4.8M · ⏱️ 14.09.2026):
  ```
  conda install -c conda-forge python-rapidjson
  ```

</details>
<details><summary><b><a href="https://github.com/explosion/srsly">srsly</a></b> (🥉26 ·  ⭐ 480) - Modern high-performance serialization utilities for Python (JSON,.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/explosion/srsly) ⭐ 487 | 🐛 10 | 🌐 Python | 📅 2026-03-27 (👨‍💻 16 · 🔀 37 · 📦 70K):

  ```
  git clone https://github.com/explosion/srsly
  ```
* [PyPi](https://pypi.org/project/srsly) (📥 22M / month · 📦 270 · ⏱️ 23.03.2026):
  ```
  pip install srsly
  ```
* [Conda](https://anaconda.org/conda-forge/srsly) (📥 2.7M · ⏱️ 02.09.2026):
  ```
  conda install -c conda-forge srsly
  ```

</details>
<details><summary><b><a href="https://github.com/telegraphic/hickle">hickle</a></b> (🥉22 ·  ⭐ 490) - a HDF5-based python pickle replacement. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/telegraphic/hickle) ⭐ 503 | 🐛 2 | 🌐 Python | 📅 2026-08-06 (👨‍💻 29 · 🔀 70 · 📦 1.1K):

  ```
  git clone https://github.com/telegraphic/hickle
  ```
* [PyPi](https://pypi.org/project/hickle) (📥 42K / month · 📦 39 · ⏱️ 30.03.2024):
  ```
  pip install hickle
  ```
* [Conda](https://anaconda.org/conda-forge/hickle) (📥 43K · ⏱️ 06.09.2026):
  ```
  conda install -c conda-forge hickle
  ```

</details>
<details><summary><b><a href="https://github.com/samuelcolvin/rtoml">rtoml</a></b> (🥉22 ·  ⭐ 380 · 💤) - A fast TOML library for python implemented in rust. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/samuelcolvin/rtoml) ⭐ 406 | 🐛 11 | 🌐 Python | 📅 2026-06-12 (👨‍💻 16 · 🔀 30 · 📦 980):

  ```
  git clone https://github.com/samuelcolvin/rtoml
  ```
* [PyPi](https://pypi.org/project/rtoml) (📥 1.2M / month · 📦 160 · ⏱️ 19.10.2025):
  ```
  pip install rtoml
  ```

</details>
<details><summary>Show 3 hidden projects...</summary>

* <b>[ultrajson](https://github.com/ultrajson/ultrajson) ⭐ 4,497 | 🐛 31 | 🌐 C++ | 📅 2026-09-04</b> (🥉27 ·  ⭐ 4.5K) - Ultra fast JSON decoder and encoder written in C with Python.. <code>❗Unlicensed</code>
* <b>[msgpack](https://github.com/msgpack/msgpack-python) ⭐ 2,103 | 🐛 15 | 🌐 Python | 📅 2026-09-14</b> (🥉29 ·  ⭐ 2.1K) - MessagePack serializer implementation for Python.. <code>❗Unlicensed</code>
* <b>[pysimdjson](https://github.com/TkTech/pysimdjson) ⭐ 774 | 🐛 16 | 🌐 Python | 📅 2026-04-26</b> (🥉21 ·  ⭐ 710) - Python bindings for the simdjson project. <code>❗Unlicensed</code>

</details>
<br>

## Data Containers & Dataframes

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

*General-purpose data containers as well as utilities & extensions for pandas.*

<details><summary><b><a href="https://github.com/pandas-dev/pandas">pandas</a></b> (🥇40 ·  ⭐ 49K) - Flexible and powerful data analysis / manipulation library for.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code></summary>

* [GitHub](https://github.com/pandas-dev/pandas) ⭐ 49,737 | 🐛 2,697 | 🌐 Python | 📅 2026-09-18 (👨‍💻 4.3K · 🔀 20K · 📦 2.9M):

  ```
  git clone https://github.com/pandas-dev/pandas
  ```
* [PyPi](https://pypi.org/project/pandas) (📥 570M / month · 📦 130K · ⏱️ 22.07.2026):
  ```
  pip install pandas
  ```
* [Conda](https://anaconda.org/conda-forge/pandas) (📥 96M · ⏱️ 28.07.2026):
  ```
  conda install -c conda-forge pandas
  ```

</details>
<details><summary><b><a href="https://github.com/pola-rs/polars">polars</a></b> (🥇34 ·  ⭐ 40K) - Extremely fast Query Engine for DataFrames, written in Rust. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/pola-rs/polars) ⭐ 39,776 | 🐛 2,888 | 🌐 Rust | 📅 2026-09-18 (👨‍💻 770 · 🔀 3.1K · 📦 37K):

  ```
  git clone https://github.com/pola-rs/polars
  ```
* [PyPi](https://pypi.org/project/polars) (📥 56M / month · 📦 8.1K · ⏱️ 09.09.2026):
  ```
  pip install polars
  ```

</details>
<details><summary><b><a href="https://github.com/h5py/h5py">h5py</a></b> (🥇33 ·  ⭐ 2.2K) - HDF5 for Python -- The h5py package is a Pythonic interface to the HDF5.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/h5py/h5py) ⭐ 2,252 | 🐛 342 | 🌐 Python | 📅 2026-09-14 (👨‍💻 220 · 🔀 560 · 📦 360K):

  ```
  git clone https://github.com/h5py/h5py
  ```
* [PyPi](https://pypi.org/project/h5py) (📥 28M / month · 📦 11K · ⏱️ 06.03.2026):
  ```
  pip install h5py
  ```
* [Conda](https://anaconda.org/conda-forge/h5py) (📥 30M · ⏱️ 26.08.2026):
  ```
  conda install -c conda-forge h5py
  ```

</details>
<details><summary><b><a href="https://github.com/pydata/xarray">xarray</a></b> (🥈32 ·  ⭐ 4.2K) - N-D labeled arrays and datasets in Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/pydata/xarray) ⭐ 4,199 | 🐛 1,448 | 🌐 Python | 📅 2026-09-16 (👨‍💻 640 · 🔀 1.3K · 📦 46K):

  ```
  git clone https://github.com/pydata/xarray
  ```
* [PyPi](https://pypi.org/project/xarray) (📥 12M / month · 📦 6.3K · ⏱️ 09.07.2026):
  ```
  pip install xarray
  ```
* [Conda](https://anaconda.org/conda-forge/xarray) (📥 17M · ⏱️ 09.07.2026):
  ```
  conda install -c conda-forge xarray
  ```

</details>
<details><summary><b><a href="https://github.com/pydata/numexpr">numexpr</a></b> (🥈31 ·  ⭐ 2.5K) - Fast numerical array expression evaluator for Python, NumPy, Pandas,.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/pydata/numexpr) ⭐ 2,541 | 🐛 4 | 🌐 Python | 📅 2026-08-31 (👨‍💻 90 · 🔀 230 · 📦 100K):

  ```
  git clone https://github.com/pydata/numexpr
  ```
* [PyPi](https://pypi.org/project/numexpr) (📥 12M / month · 📦 1.3K · ⏱️ 18.07.2026):
  ```
  pip install numexpr
  ```
* [Conda](https://anaconda.org/conda-forge/numexpr) (📥 15M · ⏱️ 11.09.2026):
  ```
  conda install -c conda-forge numexpr
  ```

</details>
<details><summary><b><a href="https://github.com/zarr-developers/zarr-python">zarr</a></b> (🥈30 ·  ⭐ 2.1K) - An implementation of chunked, compressed, N-dimensional arrays for Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/zarr-developers/zarr-python) ⭐ 2,056 | 🐛 518 | 🌐 Python | 📅 2026-09-17 (👨‍💻 190 · 🔀 450 · 📦 9.1K):

  ```
  git clone https://github.com/zarr-developers/zarr-python
  ```
* [PyPi](https://pypi.org/project/zarr) (📥 5.2M / month · 📦 2.7K · ⏱️ 15.09.2026):
  ```
  pip install zarr
  ```
* [Conda](https://anaconda.org/conda-forge/zarr) (📥 6.6M · ⏱️ 31.07.2026):
  ```
  conda install -c conda-forge zarr
  ```

</details>
<details><summary><b><a href="https://github.com/msiemens/tinydb">TinyDB</a></b> (🥈29 ·  ⭐ 7.5K) - TinyDB is a lightweight document oriented database optimized for your.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/msiemens/tinydb) ⭐ 7,567 | 🐛 13 | 🌐 Python | 📅 2026-09-16 (👨‍💻 100 · 🔀 620 · 📦 16K):

  ```
  git clone https://github.com/msiemens/tinydb
  ```
* [PyPi](https://pypi.org/project/tinydb) (📥 3.6M / month · 📦 910 · ⏱️ 06.08.2026):
  ```
  pip install tinydb
  ```
* [Conda](https://anaconda.org/conda-forge/tinydb) (📥 1M · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge tinydb
  ```

</details>
<details><summary><b><a href="https://github.com/ekzhu/datasketch">datasketch</a></b> (🥈28 ·  ⭐ 2.9K) - MinHash, LSH, LSH Forest, Weighted MinHash, HyperLogLog,.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/ekzhu/datasketch) ⭐ 2,965 | 🐛 58 | 🌐 Python | 📅 2026-08-09 (👨‍💻 39 · 🔀 320 · 📦 2.7K):

  ```
  git clone https://github.com/ekzhu/datasketch
  ```
* [PyPi](https://pypi.org/project/datasketch) (📥 4.2M / month · 📦 220 · ⏱️ 05.07.2026):
  ```
  pip install datasketch
  ```

</details>
<details><summary><b><a href="https://github.com/pydata/bottleneck">Bottleneck</a></b> (🥈28 ·  ⭐ 1.2K) - Fast NumPy array functions written in C. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/pydata/bottleneck) ⭐ 1,182 | 🐛 61 | 🌐 Python | 📅 2026-09-15 (👨‍💻 43 · 🔀 120 · 📦 60K):

  ```
  git clone https://github.com/pydata/bottleneck
  ```
* [PyPi](https://pypi.org/project/Bottleneck) (📥 8.2M / month · 📦 780 · ⏱️ 25.07.2026):
  ```
  pip install Bottleneck
  ```
* [Conda](https://anaconda.org/conda-forge/bottleneck) (📥 7.7M · ⏱️ 11.09.2026):
  ```
  conda install -c conda-forge bottleneck
  ```

</details>
<details><summary><b><a href="https://github.com/modin-project/modin">Modin</a></b> (🥈26 ·  ⭐ 10K · 💤) - Modin: Scale your Pandas workflows by changing a single line.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code></summary>

* [GitHub](https://github.com/modin-project/modin) ⭐ 10,391 | 🐛 715 | 🌐 Python | 📅 2026-02-10 (👨‍💻 140 · 🔀 660 · 📦 2.4K):

  ```
  git clone https://github.com/modin-project/modin
  ```
* [PyPi](https://pypi.org/project/modin) (📥 1.2M / month · 📦 66 · ⏱️ 02.10.2025):
  ```
  pip install modin
  ```
* [Conda](https://anaconda.org/conda-forge/modin-core) (📥 1M · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge modin-core
  ```

</details>
<details><summary><b><a href="https://github.com/PyTables/PyTables">PyTables</a></b> (🥈26 ·  ⭐ 1.4K) - A Python package to manage extremely large amounts of data. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/PyTables/PyTables) ⭐ 1,372 | 🐛 152 | 🌐 Python | 📅 2026-09-18 (👨‍💻 150 · 🔀 280):

  ```
  git clone https://github.com/PyTables/PyTables
  ```
* [PyPi](https://pypi.org/project/tables) (📥 2.2M / month · 📦 1.8K · ⏱️ 01.03.2026):
  ```
  pip install tables
  ```
* [Conda](https://anaconda.org/conda-forge/pytables) (📥 13M · ⏱️ 11.09.2026):
  ```
  conda install -c conda-forge pytables
  ```

</details>
<details><summary><b><a href="https://github.com/vaexio/vaex">Vaex</a></b> (🥉24 ·  ⭐ 8.5K) - Out-of-Core hybrid Apache Arrow/NumPy DataFrame for Python, ML, visualization.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/vaexio/vaex) ⭐ 8,508 | 🐛 552 | 🌐 Python | 📅 2026-04-01 (👨‍💻 76 · 🔀 600 · 📦 1K):

  ```
  git clone https://github.com/vaexio/vaex
  ```
* [PyPi](https://pypi.org/project/vaex) (📥 12K / month · 📦 68 · ⏱️ 03.02.2026):
  ```
  pip install vaex
  ```
* [Conda](https://anaconda.org/conda-forge/vaex) (📥 650K · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge vaex
  ```

</details>
<details><summary><b><a href="https://github.com/h2oai/datatable">datatable</a></b> (🥉22 ·  ⭐ 1.9K) - A Python package for manipulating 2-dimensional tabular data.. <code><a href="http://bit.ly/3postzC">MPL-2.0</a></code></summary>

* [GitHub](https://github.com/h2oai/datatable) ⭐ 1,876 | 🐛 183 | 🌐 C++ | 📅 2026-09-01 (👨‍💻 42 · 🔀 160):

  ```
  git clone https://github.com/h2oai/datatable
  ```
* [PyPi](https://pypi.org/project/datatable) (📥 25K / month · 📦 54 · ⏱️ 01.12.2023):
  ```
  pip install datatable
  ```
* [Conda](https://anaconda.org/conda-forge/datatable) (📥 65K · ⏱️ 09.09.2026):
  ```
  conda install -c conda-forge datatable
  ```

</details>
<details><summary><b><a href="https://github.com/patx/pickledb">pickleDB</a></b> (🥉20 ·  ⭐ 1.1K) - pickleDB is an in memory key-value store using Pythons orjson module.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/patx/pickledb) ⭐ 1,072 | 🐛 18 | 🌐 Python | 📅 2026-05-10 (👨‍💻 17 · 🔀 130 · 📦 3.4K):

  ```
  git clone https://github.com/patx/pickledb
  ```
* [PyPi](https://pypi.org/project/pickledb) (📥 25K / month · 📦 8 · ⏱️ 05.01.2026):
  ```
  pip install pickledb
  ```

</details>
<details><summary><b><a href="https://github.com/static-frame/static-frame">StaticFrame</a></b> (🥉19 ·  ⭐ 480) - Immutable and statically-typeable DataFrames with runtime type and.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 31 · 🔀 38 · 📦 33):

  ```
  git clone https://github.com/InvestmentSystems/static-frame
  ```
* [PyPi](https://pypi.org/project/static-frame) (📥 5.1K / month · 📦 6 · ⏱️ 11.08.2026):
  ```
  pip install static-frame
  ```
* [Conda](https://anaconda.org/conda-forge/static-frame) (📥 670K · ⏱️ 07.07.2026):
  ```
  conda install -c conda-forge static-frame
  ```

</details>
<details><summary>Show 16 hidden projects...</summary>

* <b>[numpy](https://github.com/numpy/numpy) ⭐ 32,766 | 🐛 2,285 | 🌐 Python | 📅 2026-09-18</b> (🥇38 ·  ⭐ 33K) - The fundamental package for scientific computing with Python. <code>❗Unlicensed</code>
* <b>[pandera](https://github.com/pandera-dev/pandera) ⭐ 4,457 | 🐛 429 | 🌐 Python | 📅 2026-09-14</b> (🥈27 ·  ⭐ 4.4K) - A light-weight and flexible data validation and testing.. <code>❗Unlicensed</code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code>
* <b>[Pandaral·lel](https://github.com/nalepae/pandarallel) ⭐ 3,798 | 🐛 99 | 🌐 Python | 📅 2024-07-09</b> (🥉23 ·  ⭐ 3.8K · 💀) - A simple and efficient tool to parallelize Pandas.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code> <code>jupyter</code>
* <b>[Koalas](https://github.com/databricks/koalas) ⭐ 3,375 | 🐛 108 | 🌐 Python | 📅 2024-03-20</b> (🥉24 ·  ⭐ 3.4K · 💀) - Koalas: pandas API on Apache Spark. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code> <code>spark</code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code>
* <b>[Blaze](https://github.com/blaze/blaze) ⭐ 3,189 | 🐛 268 | 🌐 Python | 📅 2023-09-29</b> (🥉24 ·  ⭐ 3.2K · 💀) - NumPy and Pandas interface to Big Data. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[docarray](https://github.com/docarray/docarray) ⚠️ Archived</b> (🥈28 ·  ⭐ 3.1K · 💀) - Represent, send, store and search multimodal data. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[Arctic](https://github.com/man-group/arctic) ⭐ 3,086 | 🐛 97 | 🌐 Python | 📅 2024-04-08</b> (🥉21 ·  ⭐ 3.1K) - Arctic is a high performance datastore for numeric data. <code><a href="https://tldrlegal.com/search?q=LGPL-2.1">❗️LGPL-2.1</a></code>
* <b>[sklearn-pandas](https://github.com/scikit-learn-contrib/sklearn-pandas) ⭐ 2,841 | 🐛 43 | 🌐 Python | 📅 2023-06-08</b> (🥉24 ·  ⭐ 2.8K · 💀) - Pandas integration with sklearn. <code><a href="https://tldrlegal.com/search?q=Zlib">❗️Zlib</a></code> <code>sklearn</code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code>
* <b>[swifter](https://github.com/jmcarpenter2/swifter) ⭐ 2,633 | 🐛 26 | 🌐 Python | 📅 2024-03-20</b> (🥉20 ·  ⭐ 2.5K · 💀) - A package which efficiently applies any function to a pandas.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code>
* <b>[pandasql](https://github.com/yhat/pandasql) ⚠️ Archived</b> (🥉22 ·  ⭐ 1.3K · 💀) - sqldf for pandas. <code><a href="http://bit.ly/34MBwT8">MIT</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code>
* <b>[bcolz](https://github.com/Blosc/bcolz) ⚠️ Archived</b> (🥉22 ·  ⭐ 960 · 💀) - A columnar data container that can be compressed. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[Bounter](https://github.com/RaRe-Technologies/bounter) ⭐ 930 | 🐛 19 | 🌐 Python | 📅 2022-11-20</b> (🥉13 ·  ⭐ 930 · 💀) - Counter for large datasets. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[PandaPy](https://github.com/firmai/pandapy) ⭐ 547 | 🐛 2 | 🌐 Python | 📅 2021-10-20</b> (🥉11 ·  ⭐ 490 · 💀) - PandaPy has the speed of NumPy and the usability of Pandas 10x to.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code>
* <b>[Pandas Summary](https://github.com/mouradmourafiq/pandas-summary) ⭐ 534 | 🐛 7 | 🌐 Python | 📅 2026-09-17</b> (🥉17 ·  ⭐ 530 · 💀) - An extension to pandas describe function. <code><a href="http://bit.ly/34MBwT8">MIT</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code>
* <b>[fletcher](https://github.com/xhochy/fletcher) ⚠️ Archived</b> (🥉20 ·  ⭐ 230 · 💀) - Pandas ExtensionDType/Array backed by Apache Arrow. <code><a href="http://bit.ly/34MBwT8">MIT</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code>
* <b>[daffy](https://github.com/vertti/daffy) ⭐ 59 | 🐛 1 | 🌐 Python | 📅 2026-09-18</b> (🥉15 ·  ⭐ 58) - Lightweight DataFrame validation decorators for Pandas, Polars, Modin,.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code>

</details>
<br>

## Data Structures

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/tobgu/pyrsistent">pyrsistent</a></b> (🥇32 ·  ⭐ 2.2K) - Persistent/Immutable/Functional data structures for Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/tobgu/pyrsistent) ⭐ 2,201 | 🐛 40 | 🌐 Python | 📅 2026-01-31 (👨‍💻 78 · 🔀 160 · 📦 370K):

  ```
  git clone https://github.com/tobgu/pyrsistent
  ```
* [PyPi](https://pypi.org/project/pyrsistent) (📥 26M / month · 📦 1K · ⏱️ 25.10.2023):
  ```
  pip install pyrsistent
  ```
* [Conda](https://anaconda.org/conda-forge/pyrsistent) (📥 22M · ⏱️ 04.09.2026):
  ```
  conda install -c conda-forge pyrsistent
  ```

</details>
<details><summary><b><a href="https://github.com/jab/bidict">bidict</a></b> (🥇30 ·  ⭐ 1.6K) - The bidirectional mapping library for Python. <code><a href="http://bit.ly/3postzC">MPL-2.0</a></code></summary>

* [GitHub](https://github.com/jab/bidict) ⭐ 1,587 | 🐛 1 | 🌐 Python | 📅 2026-08-25 (👨‍💻 26 · 🔀 69 · 📦 46K):

  ```
  git clone https://github.com/jab/bidict
  ```
* [PyPi](https://pypi.org/project/bidict) (📥 25M / month · 📦 690 · ⏱️ 25.08.2026):
  ```
  pip install bidict
  ```
* [Conda](https://anaconda.org/conda-forge/bidict) (📥 960K · ⏱️ 26.08.2026):
  ```
  conda install -c conda-forge bidict
  ```

</details>
<details><summary><b><a href="https://github.com/aio-libs/multidict">multidict</a></b> (🥈28 ·  ⭐ 490) - The multidict implementation. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/aio-libs/multidict) ⭐ 492 | 🐛 19 | 🌐 Python | 📅 2026-09-18 (👨‍💻 74 · 🔀 130):

  ```
  git clone https://github.com/aio-libs/multidict
  ```
* [PyPi](https://pypi.org/project/multidict) (📥 490M / month · 📦 2.3K · ⏱️ 09.09.2026):
  ```
  pip install multidict
  ```
* [Conda](https://anaconda.org/conda-forge/multidict) (📥 29M · ⏱️ 20.02.2026):
  ```
  conda install -c conda-forge multidict
  ```

</details>
<details><summary><b><a href="https://github.com/mewwts/addict">addict</a></b> (🥈27 ·  ⭐ 2.5K) - The Python Dict thats better than heroin. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/mewwts/addict) ⭐ 2,547 | 🐛 27 | 🌐 Python | 📅 2026-03-24 (👨‍💻 34 · 🔀 130 · 📦 18K):

  ```
  git clone https://github.com/mewwts/addict
  ```
* [PyPi](https://pypi.org/project/addict) (📥 5.9M / month · 📦 450 · ⏱️ 21.11.2020):
  ```
  pip install addict
  ```
* [Conda](https://anaconda.org/conda-forge/addict) (📥 1.1M · ⏱️ 02.12.2025):
  ```
  conda install -c conda-forge addict
  ```

</details>
<details><summary><b><a href="https://github.com/mahmoud/glom">glom</a></b> (🥈27 ·  ⭐ 2.2K) - Pythons nested data operator (and CLI), for all your declarative.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/mahmoud/glom) ⭐ 2,163 | 🐛 133 | 🌐 Python | 📅 2026-09-08 (👨‍💻 24 · 🔀 84 · 📦 3.7K):

  ```
  git clone https://github.com/mahmoud/glom
  ```
* [PyPi](https://pypi.org/project/glom) (📥 22M / month · 📦 250 · ⏱️ 29.12.2025):
  ```
  pip install glom
  ```
* [Conda](https://anaconda.org/conda-forge/glom) (📥 69K · ⏱️ 29.12.2025):
  ```
  conda install -c conda-forge glom
  ```

</details>
<details><summary><b><a href="https://github.com/c0fec0de/anytree">anytree</a></b> (🥉26 ·  ⭐ 1K) - Python tree data library. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/c0fec0de/anytree) ⭐ 1,093 | 🐛 42 | 🌐 Python | 📅 2025-04-08 (👨‍💻 30 · 🔀 140 · 📦 24K):

  ```
  git clone https://github.com/c0fec0de/anytree
  ```
* [PyPi](https://pypi.org/project/anytree) (📥 5.2M / month · 📦 580 · ⏱️ 08.04.2025):
  ```
  pip install anytree
  ```
* [Conda](https://anaconda.org/conda-forge/anytree) (📥 110K · ⏱️ 25.04.2026):
  ```
  conda install -c conda-forge anytree
  ```

</details>
<details><summary><b><a href="https://github.com/fabiocaccamo/python-benedict">python-benedict</a></b> (🥉25 ·  ⭐ 1.6K) - dict subclass with keylist/keypath support, built-in I/O.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/fabiocaccamo/python-benedict) ⭐ 1,607 | 🐛 10 | 🌐 Python | 📅 2026-09-02 (👨‍💻 18 · 🔀 49 · 📦 2K):

  ```
  git clone https://github.com/fabiocaccamo/python-benedict
  ```
* [PyPi](https://pypi.org/project/python-benedict) (📥 790K / month · 📦 110 · ⏱️ 07.07.2026):
  ```
  pip install python-benedict
  ```
* [Conda](https://anaconda.org/conda-forge/python-benedict) (📥 280K · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge python-benedict
  ```

</details>
<details><summary><b><a href="https://github.com/MagicStack/immutables">immutables</a></b> (🥉24 ·  ⭐ 1.2K) - A high-performance immutable mapping type for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/MagicStack/immutables) ⭐ 1,203 | 🐛 12 | 🌐 C | 📅 2025-05-20 (👨‍💻 17 · 🔀 59 · 📦 8K):

  ```
  git clone https://github.com/MagicStack/immutables
  ```
* [PyPi](https://pypi.org/project/immutables) (📥 1.9M / month · 📦 140 · ⏱️ 10.10.2024):
  ```
  pip install immutables
  ```
* [Conda](https://anaconda.org/conda-forge/immutables) (📥 2M · ⏱️ 05.09.2026):
  ```
  conda install -c conda-forge immutables
  ```

</details>
<details><summary><b><a href="https://github.com/cdgriffith/Box">python-box</a></b> (🥉23 ·  ⭐ 2.8K) - Python dictionaries with advanced dot notation access. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/cdgriffith/Box) ⭐ 2,832 | 🐛 50 | 🌐 Python | 📅 2026-02-21 (👨‍💻 1 · 🔀 120):

  ```
  git clone https://github.com/cdgriffith/Box
  ```
* [PyPi](https://pypi.org/project/python-box) (📥 5.2M / month · 📦 590 · ⏱️ 21.02.2026):
  ```
  pip install python-box
  ```
* [Conda](https://anaconda.org/conda-forge/python-box) (📥 1.3M · ⏱️ 06.09.2026):
  ```
  conda install -c conda-forge python-box
  ```

</details>
<details><summary><b><a href="https://github.com/aio-libs/janus">janus</a></b> (🥉23 ·  ⭐ 960) - Thread-safe asyncio-aware queue for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/aio-libs/janus) ⭐ 968 | 🐛 12 | 🌐 Python | 📅 2026-08-03 (👨‍💻 30 · 🔀 54):

  ```
  git clone https://github.com/aio-libs/janus
  ```
* [PyPi](https://pypi.org/project/janus) (📥 4.5M / month · 📦 130 · ⏱️ 13.12.2024):
  ```
  pip install janus
  ```
* [Conda](https://anaconda.org/conda-forge/janus) (📥 170K · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge janus
  ```

</details>
<details><summary>Show 5 hidden projects...</summary>

* <b>[python-sortedcontainers](https://github.com/grantjenks/python-sortedcontainers) ⭐ 3,982 | 🐛 40 | 🌐 Python | 📅 2024-03-08</b> (🥈28 ·  ⭐ 3.9K · 💀) - Python Sorted Container Types: Sorted List, Sorted.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[sqlitedict](https://github.com/piskvorky/sqlitedict) ⭐ 1,245 | 🐛 38 | 🌐 Python | 📅 2022-12-07</b> (🥉21 ·  ⭐ 1.2K · 💀) - Persistent dict in Python, backed up by sqlite3 and pickle,.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[munch](https://github.com/Infinidat/munch) ⭐ 786 | 🐛 16 | 🌐 Python | 📅 2024-06-05</b> (🥉22 ·  ⭐ 780 · 💀) - A Munch is a Python dictionary that provides attribute-style access (a.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[ordered-set](https://github.com/rspeer/ordered-set) ⭐ 231 | 🐛 18 | 🌐 Python | 📅 2024-08-09</b> (🥈27 ·  ⭐ 210 · 💀) - A mutable set that remembers the order of its entries. One of.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[cleverdict](https://github.com/PFython/cleverdict) ⭐ 103 | 🐛 14 | 🌐 Python | 📅 2024-03-20</b> (🥉15 ·  ⭐ 91 · 💀) - A JSON-friendly data structure which allows both object attributes.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>

</details>
<br>

## Data Validation

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/pydantic/pydantic">pydantic</a></b> (🥇36 ·  ⭐ 29K) - Data validation using Python type hints. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 810 · 🔀 2.9K · 📦 1.1M):

  ```
  git clone https://github.com/samuelcolvin/pydantic
  ```
* [PyPi](https://pypi.org/project/pydantic) (📥 890M / month · 📦 100K · ⏱️ 09.09.2026):
  ```
  pip install pydantic
  ```
* [Conda](https://anaconda.org/conda-forge/pydantic) (📥 31M · ⏱️ 28.08.2026):
  ```
  conda install -c conda-forge pydantic
  ```

</details>
<details><summary><b><a href="https://github.com/python-jsonschema/jsonschema">jsonschema</a></b> (🥇32 ·  ⭐ 5K) - An implementation of JSON Schema validation for Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 120 · 🔀 630 · 📦 860K):

  ```
  git clone https://github.com/Julian/jsonschema
  ```
* [PyPi](https://pypi.org/project/jsonschema) (📥 480M / month · 📦 9.6K · ⏱️ 07.01.2026):
  ```
  pip install jsonschema
  ```
* [Conda](https://anaconda.org/conda-forge/jsonschema) (📥 53M · ⏱️ 24.08.2026):
  ```
  conda install -c conda-forge jsonschema
  ```

</details>
<details><summary><b><a href="https://github.com/pyeve/cerberus">cerberus</a></b> (🥈29 ·  ⭐ 3.3K) - Lightweight, extensible data validation library for Python. <code><a href="http://bit.ly/3hkKRql">ISC</a></code></summary>

* [GitHub](https://github.com/pyeve/cerberus) ⭐ 3,281 | 🐛 27 | 🌐 Python | 📅 2026-07-01 (👨‍💻 67 · 🔀 240 · 📦 18K):

  ```
  git clone https://github.com/pyeve/cerberus
  ```
* [PyPi](https://pypi.org/project/cerberus) (📥 5.2M / month · 📦 770 · ⏱️ 06.11.2025):
  ```
  pip install cerberus
  ```
* [Conda](https://anaconda.org/conda-forge/cerberus) (📥 890K · ⏱️ 17.07.2026):
  ```
  conda install -c conda-forge cerberus
  ```

</details>
<details><summary><b><a href="https://github.com/keleshev/schema">schema</a></b> (🥈29 ·  ⭐ 2.9K) - Schema validation just got Pythonic. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/keleshev/schema) ⭐ 2,946 | 🐛 110 | 🌐 Python | 📅 2026-06-20 (👨‍💻 75 · 🔀 220 · 📦 17K):

  ```
  git clone https://github.com/keleshev/schema
  ```
* [PyPi](https://pypi.org/project/schema) (📥 22M / month · 📦 890 · ⏱️ 11.10.2025):
  ```
  pip install schema
  ```
* [Conda](https://anaconda.org/conda-forge/schema) (📥 720K · ⏱️ 20.07.2026):
  ```
  conda install -c conda-forge schema
  ```

</details>
<details><summary><b><a href="https://github.com/alecthomas/voluptuous">voluptuous</a></b> (🥈29 ·  ⭐ 1.8K) - CONTRIBUTIONS ONLY: Voluptuous, despite the name, is a Python data.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/alecthomas/voluptuous) ⭐ 1,851 | 🐛 47 | 🌐 Python | 📅 2026-07-25 (👨‍💻 110 · 🔀 220 · 📦 22K):

  ```
  git clone https://github.com/alecthomas/voluptuous
  ```
* [PyPi](https://pypi.org/project/voluptuous) (📥 6.3M / month · 📦 620 · ⏱️ 18.12.2025):
  ```
  pip install voluptuous
  ```
* [Conda](https://anaconda.org/conda-forge/voluptuous) (📥 1M · ⏱️ 06.09.2026):
  ```
  conda install -c conda-forge voluptuous
  ```

</details>
<details><summary><b><a href="https://github.com/holoviz/param">param</a></b> (🥉27 ·  ⭐ 520) - Declarative parameters for robust Python classes and a rich API for.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/holoviz/param) ⭐ 521 | 🐛 181 | 🌐 Python | 📅 2026-09-18 (👨‍💻 52 · 🔀 86 · 📦 18K):

  ```
  git clone https://github.com/holoviz/param
  ```
* [PyPi](https://pypi.org/project/param) (📥 1.6M / month · 📦 260 · ⏱️ 09.09.2026):
  ```
  pip install param
  ```
* [Conda](https://anaconda.org/conda-forge/param) (📥 3.1M · ⏱️ 04.09.2026):
  ```
  conda install -c conda-forge param
  ```

</details>
<details><summary><b><a href="https://github.com/samuelcolvin/dirty-equals">dirty-equals</a></b> (🥉23 ·  ⭐ 1K · 💤) - Doing dirty (but extremely useful) things with equals. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/samuelcolvin/dirty-equals) ⭐ 1,005 | 🐛 29 | 🌐 Python | 📅 2026-08-10 (👨‍💻 21 · 🔀 48 · 📦 1.1K):

  ```
  git clone https://github.com/samuelcolvin/dirty-equals
  ```
* [PyPi](https://pypi.org/project/dirty-equals) (📥 2.4M / month · 📦 100 · ⏱️ 17.11.2025):
  ```
  pip install dirty-equals
  ```
* [Conda](https://anaconda.org/conda-forge/dirty-equals) (📥 150K · ⏱️ 18.11.2025):
  ```
  conda install -c conda-forge dirty-equals
  ```

</details>
<details><summary>Show 8 hidden projects...</summary>

* <b>[schematics](https://github.com/schematics/schematics) ⭐ 2,586 | 🐛 109 | 🌐 Python | 📅 2023-07-12</b> (🥉25 ·  ⭐ 2.6K · 💀) - Python Data Structures for Humans. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[strictyaml](https://github.com/crdoconnor/strictyaml) ⭐ 1,627 | 🐛 105 | 🌐 Python | 📅 2025-05-23</b> (🥉25 ·  ⭐ 1.6K · 💀) - Type-safe YAML parser and validator. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[python-email-validator](https://github.com/JoshData/python-email-validator) ⭐ 1,448 | 🐛 16 | 🌐 Python | 📅 2026-06-26</b> (🥉26 ·  ⭐ 1.3K · 💀) - A robust email syntax and deliverability validation.. <code><a href="http://bit.ly/3rvuUlR">Unlicense</a></code>
* <b>[valideer](https://github.com/podio/valideer) ⭐ 262 | 🐛 9 | 🌐 Python | 📅 2026-07-08</b> (🥉16 ·  ⭐ 260 · 💀) - Lightweight data validation and adaptation Python library. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[validr](https://github.com/guyskk/validr) ⭐ 221 | 🐛 8 | 🌐 Python | 📅 2024-07-09</b> (🥉13 ·  ⭐ 220 · 💀) - A simple, fast, extensible python library for data validation. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[typical](https://github.com/seandstewart/typical) ⚠️ Archived</b> (🥉17 ·  ⭐ 180 · 💀) - Typical: Fast, simple, & correct data-validation using Python 3 typing. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b><a href="https://python-validators.github.io/validators">validators</a></b> (🥈28 ·  ⭐ 1.1K · 💀) - Python Data Validation for Humans. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[dataklasses](https://github.com/dabeaz/dataklasses)</b> (🥉7 ·  ⭐ 740 · 💀) - A different spin on dataclasses. <code>❗Unlicensed</code>

</details>
<br>

## Algorithms & Design Patterns

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

🔗 <b>[python-patterns](https://github.com/faif/python-patterns) ⭐ 42,992 | 🐛 14 | 🌐 Python | 📅 2026-09-17</b> ( ⭐ 43K)  - Collection of design patterns/idioms in Python.

<details><summary><b><a href="https://github.com/keon/algorithms">algorithms</a></b> (🥉23 ·  ⭐ 25K · 💤) - Minimal examples of data structures and algorithms in Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/keon/algorithms) ⭐ 25,548 | 🐛 8 | 🌐 Python | 📅 2026-08-30 (👨‍💻 220 · 🔀 4.7K · 📦 140):

  ```
  git clone https://github.com/keon/algorithms
  ```
* [PyPi](https://pypi.org/project/algorithms) (📥 2.1K / month · 📦 5 · ⏱️ 18.02.2026):
  ```
  pip install algorithms
  ```
* [Conda](https://anaconda.org/conda-forge/algorithms) (📥 5.2K · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge algorithms
  ```

</details>
<details><summary>Show 2 hidden projects...</summary>

* <b>[transitions](https://github.com/pytransitions/transitions) ⭐ 6,593 | 🐛 22 | 🌐 Python | 📅 2025-09-11</b> (🥇26 ·  ⭐ 6.1K · 💀) - A lightweight, object-oriented finite state machine.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[PyPattyrn](https://github.com/tylerlaberge/PyPattyrn) ⚠️ Archived</b> (🥉19 ·  ⭐ 2.3K · 💀) - A simple library for implementing common design patterns. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>

</details>
<br>

## Date & Time Utilities

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/arrow-py/arrow">arrow</a></b> (🥇32 ·  ⭐ 9K) - Better dates & times for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/arrow-py/arrow) ⭐ 9,049 | 🐛 202 | 🌐 Python | 📅 2026-06-22 (👨‍💻 290 · 🔀 730 · 📦 190K):

  ```
  git clone https://github.com/arrow-py/arrow
  ```
* [PyPi](https://pypi.org/project/arrow) (📥 63M / month · 📦 2.3K · ⏱️ 18.10.2025):
  ```
  pip install arrow
  ```
* [Conda](https://anaconda.org/conda-forge/arrow) (📥 15M · ⏱️ 18.10.2025):
  ```
  conda install -c conda-forge arrow
  ```

</details>
<details><summary><b><a href="https://github.com/scrapinghub/dateparser">dateparser</a></b> (🥇32 ·  ⭐ 2.9K) - python parser for human readable dates. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/scrapinghub/dateparser) ⭐ 2,860 | 🐛 344 | 🌐 Python | 📅 2026-09-16 (👨‍💻 160 · 🔀 510 · 📦 40K):

  ```
  git clone https://github.com/scrapinghub/dateparser
  ```
* [PyPi](https://pypi.org/project/dateparser) (📥 39M / month · 📦 1.7K · ⏱️ 03.09.2026):
  ```
  pip install dateparser
  ```
* [Conda](https://anaconda.org/conda-forge/dateparser) (📥 2.3M · ⏱️ 03.09.2026):
  ```
  conda install -c conda-forge dateparser
  ```

</details>
<details><summary><b><a href="https://github.com/stub42/pytz">pytz</a></b> (🥇32 ·  ⭐ 390) - pytz Python historical timezone library and database. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/stub42/pytz) ⭐ 396 | 🐛 39 | 🌐 C | 📅 2026-07-25 (👨‍💻 27 · 🔀 110 · 📦 2.5M):

  ```
  git clone https://github.com/stub42/pytz
  ```
* [PyPi](https://pypi.org/project/pytz) (📥 410M / month · 📦 16K · ⏱️ 25.07.2026):
  ```
  pip install pytz
  ```
* [Conda](https://anaconda.org/conda-forge/pytz) (📥 99M · ⏱️ 26.07.2026):
  ```
  conda install -c conda-forge pytz
  ```

</details>
<details><summary><b><a href="https://github.com/vacanza/holidays">holidays</a></b> (🥈31 ·  ⭐ 1.9K) - Open World Holidays Framework. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/vacanza/holidays) ⭐ 1,930 | 🐛 79 | 🌐 Python | 📅 2026-09-16 (👨‍💻 350 · 🔀 710 · 📦 28K):

  ```
  git clone https://github.com/vacanza/holidays
  ```
* [PyPi](https://pypi.org/project/holidays) (📥 28M / month · 📦 660 · ⏱️ 16.09.2026):
  ```
  pip install holidays
  ```
* [Conda](https://anaconda.org/conda-forge/holidays) (📥 4.7M · ⏱️ 08.09.2026):
  ```
  conda install -c conda-forge holidays
  ```

</details>
<details><summary><b><a href="https://github.com/dateutil/dateutil">python-dateutil</a></b> (🥉29 ·  ⭐ 2.6K) - Useful extensions to the standard Python datetime features. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/dateutil/dateutil) ⭐ 2,634 | 🐛 491 | 🌐 Python | 📅 2026-05-19 (👨‍💻 130 · 🔀 540):

  ```
  git clone https://github.com/dateutil/dateutil
  ```
* [PyPi](https://pypi.org/project/python-dateutil) (📥 950M / month · 📦 12K · ⏱️ 01.03.2024):
  ```
  pip install python-dateutil
  ```
* [Conda](https://anaconda.org/conda-forge/python-dateutil) (📥 110M · ⏱️ 28.06.2025):
  ```
  conda install -c conda-forge python-dateutil
  ```

</details>
<details><summary><b><a href="https://github.com/regebro/tzlocal">tzlocal</a></b> (🥉29 ·  ⭐ 220) - A Python module that tries to figure out what your local timezone is. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/regebro/tzlocal) ⭐ 220 | 🐛 3 | 🌐 Python | 📅 2026-06-29 (👨‍💻 31 · 🔀 64 · 📦 230K):

  ```
  git clone https://github.com/regebro/tzlocal
  ```
* [PyPi](https://pypi.org/project/tzlocal) (📥 170M / month · 📦 2.4K · ⏱️ 29.06.2026):
  ```
  pip install tzlocal
  ```
* [Conda](https://anaconda.org/conda-forge/tzlocal) (📥 7.4M · ⏱️ 29.06.2026):
  ```
  conda install -c conda-forge tzlocal
  ```

</details>
<details><summary><b><a href="https://pendulum.eustace.io">pendulum</a></b> (🥉28 ·  ⭐ 6.6K · 💤) - Python datetimes made easy. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 130 · 🔀 420 · 📦 41K):

  ```
  git clone https://github.com/sdispater/pendulum
  ```
* [PyPi](https://pypi.org/project/pendulum) (📥 56M / month · 📦 1.6K · ⏱️ 30.01.2026):
  ```
  pip install pendulum
  ```
* [Conda](https://anaconda.org/conda-forge/pendulum) (📥 2.4M · ⏱️ 31.01.2026):
  ```
  conda install -c conda-forge pendulum
  ```

</details>
<details><summary>Show 2 hidden projects...</summary>

* <b>[parsedatetime](https://github.com/bear/parsedatetime) ⭐ 711 | 🐛 70 | 🌐 Python | 📅 2026-07-18</b> (🥉26 ·  ⭐ 700 · 💀) - Parse human-readable date/time strings. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[isodate](https://github.com/gweis/isodate) ⭐ 175 | 🐛 36 | 🌐 Python | 📅 2024-10-09</b> (🥉29 ·  ⭐ 170 · 💀) - ISO 8601 date/time parser. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>

</details>
<br>

## File & Path Utilities

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/fsspec/filesystem_spec">filesystem_spec</a></b> (🥇34 ·  ⭐ 1.3K) - A specification that python filesystems should adhere to. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/fsspec/filesystem_spec) ⭐ 1,351 | 🐛 356 | 🌐 Python | 📅 2026-09-18 (👨‍💻 360 · 🔀 470 · 📦 310K):

  ```
  git clone https://github.com/fsspec/filesystem_spec
  ```
* [PyPi](https://pypi.org/project/fsspec) (📥 630M / month · 📦 3.9K · ⏱️ 28.07.2026):
  ```
  pip install fsspec
  ```
* [Conda](https://anaconda.org/conda-forge/fsspec) (📥 40M · ⏱️ 29.07.2026):
  ```
  conda install -c conda-forge fsspec
  ```

</details>
<details><summary><b><a href="https://github.com/gorakhargosh/watchdog">watchdog</a></b> (🥈32 ·  ⭐ 7.1K) - Python library and shell utilities to monitor filesystem events. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/gorakhargosh/watchdog) ⭐ 7,417 | 🐛 261 | 🌐 Python | 📅 2026-09-06 (👨‍💻 180 · 🔀 720 · 📦 270K):

  ```
  git clone https://github.com/gorakhargosh/watchdog
  ```
* [PyPi](https://pypi.org/project/watchdog) (📥 91M / month · 📦 2.7K · ⏱️ 01.11.2024):
  ```
  pip install watchdog
  ```
* [Conda](https://anaconda.org/conda-forge/watchdog) (📥 6.4M · ⏱️ 04.09.2026):
  ```
  conda install -c conda-forge watchdog
  ```

</details>
<details><summary><b><a href="https://github.com/tox-dev/py-filelock">filelock</a></b> (🥈31 ·  ⭐ 970) - A platform independent file lock. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 71 · 🔀 140 · 📦 740K):

  ```
  git clone https://github.com/tox-dev/py-filelock
  ```
* [PyPi](https://pypi.org/project/filelock) (📥 530M / month · 📦 5.9K · ⏱️ 17.09.2026):
  ```
  pip install filelock
  ```
* [Conda](https://anaconda.org/conda-forge/filelock) (📥 51M · ⏱️ 09.09.2026):
  ```
  conda install -c conda-forge filelock
  ```

</details>
<details><summary><b><a href="https://github.com/Tinche/aiofiles">aiofiles</a></b> (🥈30 ·  ⭐ 3.2K · 💤) - File support for asyncio. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/Tinche/aiofiles) ⭐ 3,260 | 🐛 59 | 🌐 Python | 📅 2026-08-29 (👨‍💻 45 · 🔀 170 · 📦 250K):

  ```
  git clone https://github.com/Tinche/aiofiles
  ```
* [PyPi](https://pypi.org/project/aiofiles) (📥 130M / month · 📦 7.7K · ⏱️ 09.10.2025):
  ```
  pip install aiofiles
  ```
* [Conda](https://anaconda.org/conda-forge/aiofiles) (📥 2.8M · ⏱️ 01.01.2026):
  ```
  conda install -c conda-forge aiofiles
  ```

</details>
<details><summary>Show 6 hidden projects...</summary>

* <b>[pyfilesystem2](https://github.com/pyfilesystem/pyfilesystem2) ⭐ 2,105 | 🐛 117 | 🌐 Python | 📅 2025-05-17</b> (🥉24 ·  ⭐ 2.1K · 💀) - Pythons Filesystem abstraction layer. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[path](https://github.com/jaraco/path) ⭐ 1,124 | 🐛 13 | 🌐 Python | 📅 2026-04-13</b> (🥉24 ·  ⭐ 1.1K · 💀) - Object-oriented file system path manipulation. <code>❗Unlicensed</code>
* <b>[appdirs](https://github.com/ActiveState/appdirs) ⭐ 1,079 | 🐛 44 | 🌐 Python | 📅 2023-02-10</b> (🥉28 ·  ⭐ 1.1K · 💀) - A small Python module for determining appropriate platform-specific.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[scandir](https://github.com/benhoyt/scandir) ⚠️ Archived</b> (🥉25 ·  ⭐ 540 · 💀) - Better directory iterator and faster os.walk(). Archived, as this.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[Unipath](https://github.com/mikeorr/Unipath) ⭐ 510 | 🐛 9 | 🌐 Python | 📅 2022-01-11</b> (🥉22 ·  ⭐ 510 · 💀) - An object-oriented approach to Python file/directory operations. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[zipp](https://github.com/jaraco/zipp) ⭐ 67 | 🐛 13 | 🌐 Python | 📅 2026-05-19</b> (🥉28 ·  ⭐ 67) - Backport of pathlib-compatible object wrapper for zip files. <code>❗Unlicensed</code>

</details>
<br>

## Compatiblity

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/python/typing">typing</a></b> (🥈27 ·  ⭐ 1.7K) - Python static typing home. Hosts the documentation and a user help.. <code><a href="http://bit.ly/35wkF7y">Python-2.0</a></code></summary>

* [GitHub](https://github.com/python/typing) ⭐ 1,783 | 🐛 228 | 🌐 Python | 📅 2026-09-18 (👨‍💻 150 · 🔀 290):

  ```
  git clone https://github.com/python/typing
  ```
* [PyPi](https://pypi.org/project/typing) (📥 4.3M / month · 📦 3.3K · ⏱️ 01.05.2021):
  ```
  pip install typing
  ```
* [Conda](https://anaconda.org/conda-forge/typing) (📥 4.5M · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge typing
  ```

</details>
<details><summary>Show 6 hidden projects...</summary>

* <b>[future](https://github.com/PythonCharmers/python-future) ⭐ 1,191 | 🐛 190 | 🌐 Python | 📅 2026-07-24</b> (🥈31 ·  ⭐ 1.2K · 💀) - Easy, clean, reliable Python 2/3 compatibility. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[six](https://github.com/benjaminp/six) ⭐ 1,031 | 🐛 114 | 🌐 Python | 📅 2026-02-23</b> (🥇32 ·  ⭐ 990 · 💀) - Python 2 and 3 compatibility library. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[dataclasses](https://github.com/ericvsmith/dataclasses) ⭐ 593 | 🐛 9 | 🌐 Python | 📅 2024-07-11</b> (🥉24 ·  ⭐ 590 · 💀) - A backport of the dataclasses module for Python 3.6. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[futures](https://github.com/agronholm/pythonfutures) ⭐ 236 | 🐛 5 | 🌐 Python | 📅 2022-10-31</b> (🥉24 ·  ⭐ 240 · 💀) - Backport of the concurrent.futures package to Python 2.6 and 2.7. <code><a href="http://bit.ly/35wkF7y">Python-2.0</a></code>
* <b>[pathlib2](https://github.com/jazzband/pathlib2) ⭐ 86 | 🐛 7 | 🌐 Python | 📅 2023-09-29</b> (🥉25 ·  ⭐ 86) - Backport of pathlib aiming to support the full stdlib Python API. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[contextlib2](https://github.com/jazzband/contextlib2) ⭐ 39 | 🐛 4 | 🌐 Python | 📅 2025-10-06</b> (🥉23 ·  ⭐ 39 · 💤) - contextlib2 is a backport of the standard librarys contextlib.. <code><a href="https://tldrlegal.com/search?q=psfrag">❗️psfrag</a></code>

</details>
<br>

## Cryptography

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/pyca/bcrypt">bcrypt</a></b> (🥈32 ·  ⭐ 1.4K) - Modern(-ish) password hashing for your software and your servers. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/pyca/bcrypt) ⭐ 1,502 | 🐛 8 | 🌐 Python | 📅 2026-09-14 (👨‍💻 41 · 🔀 180 · 📦 360K):

  ```
  git clone https://github.com/pyca/bcrypt
  ```
* [PyPi](https://pypi.org/project/bcrypt) (📥 170M / month · 📦 2K · ⏱️ 25.09.2025):
  ```
  pip install bcrypt
  ```
* [Conda](https://anaconda.org/conda-forge/bcrypt) (📥 13M · ⏱️ 08.09.2026):
  ```
  conda install -c conda-forge bcrypt
  ```

</details>
<details><summary><b><a href="https://github.com/Legrandin/pycryptodome">pycryptodomex</a></b> (🥉31 ·  ⭐ 3.2K) - A self-contained cryptographic library for Python. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/Legrandin/pycryptodome) ⭐ 3,263 | 🐛 96 | 🌐 C | 📅 2026-07-18 (👨‍💻 170 · 🔀 560 · 📦 150K):

  ```
  git clone https://github.com/Legrandin/pycryptodome
  ```
* [PyPi](https://pypi.org/project/pycryptodomex) (📥 50M / month · 📦 1.6K · ⏱️ 17.05.2025):
  ```
  pip install pycryptodomex
  ```
* [Conda](https://anaconda.org/conda-forge/pycryptodomex) (📥 3.1M · ⏱️ 13.09.2025):
  ```
  conda install -c conda-forge pycryptodomex
  ```

</details>
<details><summary>Show 5 hidden projects...</summary>

* <b>[tink](https://github.com/tink-crypto/tink) ⚠️ Archived</b> (🥇34 ·  ⭐ 14K · 💀) - Tink is a multi-language, cross-platform, open source library that.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[cryptography](https://github.com/pyca/cryptography) ⭐ 7,771 | 🐛 37 | 🌐 Python | 📅 2026-09-18</b> (🥇34 ·  ⭐ 7.8K) - cryptography is a package designed to expose.. <code>❗Unlicensed</code>
* <b>[keyring](https://github.com/jaraco/keyring) ⭐ 1,512 | 🐛 108 | 🌐 Python | 📅 2026-04-13</b> (🥉28 ·  ⭐ 1.4K · 💤) - Store and access your passwords safely. <code>❗Unlicensed</code>
* <b>[rsa](https://github.com/sybrenstuvel/python-rsa) ⚠️ Archived</b> (🥉28 ·  ⭐ 500 · 💀) - Python-RSA is a pure-Python RSA implementation. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[asn1crypto](https://github.com/wbond/asn1crypto) ⭐ 362 | 🐛 57 | 🌐 Python | 📅 2026-07-18</b> (🥉28 ·  ⭐ 340 · 💀) - Python ASN.1 library with a focus on performance and a pythonic API. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>

</details>
<br>

## Infrastructure & DevOps

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/boto/botocore">botocore</a></b> (🥇36 ·  ⭐ 1.6K) - The low-level, core functionality of boto3 and the AWS CLI. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/boto/botocore) ⭐ 1,644 | 🐛 196 | 🌐 Python | 📅 2026-09-18 (👨‍💻 220 · 🔀 1.2K · 📦 440K):

  ```
  git clone https://github.com/boto/botocore
  ```
* [PyPi](https://pypi.org/project/botocore) (📥 900M / month · 📦 4.9K · ⏱️ 16.09.2026):
  ```
  pip install botocore
  ```
* [Conda](https://anaconda.org/conda-forge/botocore) (📥 51M · ⏱️ 11.09.2026):
  ```
  conda install -c conda-forge botocore
  ```

</details>
<details><summary><b><a href="https://github.com/ansible/ansible">ansible</a></b> (🥇35 ·  ⭐ 71K) - Ansible is a radically simple IT automation platform that makes your.. <code><a href="http://bit.ly/2M0xdwT">❗️GPL-3.0</a></code></summary>

* [GitHub](https://github.com/ansible/ansible) ⭐ 70,719 | 🐛 837 | 🌐 Python | 📅 2026-09-18 (👨‍💻 7K · 🔀 24K · 📦 43K):

  ```
  git clone https://github.com/ansible/ansible
  ```
* [PyPi](https://pypi.org/project/ansible) (📥 8.8M / month · 📦 600 · ⏱️ 08.09.2026):
  ```
  pip install ansible
  ```
* [Conda](https://anaconda.org/conda-forge/ansible) (📥 2M · ⏱️ 08.09.2026):
  ```
  conda install -c conda-forge ansible
  ```

</details>
<details><summary><b><a href="https://github.com/docker/docker-py">docker</a></b> (🥈34 ·  ⭐ 7.2K) - A Python library for the Docker Engine API. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/docker/docker-py) ⭐ 7,214 | 🐛 564 | 🌐 Python | 📅 2026-09-15 (👨‍💻 460 · 🔀 1.7K · 📦 130K):

  ```
  git clone https://github.com/docker/docker-py
  ```
* [PyPi](https://pypi.org/project/docker) (📥 140M / month · 📦 5.8K · ⏱️ 09.07.2026):
  ```
  pip install docker
  ```
* [Conda](https://anaconda.org/conda-forge/docker-py) (📥 8.2M · ⏱️ 12.07.2026):
  ```
  conda install -c conda-forge docker-py
  ```

</details>
<details><summary><b><a href="https://github.com/pulumi/pulumi">pulumi</a></b> (🥈33 ·  ⭐ 26K) - Pulumi - Infrastructure as Code in any programming language. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/pulumi/pulumi) ⭐ 25,700 | 🐛 2,400 | 🌐 Go | 📅 2026-09-18 (👨‍💻 4.3K · 🔀 1.4K · 📦 11K):

  ```
  git clone https://github.com/pulumi/pulumi
  ```
* [PyPi](https://pypi.org/project/pulumi) (📥 6.6M / month · 📦 590 · ⏱️ 17.09.2026):
  ```
  pip install pulumi
  ```
* [npm](https://www.npmjs.com/package/@pulumi/pulumi) (📥 12M / month · 📦 930 · ⏱️ 17.09.2026):
  ```
  npm install @pulumi/pulumi
  ```

</details>
<details><summary><b><a href="https://github.com/aws/aws-cli">awscli</a></b> (🥈31 ·  ⭐ 17K) - Universal Command Line Interface for Amazon Web Services. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/aws/aws-cli) ⭐ 17,266 | 🐛 740 | 🌐 Python | 📅 2026-09-18 (👨‍💻 510 · 🔀 4.6K · 📦 5):

  ```
  git clone https://github.com/aws/aws-cli
  ```
* [PyPi](https://pypi.org/project/awscli) (📥 100M / month · 📦 780 · ⏱️ 27.08.2026):
  ```
  pip install awscli
  ```
* [Conda](https://anaconda.org/conda-forge/awscli) (📥 63M · ⏱️ 15.09.2026):
  ```
  conda install -c conda-forge awscli
  ```

</details>
<details><summary><b><a href="https://github.com/paramiko/paramiko">paramiko</a></b> (🥈31 ·  ⭐ 9.8K) - The leading native Python SSHv2 protocol library. <code><a href="https://tldrlegal.com/search?q=LGPL-2.1">❗️LGPL-2.1</a></code></summary>

* [GitHub](https://github.com/paramiko/paramiko) ⭐ 9,855 | 🐛 1,195 | 🌐 Python | 📅 2026-08-29 (👨‍💻 190 · 🔀 2.1K):

  ```
  git clone https://github.com/paramiko/paramiko
  ```
* [PyPi](https://pypi.org/project/paramiko) (📥 120M / month · 📦 4.6K · ⏱️ 09.05.2026):
  ```
  pip install paramiko
  ```
* [Conda](https://anaconda.org/conda-forge/paramiko) (📥 10M · ⏱️ 19.05.2026):
  ```
  conda install -c conda-forge paramiko
  ```

</details>
<details><summary><b><a href="https://github.com/kubernetes-client/python">kubernetes</a></b> (🥈31 ·  ⭐ 7.7K) - Official Python client library for kubernetes. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/kubernetes-client/python) ⭐ 7,668 | 🐛 74 | 🌐 Python | 📅 2026-09-17 (👨‍💻 260 · 🔀 3.5K):

  ```
  git clone https://github.com/kubernetes-client/python
  ```
* [PyPi](https://pypi.org/project/kubernetes) (📥 80M / month · 📦 2.6K · ⏱️ 02.09.2026):
  ```
  pip install kubernetes
  ```
* [Conda](https://anaconda.org/conda-forge/kubernetes) (📥 880K · ⏱️ 11.12.2025):
  ```
  conda install -c conda-forge kubernetes
  ```

</details>
<details><summary><b><a href="https://github.com/ktbyers/netmiko">netmiko</a></b> (🥈29 ·  ⭐ 4.2K) - Multi-vendor library to simplify Paramiko SSH connections to network.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/ktbyers/netmiko) ⭐ 4,285 | 🐛 63 | 🌐 Python | 📅 2026-08-18 (👨‍💻 280 · 🔀 1.4K · 📦 5.7K):

  ```
  git clone https://github.com/ktbyers/netmiko
  ```
* [PyPi](https://pypi.org/project/netmiko) (📥 980K / month · 📦 270 · ⏱️ 12.05.2026):
  ```
  pip install netmiko
  ```

</details>
<details><summary><b><a href="https://github.com/tomerfiliba/plumbum">plumbum</a></b> (🥈29 ·  ⭐ 3.1K) - Plumbum: Shell Combinators. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/tomerfiliba/plumbum) ⭐ 3,052 | 🐛 140 | 🌐 Python | 📅 2026-09-02 (👨‍💻 130 · 🔀 200 · 📦 9.7K):

  ```
  git clone https://github.com/tomerfiliba/plumbum
  ```
* [PyPi](https://pypi.org/project/plumbum) (📥 4.8M / month · 📦 390 · ⏱️ 22.07.2026):
  ```
  pip install plumbum
  ```
* [Conda](https://anaconda.org/conda-forge/plumbum) (📥 1.6M · ⏱️ 23.07.2026):
  ```
  conda install -c conda-forge plumbum
  ```

</details>
<details><summary><b><a href="https://github.com/docker/compose">docker-compose</a></b> (🥉28 ·  ⭐ 38K) - Define and run multi-container applications with Docker. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/docker/compose) ⭐ 38,182 | 🐛 104 | 🌐 Go | 📅 2026-09-18 (👨‍💻 670 · 🔀 5.8K · 📦 270):

  ```
  git clone https://github.com/docker/compose
  ```
* [PyPi](https://pypi.org/project/docker-compose) (📥 810K / month · 📦 250 · ⏱️ 10.05.2021):
  ```
  pip install docker-compose
  ```
* [Conda](https://anaconda.org/conda-forge/docker-compose) (📥 930K · ⏱️ 03.09.2026):
  ```
  conda install -c conda-forge docker-compose
  ```

</details>
<details><summary><b><a href="https://github.com/fabric/fabric">fabric</a></b> (🥉28 ·  ⭐ 15K) - Simple, Pythonic remote execution and deployment. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/fabric/fabric) ⭐ 15,503 | 🐛 509 | 🌐 Python | 📅 2026-04-10 (👨‍💻 140 · 🔀 2K · 📦 24):

  ```
  git clone https://github.com/fabric/fabric
  ```
* [PyPi](https://pypi.org/project/fabric) (📥 8.6M / month · 📦 450 · ⏱️ 06.04.2026):
  ```
  pip install fabric
  ```
* [Conda](https://anaconda.org/conda-forge/fabric) (📥 690K · ⏱️ 06.04.2026):
  ```
  conda install -c conda-forge fabric
  ```

</details>
<details><summary><b><a href="https://pyinfra.com">pyinfra</a></b> (🥉24 ·  ⭐ 5.9K) - pyinfra automates/provisions/manages/deploys infrastructure. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 220 · 🔀 530 · 📦 250):

  ```
  git clone https://github.com/Fizzadar/pyinfra
  ```
* [PyPi](https://pypi.org/project/pyinfra) (📥 190K / month · 📦 46 · ⏱️ 27.07.2026):
  ```
  pip install pyinfra
  ```

</details>
<details><summary><b><a href="https://github.com/emre/storm">storm</a></b> (🥉18 ·  ⭐ 3.3K) - Manage your SSH like a boss. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/emre/storm) ⚠️ Archived (👨‍💻 30 · 🔀 150):

  ```
  git clone https://github.com/emre/storm
  ```
* [PyPi](https://pypi.org/project/storm) (📥 3K / month · 📦 4 · ⏱️ 16.09.2026):
  ```
  pip install storm
  ```

</details>
<details><summary>Show 7 hidden projects...</summary>

* <b>[schedule](https://github.com/dbader/schedule) ⭐ 12,270 | 🐛 181 | 🌐 Python | 📅 2024-05-25</b> (🥈29 ·  ⭐ 12K · 💀) - Python job scheduling for humans. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[wssh](https://github.com/aluzzardi/wssh) ⭐ 1,372 | 🐛 26 | 🌐 JavaScript | 📅 2019-02-12</b> (🥉15 ·  ⭐ 1.3K · 💀) - SSH to WebSockets Bridge. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[sshtunnel](https://github.com/pahaz/sshtunnel) ⭐ 1,298 | 🐛 81 | 🌐 Python | 📅 2025-08-27</b> (🥉27 ·  ⭐ 1.2K · 💀) - SSH tunnels to remote server. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[parallel-ssh](https://github.com/ParallelSSH/parallel-ssh) ⭐ 1,281 | 🐛 10 | 🌐 Python | 📅 2026-08-05</b> (🥉22 ·  ⭐ 1.3K · 💤) - Asynchronous parallel SSH client library. <code><a href="https://tldrlegal.com/search?q=LGPL-2.1">❗️LGPL-2.1</a></code>
* <b>[fabtools](https://github.com/fabtools/fabtools) ⭐ 1,251 | 🐛 104 | 🌐 Python | 📅 2019-12-13</b> (🥉20 ·  ⭐ 1.2K · 💀) - Tools for writing awesome Fabric files. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code>
* <b>[pypyr](https://github.com/pypyr/pypyr) ⭐ 645 | 🐛 16 | 🌐 Python | 📅 2023-12-19</b> (🥉16 ·  ⭐ 640 · 💀) - pypyr task-runner cli & api for automation pipelines. Automate.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[Grai](https://github.com/grai-io/grai-core) ⭐ 317 | 🐛 51 | 🌐 Python | 📅 2026-01-30</b> (🥉11 ·  ⭐ 310 · 💤) - Platform to programmatically manage, test, and debug data.. <code><a href="https://tldrlegal.com/search?q=MIT-0">❗️MIT-0</a></code>

</details>
<br>

## Process Utilities

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/amoffat/sh">sh</a></b> (🥈30 ·  ⭐ 7.2K) - Python process launching. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/amoffat/sh) ⭐ 7,241 | 🐛 3 | 🌐 Python | 📅 2026-07-25 (👨‍💻 110 · 🔀 510 · 📦 19K):

  ```
  git clone https://github.com/amoffat/sh
  ```
* [PyPi](https://pypi.org/project/sh) (📥 8.4M / month · 📦 860 · ⏱️ 25.07.2026):
  ```
  pip install sh
  ```
* [Conda](https://anaconda.org/conda-forge/sh) (📥 520K · ⏱️ 06.06.2026):
  ```
  conda install -c conda-forge sh
  ```

</details>
<details><summary><b><a href="https://github.com/Supervisor/supervisor">supervisor</a></b> (🥉29 ·  ⭐ 8.9K · 💤) - Supervisor process control system for Unix.. <code><a href="https://tldrlegal.com/search?q=Repoze%20Public%20License">❗️Repoze Public License</a></code></summary>

* [GitHub](https://github.com/Supervisor/supervisor) ⭐ 9,117 | 🐛 182 | 🌐 Python | 📅 2025-12-21 (👨‍💻 180 · 🔀 1.3K · 📦 14K):

  ```
  git clone https://github.com/Supervisor/supervisor
  ```
* [PyPi](https://pypi.org/project/supervisor) (📥 12M / month · 📦 160 · ⏱️ 23.08.2025):
  ```
  pip install supervisor
  ```
* [Conda](https://anaconda.org/conda-forge/supervisor) (📥 810K · ⏱️ 27.02.2026):
  ```
  conda install -c conda-forge supervisor
  ```

</details>
<details><summary>Show 2 hidden projects...</summary>

* <b>[pexpect](https://github.com/pexpect/pexpect) ⭐ 2,849 | 🐛 171 | 🌐 Python | 📅 2025-04-11</b> (🥇31 ·  ⭐ 2.7K · 💀) - A Python module for controlling interactive programs in a pseudo-.. <code><a href="http://bit.ly/3hkKRql">ISC</a></code>
* <b>[ptyprocess](https://github.com/pexpect/ptyprocess) ⭐ 240 | 🐛 21 | 🌐 Python | 📅 2026-08-26</b> (🥉29 ·  ⭐ 230 · 💀) - Run a subprocess in a pseudo terminal. <code><a href="http://bit.ly/3hkKRql">ISC</a></code>

</details>
<br>

## Asynchronous Programming

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/agronholm/anyio">anyio</a></b> (🥇34 ·  ⭐ 2.4K) - High level asynchronous concurrency and networking framework that works on.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/agronholm/anyio) ⭐ 2,544 | 🐛 112 | 🌐 Python | 📅 2026-09-15 (👨‍💻 100 · 🔀 180 · 📦 680K):

  ```
  git clone https://github.com/agronholm/anyio
  ```
* [PyPi](https://pypi.org/project/anyio) (📥 840M / month · 📦 8.1K · ⏱️ 05.09.2026):
  ```
  pip install anyio
  ```
* [Conda](https://anaconda.org/conda-forge/anyio) (📥 43M · ⏱️ 07.09.2026):
  ```
  conda install -c conda-forge anyio
  ```

</details>
<details><summary><b><a href="https://github.com/MagicStack/uvloop">uvloop</a></b> (🥈33 ·  ⭐ 12K) - Ultra fast asyncio event loop. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/MagicStack/uvloop) ⭐ 11,895 | 🐛 161 | 🌐 Cython | 📅 2026-07-14 (👨‍💻 79 · 🔀 610 · 📦 230K):

  ```
  git clone https://github.com/MagicStack/uvloop
  ```
* [PyPi](https://pypi.org/project/uvloop) (📥 220M / month · 📦 2K · ⏱️ 16.10.2025):
  ```
  pip install uvloop
  ```
* [Conda](https://anaconda.org/conda-forge/uvloop) (📥 5M · ⏱️ 07.11.2025):
  ```
  conda install -c conda-forge uvloop
  ```

</details>
<details><summary><b><a href="https://github.com/fastapi/asyncer">asyncer</a></b> (🥉26 ·  ⭐ 2.5K) - Asyncer, async and await, focused on developer experience. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 25 · 🔀 87 · 📦 8.4K):

  ```
  git clone https://github.com/tiangolo/asyncer
  ```
* [PyPi](https://pypi.org/project/asyncer) (📥 6.6M / month · 📦 320 · ⏱️ 25.06.2026):
  ```
  pip install asyncer
  ```
* [Conda](https://anaconda.org/conda-forge/asyncer) (📥 52K · ⏱️ 25.06.2026):
  ```
  conda install -c conda-forge asyncer
  ```

</details>
<details><summary><b><a href="https://github.com/aiokitchen/aiomisc">aiomisc</a></b> (🥉23 ·  ⭐ 420) - aiomisc - miscellaneous utils for asyncio. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/aiokitchen/aiomisc) ⭐ 425 | 🐛 10 | 🌐 Python | 📅 2026-09-18 (👨‍💻 22 · 🔀 28 · 📦 450):

  ```
  git clone https://github.com/aiokitchen/aiomisc
  ```
* [PyPi](https://pypi.org/project/aiomisc) (📥 62K / month · 📦 61 · ⏱️ 06.09.2026):
  ```
  pip install aiomisc
  ```

</details>
<details><summary>Show 3 hidden projects...</summary>

* <b>[greenlet](https://github.com/python-greenlet/greenlet) ⭐ 1,850 | 🐛 22 | 🌐 C++ | 📅 2026-09-14</b> (🥈32 ·  ⭐ 1.8K) - Lightweight in-process concurrent programming. <code>❗Unlicensed</code>
* <b>[unsync](https://github.com/alex-sherman/unsync) ⭐ 901 | 🐛 3 | 🌐 Python | 📅 2025-01-09</b> (🥉20 ·  ⭐ 900 · 💀) - Unsynchronize asyncio. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[stopit](https://github.com/glenfant/stopit) ⭐ 127 | 🐛 17 | 🌐 Python | 📅 2024-01-09</b> (🥉19 ·  ⭐ 35 · 💤) - Raise asynchronous exceptions in other thread, control the timeout.. <code><a href="http://bit.ly/2M0xdwT">❗️GPL-3.0</a></code>

</details>
<br>

## Configuration

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/theskumar/python-dotenv">python-dotenv</a></b> (🥇36 ·  ⭐ 8.8K) - Reads key-value pairs from a .env file and can set them as.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/theskumar/python-dotenv) ⭐ 8,877 | 🐛 122 | 🌐 Python | 📅 2026-08-23 (👨‍💻 120 · 🔀 560 · 📦 1.8M):

  ```
  git clone https://github.com/theskumar/python-dotenv
  ```
* [PyPi](https://pypi.org/project/python-dotenv) (📥 630M / month · 📦 45K · ⏱️ 16.08.2026):
  ```
  pip install python-dotenv
  ```
* [Conda](https://anaconda.org/conda-forge/python-dotenv) (📥 16M · ⏱️ 17.08.2026):
  ```
  conda install -c conda-forge python-dotenv
  ```

</details>
<details><summary><b><a href="https://github.com/ipython/traitlets">traitlets</a></b> (🥈32 ·  ⭐ 650) - A lightweight Traits like module. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/ipython/traitlets) ⭐ 653 | 🐛 111 | 🌐 Python | 📅 2026-09-03 (👨‍💻 130 · 🔀 210 · 📦 620K):

  ```
  git clone https://github.com/ipython/traitlets
  ```
* [PyPi](https://pypi.org/project/traitlets) (📥 120M / month · 📦 2.3K · ⏱️ 03.08.2026):
  ```
  pip install traitlets
  ```
* [Conda](https://anaconda.org/conda-forge/traitlets) (📥 57M · ⏱️ 03.08.2026):
  ```
  conda install -c conda-forge traitlets
  ```

</details>
<details><summary><b><a href="https://github.com/omry/omegaconf">omegaconf</a></b> (🥈31 ·  ⭐ 2.4K) - Flexible Python configuration system. The last one you will ever need. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/omry/omegaconf) ⭐ 2,430 | 🐛 32 | 🌐 Python | 📅 2026-09-18 (👨‍💻 48 · 🔀 160 · 📦 65K):

  ```
  git clone https://github.com/omry/omegaconf
  ```
* [PyPi](https://pypi.org/project/omegaconf) (📥 31M / month · 📦 2.6K · ⏱️ 07.08.2026):
  ```
  pip install omegaconf
  ```
* [Conda](https://anaconda.org/conda-forge/omegaconf) (📥 2.9M · ⏱️ 10.08.2026):
  ```
  conda install -c conda-forge omegaconf
  ```

</details>
<details><summary><b><a href="https://github.com/dynaconf/dynaconf">Dynaconf</a></b> (🥉26 ·  ⭐ 4.3K) - The dynamic configurator for your Python Project. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 130 · 🔀 340):

  ```
  git clone https://github.com/rochacbruno/dynaconf
  ```
* [PyPi](https://pypi.org/project/dynaconf) (📥 5.1M / month · 📦 780 · ⏱️ 05.08.2026):
  ```
  pip install dynaconf
  ```
* [Conda](https://anaconda.org/conda-forge/dynaconf) (📥 240K · ⏱️ 06.08.2026):
  ```
  conda install -c conda-forge dynaconf
  ```

</details>
<details><summary><b><a href="https://github.com/DiffSK/configobj">configobj</a></b> (🥉26 ·  ⭐ 340) - Python 3+ compatible port of the configobj library. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/DiffSK/configobj) ⭐ 337 | 🐛 74 | 🌐 Python | 📅 2026-08-19 (👨‍💻 13 · 🔀 88 · 📦 50K):

  ```
  git clone https://github.com/DiffSK/configobj
  ```
* [PyPi](https://pypi.org/project/configobj) (📥 5.7M / month · 📦 650 · ⏱️ 21.09.2024):
  ```
  pip install configobj
  ```
* [Conda](https://anaconda.org/conda-forge/configobj) (📥 1.4M · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge configobj
  ```

</details>
<details><summary><b><a href="https://github.com/willkg/everett">everett</a></b> (🥉19 ·  ⭐ 150 · 💤) - configuration library for python projects. <code><a href="http://bit.ly/3postzC">MPL-2.0</a></code></summary>

* [GitHub](https://github.com/willkg/everett) ⚠️ Archived (👨‍💻 9 · 🔀 14 · 📋 96 - 4% open · ⏱️ 15.10.2025):

  ```
  git clone https://github.com/willkg/everett
  ```
* [PyPi](https://pypi.org/project/everett) (📥 440K / month · 📦 22 · ⏱️ 15.10.2025):
  ```
  pip install everett
  ```
* [Conda](https://anaconda.org/conda-forge/everett) (📥 130K · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge everett
  ```

</details>
<details><summary>Show 3 hidden projects...</summary>

* <b>[python-decouple](http://github.com/henriquebastos/python-decouple/) ⭐ 3,037 | 🐛 23 | 🌐 Python | 📅 2024-11-28</b> (🥈27 ·  ⭐ 3K · 💀) - Strict separation of settings from code. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[gin-config](https://github.com/google/gin-config) ⭐ 2,156 | 🐛 61 | 🌐 Python | 📅 2026-09-09</b> (🥉24 ·  ⭐ 2.1K · 💀) - Gin provides a lightweight configuration framework for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[hydra](http://github.com/crankycoder/Hydra) ⭐ 22 | 🐛 7 | 🌐 Python | 📅 2023-12-24</b> (🥉20 ·  ⭐ 20) - A high performance persistent bloom filter. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>

</details>
<br>

## CLI Development

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/pallets/click">click</a></b> (🥇38 ·  ⭐ 18K) - Python composable command line interface toolkit. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/pallets/click) ⭐ 17,674 | 🐛 85 | 🌐 Python | 📅 2026-09-12 (👨‍💻 450 · 🔀 2K · 📦 2.4M):

  ```
  git clone https://github.com/pallets/click
  ```
* [PyPi](https://pypi.org/project/click) (📥 860M / month · 📦 75K · ⏱️ 26.08.2026):
  ```
  pip install click
  ```
* [Conda](https://anaconda.org/conda-forge/click) (📥 74M · ⏱️ 07.09.2026):
  ```
  conda install -c conda-forge click
  ```

</details>
<details><summary><b><a href="https://github.com/Textualize/rich">rich</a></b> (🥇36 ·  ⭐ 56K) - Rich is a Python library for rich text and beautiful formatting in the terminal. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/Textualize/rich) ⭐ 57,387 | 🐛 378 | 🌐 Python | 📅 2026-06-23 (👨‍💻 290 · 🔀 2.2K · 📦 570K):

  ```
  git clone https://github.com/Textualize/rich
  ```
* [PyPi](https://pypi.org/project/rich) (📥 470M / month · 📦 44K · ⏱️ 12.04.2026):
  ```
  pip install rich
  ```
* [Conda](https://anaconda.org/conda-forge/rich) (📥 27M · ⏱️ 12.04.2026):
  ```
  conda install -c conda-forge rich
  ```

</details>
<details><summary><b><a href="https://github.com/fastapi/typer">Typer</a></b> (🥈33 ·  ⭐ 20K) - Typer, build great CLIs. Easy to code. Based on Python type hints. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 110 · 🔀 970 · 📦 280K):

  ```
  git clone https://github.com/tiangolo/typer
  ```
* [PyPi](https://pypi.org/project/typer) (📥 260M / month · 📦 35K · ⏱️ 28.08.2026):
  ```
  pip install typer
  ```
* [Conda](https://anaconda.org/conda-forge/typer) (📥 15M · ⏱️ 28.08.2026):
  ```
  conda install -c conda-forge typer
  ```

</details>
<details><summary><b><a href="https://github.com/kislyuk/argcomplete">argcomplete</a></b> (🥈32 ·  ⭐ 1.6K) - Python and tab completion, better together. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/kislyuk/argcomplete) ⭐ 1,582 | 🐛 73 | 🌐 Python | 📅 2026-08-17 (👨‍💻 88 · 🔀 150 · 📦 76K):

  ```
  git clone https://github.com/kislyuk/argcomplete
  ```
* [PyPi](https://pypi.org/project/argcomplete) (📥 62M / month · 📦 2.4K · ⏱️ 06.08.2026):
  ```
  pip install argcomplete
  ```
* [Conda](https://anaconda.org/conda-forge/argcomplete) (📥 4.1M · ⏱️ 08.08.2026):
  ```
  conda install -c conda-forge argcomplete
  ```

</details>
<details><summary><b><a href="https://github.com/google/python-fire">python-fire</a></b> (🥈30 ·  ⭐ 28K · 💤) - Python Fire is a library for automatically generating.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/google/python-fire) ⭐ 28,225 | 🐛 196 | 🌐 Python | 📅 2026-07-01 (👨‍💻 67 · 🔀 1.5K · 📦 54K):

  ```
  git clone https://github.com/google/python-fire
  ```
* [PyPi](https://pypi.org/project/fire) (📥 18M / month · 📦 3K · ⏱️ 16.08.2025):
  ```
  pip install fire
  ```
* [Conda](https://anaconda.org/conda-forge/fire) (📥 1.7M · ⏱️ 12.09.2025):
  ```
  conda install -c conda-forge fire
  ```

</details>
<details><summary><b><a href="https://github.com/bw2/ConfigArgParse">ConfigArgParse</a></b> (🥉28 ·  ⭐ 760) - Drop-in replacement for argparse with added support for config.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/bw2/ConfigArgParse) ⭐ 758 | 🐛 0 | 🌐 Python | 📅 2026-09-14 (👨‍💻 58 · 🔀 120 · 📦 26K):

  ```
  git clone https://github.com/bw2/ConfigArgParse
  ```
* [PyPi](https://pypi.org/project/configargparse) (📥 18M / month · 📦 820 · ⏱️ 06.09.2026):
  ```
  pip install configargparse
  ```
* [Conda](https://anaconda.org/conda-forge/configargparse) (📥 2.3M · ⏱️ 07.09.2026):
  ```
  conda install -c conda-forge configargparse
  ```

</details>
<details><summary><b><a href="https://github.com/prompt-toolkit/python-prompt-toolkit">python-prompt-toolkit</a></b> (🥉27 ·  ⭐ 11K) - Library for building powerful interactive command line.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/prompt-toolkit/python-prompt-toolkit) ⭐ 10,575 | 🐛 719 | 🌐 Python | 📅 2026-07-26 (👨‍💻 250 · 🔀 800):

  ```
  git clone https://github.com/prompt-toolkit/python-prompt-toolkit
  ```
* [PyPi](https://pypi.org/project/prompt_toolkit) (📥 190M / month · ⏱️ 26.07.2026):
  ```
  pip install prompt_toolkit
  ```
* [Conda](https://anaconda.org/conda-forge/prompt-toolkit) (📥 48M · ⏱️ 27.07.2026):
  ```
  conda install -c conda-forge prompt-toolkit
  ```

</details>
<details><summary><b><a href="https://github.com/tmbo/questionary">questionary</a></b> (🥉27 ·  ⭐ 1.8K · 💤) - Python library to build pretty command line user prompts Easy to.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/tmbo/questionary) ⭐ 2,180 | 🐛 67 | 🌐 Python | 📅 2026-08-18 (👨‍💻 57 · 🔀 99 · 📦 25K):

  ```
  git clone https://github.com/tmbo/questionary
  ```
* [PyPi](https://pypi.org/project/questionary) (📥 22M / month · 📦 1.4K · ⏱️ 28.08.2025):
  ```
  pip install questionary
  ```
* [Conda](https://anaconda.org/conda-forge/questionary) (📥 530K · ⏱️ 08.09.2025):
  ```
  conda install -c conda-forge questionary
  ```

</details>
<details><summary><b><a href="https://github.com/erikrose/blessings">blessings</a></b> (🥉26 ·  ⭐ 1.5K) - A thin, practical wrapper around terminal capabilities in Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/erikrose/blessings) ⭐ 1,488 | 🐛 32 | 🌐 Python | 📅 2025-08-28 (👨‍💻 17 · 🔀 150 · 📦 11K):

  ```
  git clone https://github.com/erikrose/blessings
  ```
* [PyPi](https://pypi.org/project/blessings) (📥 750K / month · 📦 460 · ⏱️ 21.06.2018):
  ```
  pip install blessings
  ```
* [Conda](https://anaconda.org/conda-forge/blessings) (📥 510K · ⏱️ 05.09.2026):
  ```
  conda install -c conda-forge blessings
  ```

</details>
<details><summary><b><a href="https://github.com/jazzband/docopt-ng">docopt-ng</a></b> (🥉24 ·  ⭐ 220) - Humane command line arguments parser. Now with maintenance, typehints,.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/jazzband/docopt-ng) ⭐ 223 | 🐛 14 | 🌐 Python | 📅 2026-07-19 (👨‍💻 54 · 🔀 27 · 📦 870):

  ```
  git clone https://github.com/jazzband/docopt-ng
  ```
* [PyPi](https://pypi.org/project/docopt-ng) (📥 1.8M / month · 📦 100 · ⏱️ 30.05.2023):
  ```
  pip install docopt-ng
  ```

</details>
<details><summary><b><a href="https://github.com/kdeldycke/click-extra">Click Extra</a></b> (🥉18 ·  ⭐ 120) - Drop-in replacement for Click to make user-friendly and.. <code><a href="http://bit.ly/2KucAZR">❗️GPL-2.0</a></code></summary>

* [GitHub](https://github.com/kdeldycke/click-extra) ⭐ 123 | 🐛 6 | 🌐 Python | 📅 2026-09-18 (👨‍💻 16 · 🔀 12 · 📦 140):

  ```
  git clone https://github.com/kdeldycke/click-extra
  ```
* [PyPi](https://pypi.org/project/click-extra) (📥 75K / month · 📦 51 · ⏱️ 15.09.2026):
  ```
  pip install click-extra
  ```

</details>
<details><summary>Show 9 hidden projects...</summary>

* <b>[docopt](https://github.com/docopt/docopt) ⭐ 8,004 | 🐛 267 | 🌐 Python | 📅 2025-06-23</b> (🥈29 ·  ⭐ 8K · 💀) - Create *beautiful* command-line interfaces with Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[asciimatics](https://github.com/peterbrittain/asciimatics) ⭐ 4,303 | 🐛 17 | 🌐 Python | 📅 2026-07-04</b> (🥉24 ·  ⭐ 4.3K · 💀) - A cross platform package to do curses-like operations, plus.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[colorama](https://github.com/tartley/colorama) ⭐ 3,795 | 🐛 143 | 🌐 Python | 📅 2026-05-13</b> (🥈32 ·  ⭐ 3.7K · 💀) - Simple cross-platform colored terminal text in Python. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[bashplotlib](https://github.com/glamp/bashplotlib) ⭐ 1,919 | 🐛 24 | 🌐 Python | 📅 2024-06-23</b> (🥉19 ·  ⭐ 1.9K · 💀) - plotting in the terminal. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[cleo](https://github.com/python-poetry/cleo) ⭐ 1,354 | 🐛 60 | 🌐 Python | 📅 2026-09-07</b> (🥉27 ·  ⭐ 1.3K · 💀) - Cleo allows you to create beautiful and testable command-line interfaces. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[colout](https://github.com/nojhan/colout) ⭐ 1,161 | 🐛 14 | 🌐 Python | 📅 2026-06-14</b> (🥉16 ·  ⭐ 1.2K · 💀) - Color text streams with a polished command line interface. <code><a href="http://bit.ly/2M0xdwT">❗️GPL-3.0</a></code>
* <b>[wcwidth](https://github.com/jquast/wcwidth) ⭐ 465 | 🐛 1 | 🌐 C | 📅 2026-09-18</b> (🥈30 ·  ⭐ 460) - Python library that measures the width of strings in a terminal. <code>❗Unlicensed</code>
* <b>[clint](https://github.com/kennethreitz/clint) ⚠️ Archived</b> (🥉20 ·  ⭐ 95 · 💀) - Python Command Line Interface Tools. <code><a href="http://bit.ly/3hkKRql">ISC</a></code>
* <b>[onecite](https://github.com/HzaCode/OneCite) ⭐ 66 | 🐛 1 | 🌐 Python | 📅 2026-08-06</b> (🥉11 ·  ⭐ 56) - An intelligent toolkit to automatically parse, complete, and format.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>

</details>
<br>

## Development Tools

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

🔗 <b>[best-of-python-dev](https://github.com/ml-tooling/best-of-python-dev) ⭐ 1,302 | 🐛 33 | 🌐 Python | 📅 2026-09-17</b> ( ⭐ 1.2K · 💤)  - A ranked list of awesome python developer tools and libraries...

<br>

## Data Caching

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/tkem/cachetools">cachetools</a></b> (🥇34 ·  ⭐ 2.8K) - Extensible memoizing collections and decorators. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/tkem/cachetools) ⭐ 2,777 | 🐛 2 | 🌐 Python | 📅 2026-09-18 (👨‍💻 26 · 🔀 210 · 📦 700K):

  ```
  git clone https://github.com/tkem/cachetools
  ```
* [PyPi](https://pypi.org/project/cachetools) (📥 250M / month · 📦 5.3K · ⏱️ 16.09.2026):
  ```
  pip install cachetools
  ```
* [Conda](https://anaconda.org/conda-forge/cachetools) (📥 24M · ⏱️ 31.08.2026):
  ```
  conda install -c conda-forge cachetools
  ```

</details>
<details><summary><b><a href="https://github.com/aio-libs/aiocache">aiocache</a></b> (🥈27 ·  ⭐ 1.4K · 💤) - Asyncio cache manager for redis, memcached and memory. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/aio-libs/aiocache) ⭐ 1,437 | 🐛 67 | 🌐 Python | 📅 2026-06-28 (👨‍💻 53 · 🔀 170 · 📦 5.1K):

  ```
  git clone https://github.com/aio-libs/aiocache
  ```
* [PyPi](https://pypi.org/project/aiocache) (📥 7M / month · 📦 150 · ⏱️ 25.09.2024):
  ```
  pip install aiocache
  ```

</details>
<details><summary><b><a href="https://github.com/pydanny/cached-property">cached-property</a></b> (🥈27 ·  ⭐ 700) - A decorator for caching properties in classes. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/pydanny/cached-property) ⭐ 704 | 🐛 42 | 🌐 Python | 📅 2024-12-07 (👨‍💻 25 · 🔀 79 · 📦 59K):

  ```
  git clone https://github.com/pydanny/cached-property
  ```
* [PyPi](https://pypi.org/project/cached-property) (📥 22M / month · 📦 1.6K · ⏱️ 25.10.2024):
  ```
  pip install cached-property
  ```
* [Conda](https://anaconda.org/conda-forge/cached-property) (📥 28M · ⏱️ 25.08.2026):
  ```
  conda install -c conda-forge cached-property
  ```

</details>
<details><summary><b><a href="https://github.com/bbangert/beaker">beaker</a></b> (🥉24 ·  ⭐ 510) - WSGI middleware for sessions and caching. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/bbangert/beaker) ⭐ 546 | 🐛 75 | 🌐 Python | 📅 2026-05-30 (👨‍💻 94 · 🔀 140 · 📦 6.2K):

  ```
  git clone https://github.com/bbangert/beaker
  ```
* [PyPi](https://pypi.org/project/beaker) (📥 170K / month · 📦 94 · ⏱️ 30.05.2026):
  ```
  pip install beaker
  ```
* [Conda](https://anaconda.org/conda-forge/beaker) (📥 200K · ⏱️ 30.05.2026):
  ```
  conda install -c conda-forge beaker
  ```

</details>
<details><summary><b><a href="https://github.com/lericson/pylibmc">pylibmc</a></b> (🥉24 ·  ⭐ 480) - A Python wrapper around the libmemcached interface from TangentOrg. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/lericson/pylibmc) ⭐ 492 | 🐛 40 | 🌐 C | 📅 2025-04-12 (👨‍💻 57 · 🔀 140 · 📦 5.9K):

  ```
  git clone https://github.com/lericson/pylibmc
  ```
* [PyPi](https://pypi.org/project/pylibmc) (📥 420K / month · 📦 79 · ⏱️ 30.08.2022):
  ```
  pip install pylibmc
  ```
* [Conda](https://anaconda.org/conda-forge/pylibmc) (📥 490K · ⏱️ 06.09.2026):
  ```
  conda install -c conda-forge pylibmc
  ```

</details>
<details><summary>Show 1 hidden projects...</summary>

* <b>[cachier](https://github.com/python-cachier/cachier) ⭐ 664 | 🐛 20 | 🌐 Python | 📅 2026-09-01</b> (🥉17 ·  ⭐ 660) - Persistent, stale-free, local and cross-machine caching for.. <code>❗Unlicensed</code>

</details>
<br>

## GUI Development

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

🔗 <b>[best-of-web-python - Web UI](https://github.com/ml-tooling/best-of-web-python) ⭐ 2,759 | 🐛 19 | 📅 2026-09-17</b> ( ⭐ 2.6K · 💤)  - Collection of libraries to implement web-based UIs.

<details><summary><b><a href="https://github.com/kivy/kivy">kivy</a></b> (🥇29 ·  ⭐ 19K) - Open source UI framework written in Python, running on Windows, Linux, macOS,.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/kivy/kivy) ⭐ 19,017 | 🐛 845 | 🌐 Python | 📅 2026-09-18 (👨‍💻 640 · 🔀 3.1K · 📦 18K):

  ```
  git clone https://github.com/kivy/kivy
  ```
* [PyPi](https://pypi.org/project/kivy) (📥 200K / month · 📦 360 · ⏱️ 26.12.2024):
  ```
  pip install kivy
  ```
* [Conda](https://anaconda.org/conda-forge/kivy) (📥 720K · ⏱️ 29.03.2026):
  ```
  conda install -c conda-forge kivy
  ```

</details>
<details><summary><b><a href="https://github.com/hoffstadt/DearPyGui">DearPyGui</a></b> (🥈27 ·  ⭐ 15K) - Dear PyGui: A fast and powerful Graphical User Interface Toolkit for.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/hoffstadt/DearPyGui) ⭐ 15,623 | 🐛 329 | 🌐 C++ | 📅 2026-05-13 (👨‍💻 76 · 🔀 780 · 📦 4.5K):

  ```
  git clone https://github.com/hoffstadt/DearPyGui
  ```
* [PyPi](https://pypi.org/project/dearpygui) (📥 190K / month · 📦 210 · ⏱️ 01.05.2026):
  ```
  pip install dearpygui
  ```

</details>
<details><summary><b><a href="https://github.com/chriskiehl/Gooey">Gooey</a></b> (🥉23 ·  ⭐ 21K) - Turn (almost) any Python command line program into a full GUI application.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/chriskiehl/Gooey) ⭐ 21,904 | 🐛 180 | 🌐 Python | 📅 2026-09-12 (👨‍💻 100 · 🔀 1K · 📦 1.1K):

  ```
  git clone https://github.com/chriskiehl/Gooey
  ```
* [PyPi](https://pypi.org/project/gooey) (📥 4.6K / month · 📦 100 · ⏱️ 29.01.2022):
  ```
  pip install gooey
  ```
* [Conda](https://anaconda.org/conda-forge/gooey) (📥 150K · ⏱️ 26.03.2026):
  ```
  conda install -c conda-forge gooey
  ```

</details>
<details><summary><b><a href="https://github.com/beeware/toga">toga</a></b> (🥉23 ·  ⭐ 5.4K) - A Python native, OS native GUI toolkit. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/beeware/toga) ⭐ 5,414 | 🐛 312 | 🌐 Python | 📅 2026-09-17 (👨‍💻 350 · 🔀 820):

  ```
  git clone https://github.com/beeware/toga
  ```
* [PyPi](https://pypi.org/project/toga) (📥 4.7K / month · 📦 50 · ⏱️ 08.07.2026):
  ```
  pip install toga
  ```
* [npm](https://www.npmjs.com/package/@pybee/toga) (📥 44 / month · 📦 5 · ⏱️ 23.04.2017):
  ```
  npm install @pybee/toga
  ```

</details>
<details><summary>Show 5 hidden projects...</summary>

* <b>[PySimpleGUI](https://github.com/PySimpleGUI/PySimpleGUI) ⭐ 13,817 | 🐛 708 | 🌐 Python | 📅 2026-08-30</b> (🥈24 ·  ⭐ 14K) - PySimpleGUI is a Python package that enables Python.. <code><a href="https://tldrlegal.com/search?q=PySimpleGUI%20License">❗️PySimpleGUI License</a></code>
* <b>[Eel](https://github.com/python-eel/Eel) ⚠️ Archived</b> (🥈25 ·  ⭐ 6.8K · 💀) - A little Python library for making simple Electron-like HTML/JS GUI apps. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[flexx](https://github.com/flexxui/flexx) ⭐ 3,328 | 🐛 97 | 🌐 Python | 📅 2025-01-20</b> (🥉20 ·  ⭐ 3.3K · 💀) - Write desktop and web apps in pure Python. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code>
* <b>[Phoenix](https://github.com/wxWidgets/Phoenix) ⭐ 2,625 | 🐛 609 | 🌐 Python | 📅 2026-09-09</b> (🥉20 ·  ⭐ 2.6K) - wxPythons Project Phoenix. A new implementation of wxPython,.. <code><a href="https://tldrlegal.com/search?q=wxWindows">❗️wxWindows</a></code>
* <b>[enaml](https://github.com/nucleic/enaml) ⭐ 1,575 | 🐛 62 | 🌐 Python | 📅 2026-09-18</b> (🥉19 ·  ⭐ 1.6K · 💤) - Declarative User Interfaces for Python. <code>❗Unlicensed</code>

</details>
<br>

## Computer & Machine Vision

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

🔗 <b>[best-of-ml-python - Computer Vision](https://github.com/ml-tooling/best-of-ml-python) ⭐ 23,811 | 🐛 57 | 📅 2026-09-17</b> ( ⭐ 16K · 💀)  - Collection of computer vision and image processing..

<details><summary>Show 1 hidden projects...</summary>

* <b>[Pipeless](https://github.com/pipeless-ai/pipeless) ⭐ 851 | 🐛 17 | 🌐 Rust | 📅 2024-05-08</b> (🥇12 ·  ⭐ 850 · 💀) - An open-source framework to create and deploy computer vision.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>

</details>
<br>

## Machine Learning & Data Engineering

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

🔗 <b>[best-of-ml-python](https://github.com/ml-tooling/best-of-ml-python) ⭐ 23,811 | 🐛 57 | 📅 2026-09-17</b> ( ⭐ 16K · 💀)  - A ranked list of awesome machine learning Python libraries. Updated..

<br>

## Text Data

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

🔗 <b>[best-of-ml-python - NLP](https://github.com/ml-tooling/best-of-ml-python#text-data--nlp) ⭐ 23,811 | 🐛 57 | 📅 2026-09-17</b> ( ⭐ 16K · 💀)  - Collection of text processing and NLP libraries.

<details><summary><b><a href="https://github.com/chardet/chardet">chardet</a></b> (🥇33 ·  ⭐ 2.7K) - Python character encoding detector. <code><a href="https://tldrlegal.com/search?q=0BSD">❗️0BSD</a></code></summary>

* [GitHub](https://github.com/chardet/chardet) ⭐ 2,666 | 🐛 0 | 🌐 Python | 📅 2026-08-30 (👨‍💻 58 · 🔀 310 · 📦 970K):

  ```
  git clone https://github.com/chardet/chardet
  ```
* [PyPi](https://pypi.org/project/chardet) (📥 180M / month · 📦 7.4K · ⏱️ 14.08.2026):
  ```
  pip install chardet
  ```
* [Conda](https://anaconda.org/conda-forge/chardet) (📥 35M · ⏱️ 17.08.2026):
  ```
  conda install -c conda-forge chardet
  ```
* [npm](https://www.npmjs.com/package/@pypi/chardet) (📥 82 / month · 📦 7 · ⏱️ 20.08.2017):
  ```
  npm install @pypi/chardet
  ```

</details>
<details><summary><b><a href="https://github.com/carpedm20/emoji">emoji</a></b> (🥇31 ·  ⭐ 2K · 📈) - emoji terminal output for Python. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/carpedm20/emoji) ⭐ 2,038 | 🐛 27 | 🌐 Python | 📅 2026-09-16 (👨‍💻 70 · 🔀 300 · 📦 93K):

  ```
  git clone https://github.com/carpedm20/emoji
  ```
* [PyPi](https://pypi.org/project/emoji) (📥 13M / month · 📦 1.6K · ⏱️ 16.09.2026):
  ```
  pip install emoji
  ```
* [Conda](https://anaconda.org/conda-forge/emoji) (📥 280K · ⏱️ 21.09.2025):
  ```
  conda install -c conda-forge emoji
  ```

</details>
<details><summary><b><a href="https://github.com/un33k/python-slugify">python-slugify</a></b> (🥇31 ·  ⭐ 1.6K) - Returns unicode slugs. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/un33k/python-slugify) ⭐ 1,624 | 🐛 4 | 🌐 Python | 📅 2026-09-09 (👨‍💻 40 · 🔀 140 · 📦 120K):

  ```
  git clone https://github.com/un33k/python-slugify
  ```
* [PyPi](https://pypi.org/project/python-slugify) (📥 67M / month · 📦 2.2K · ⏱️ 08.09.2026):
  ```
  pip install python-slugify
  ```
* [Conda](https://anaconda.org/conda-forge/python-slugify) (📥 4.2M · ⏱️ 09.09.2026):
  ```
  conda install -c conda-forge python-slugify
  ```

</details>
<details><summary><b><a href="https://github.com/daviddrysdale/python-phonenumbers">phonenumbers</a></b> (🥈29 ·  ⭐ 3.8K) - Python port of Googles libphonenumber. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/daviddrysdale/python-phonenumbers) ⭐ 3,773 | 🐛 12 | 🌐 Python | 📅 2026-09-18 (👨‍💻 34 · 🔀 440):

  ```
  git clone https://github.com/daviddrysdale/python-phonenumbers
  ```
* [PyPi](https://pypi.org/project/phonenumbers) (📥 35M / month · 📦 1K · ⏱️ 10.09.2026):
  ```
  pip install phonenumbers
  ```
* [Conda](https://anaconda.org/conda-forge/phonenumbers) (📥 2.6M · ⏱️ 10.09.2026):
  ```
  conda install -c conda-forge phonenumbers
  ```

</details>
<details><summary><b><a href="https://github.com/jmoiron/humanize">humanize</a></b> (🥈29 ·  ⭐ 1.7K) - python humanize functions. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/jmoiron/humanize) ⭐ 1,705 | 🐛 1 | 🌐 Python | 📅 2022-07-17 (👨‍💻 62 · 🔀 150):

  ```
  git clone https://github.com/jmoiron/humanize
  ```
* [PyPi](https://pypi.org/project/humanize) (📥 50M / month · 📦 1.9K · ⏱️ 30.06.2026):
  ```
  pip install humanize
  ```
* [Conda](https://anaconda.org/conda-forge/humanize) (📥 1.7M · ⏱️ 01.07.2026):
  ```
  conda install -c conda-forge humanize
  ```

</details>
<details><summary><b><a href="https://github.com/WojciechMula/pyahocorasick">pyahocorasick</a></b> (🥉28 ·  ⭐ 1.1K) - Python module (C extension and plain python) implementing Aho-.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/WojciechMula/pyahocorasick) ⭐ 1,126 | 🐛 36 | 🌐 C | 📅 2026-04-27 (👨‍💻 33 · 🔀 140 · 📦 4.4K):

  ```
  git clone https://github.com/WojciechMula/pyahocorasick
  ```
* [PyPi](https://pypi.org/project/pyahocorasick) (📥 5.7M / month · 📦 270 · ⏱️ 27.04.2026):
  ```
  pip install pyahocorasick
  ```
* [Conda](https://anaconda.org/conda-forge/pyahocorasick) (📥 610K · ⏱️ 05.09.2026):
  ```
  conda install -c conda-forge pyahocorasick
  ```

</details>
<details><summary><b><a href="https://github.com/alexanderlukanin13/coolname">coolname</a></b> (🥉24 ·  ⭐ 180) - Random Name and Slug Generator. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/alexanderlukanin13/coolname) ⭐ 186 | 🐛 3 | 🌐 Python | 📅 2026-07-24 (👨‍💻 7 · 🔀 16 · 📦 4.4K):

  ```
  git clone https://github.com/alexanderlukanin13/coolname
  ```
* [PyPi](https://pypi.org/project/coolname) (📥 7.7M / month · 📦 170 · ⏱️ 23.04.2026):
  ```
  pip install coolname
  ```
* [Conda](https://anaconda.org/conda-forge/coolname) (📥 270K · ⏱️ 29.05.2026):
  ```
  conda install -c conda-forge coolname
  ```

</details>
<details><summary><b><a href="https://github.com/scrapinghub/price-parser">price-parser</a></b> (🥉21 ·  ⭐ 350) - Extract price amount and currency symbol from a raw text string. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/scrapinghub/price-parser) ⭐ 348 | 🐛 39 | 🌐 Python | 📅 2026-08-06 (👨‍💻 19 · 🔀 54 · 📦 570):

  ```
  git clone https://github.com/scrapinghub/price-parser
  ```
* [PyPi](https://pypi.org/project/price-parser) (📥 790K / month · 📦 40 · ⏱️ 19.03.2026):
  ```
  pip install price-parser
  ```

</details>
<details><summary>Show 3 hidden projects...</summary>

* <b>[inflect](https://github.com/jaraco/inflect) ⭐ 1,083 | 🐛 64 | 🌐 Python | 📅 2026-04-13</b> (🥉27 ·  ⭐ 1.1K · 💀) - Correctly generate plurals, ordinals, indefinite articles; convert.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[awesome-slugify](https://github.com/voronind/awesome-slugify) ⭐ 491 | 🐛 18 | 🌐 Python | 📅 2020-04-17</b> (🥉19 ·  ⭐ 480 · 💀) - Python flexible slugify function. <code><a href="http://bit.ly/2M0xdwT">❗️GPL-3.0</a></code>
* <b>[millify](https://github.com/azaitsev/millify) ⭐ 113 | 🐛 13 | 🌐 Python | 📅 2022-04-19</b> (🥉17 ·  ⭐ 110 · 💀) - Convert long numbers into a human-readable format in Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>

</details>
<br>

## Web Development

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

🔗 <b>[best-of-web-python](https://github.com/ml-tooling/best-of-web-python) ⭐ 2,759 | 🐛 19 | 📅 2026-09-17</b> ( ⭐ 2.6K · 💤)  - A ranked list of awesome python libraries for web development...

<br>

## Database Clients

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

*Libraries for connecting to, operating, and querying databases.*

<details><summary><b><a href="https://github.com/boto/boto3">boto3</a></b> (🥇38 ·  ⭐ 9.9K) - AWS SDK for Python (Boto3). <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/boto/boto3) ⭐ 9,904 | 🐛 201 | 🌐 Python | 📅 2026-09-18 (👨‍💻 170 · 🔀 2K · 📦 610K):

  ```
  git clone https://github.com/boto/boto3
  ```
* [PyPi](https://pypi.org/project/boto3) (📥 2.7B / month · 📦 23K · ⏱️ 16.09.2026):
  ```
  pip install boto3
  ```
* [Conda](https://anaconda.org/conda-forge/boto3) (📥 36M · ⏱️ 14.09.2026):
  ```
  conda install -c conda-forge boto3
  ```

</details>
<details><summary><b><a href="https://github.com/sqlalchemy/sqlalchemy">SQLAlchemy</a></b> (🥇37 ·  ⭐ 12K) - The Database Toolkit for Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/sqlalchemy/sqlalchemy) ⭐ 12,164 | 🐛 209 | 🌐 Python | 📅 2026-09-15 (👨‍💻 860 · 🔀 1.8K · 📦 1.2M):

  ```
  git clone https://github.com/sqlalchemy/sqlalchemy
  ```
* [PyPi](https://pypi.org/project/SQLAlchemy) (📥 340M / month · 📦 23K · ⏱️ 15.09.2026):
  ```
  pip install SQLAlchemy
  ```
* [Conda](https://anaconda.org/conda-forge/sqlalchemy) (📥 32M · ⏱️ 12.08.2026):
  ```
  conda install -c conda-forge sqlalchemy
  ```

</details>
<details><summary><b><a href="https://github.com/sqlalchemy/alembic">alembic</a></b> (🥇35 ·  ⭐ 4.4K) - A database migrations tool for SQLAlchemy. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/sqlalchemy/alembic) ⭐ 4,401 | 🐛 119 | 🌐 Python | 📅 2026-09-11 (👨‍💻 230 · 🔀 380 · 📦 350K):

  ```
  git clone https://github.com/sqlalchemy/alembic
  ```
* [PyPi](https://pypi.org/project/alembic) (📥 190M / month · 📦 5K · ⏱️ 11.09.2026):
  ```
  pip install alembic
  ```
* [Conda](https://anaconda.org/conda-forge/alembic) (📥 12M · ⏱️ 13.09.2026):
  ```
  conda install -c conda-forge alembic
  ```

</details>
<details><summary><b><a href="https://github.com/googleapis/python-bigquery">python-bigquery</a></b> (🥇35 ·  ⭐ 800) - This library has moved to.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/googleapis/python-bigquery) ⚠️ Archived (👨‍💻 180 · 🔀 320 · ⏱️ 06.03.2026):

  ```
  git clone https://github.com/googleapis/python-bigquery
  ```
* [PyPi](https://pypi.org/project/google-cloud-bigquery) (📥 150M / month · 📦 2K · ⏱️ 14.09.2026):
  ```
  pip install google-cloud-bigquery
  ```
* [Conda](https://anaconda.org/conda-forge/google-cloud-bigquery) (📥 4.1M · ⏱️ 04.09.2026):
  ```
  conda install -c conda-forge google-cloud-bigquery
  ```

</details>
<details><summary><b><a href="https://github.com/redis/redis-py">redis</a></b> (🥇34 ·  ⭐ 14K) - Redis Python client. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/redis/redis-py) ⭐ 13,644 | 🐛 85 | 🌐 Python | 📅 2026-09-18 (👨‍💻 580 · 🔀 2.7K):

  ```
  git clone https://github.com/redis/redis-py
  ```
* [PyPi](https://pypi.org/project/redis) (📥 210M / month · 📦 13K · ⏱️ 30.07.2026):
  ```
  pip install redis
  ```
* [Conda](https://anaconda.org/conda-forge/redis-py) (📥 2.9M · ⏱️ 31.07.2026):
  ```
  conda install -c conda-forge redis-py
  ```

</details>
<details><summary><b><a href="https://github.com/PyMySQL/PyMySQL">PyMySQL</a></b> (🥇33 ·  ⭐ 7.8K) - MySQL client library for Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/PyMySQL/PyMySQL) ⭐ 7,845 | 🐛 15 | 🌐 Python | 📅 2026-09-18 (👨‍💻 130 · 🔀 1.4K · 📦 290K):

  ```
  git clone https://github.com/PyMySQL/PyMySQL
  ```
* [PyPi](https://pypi.org/project/PyMySQL) (📥 80M / month · 📦 3.4K · ⏱️ 17.09.2026):
  ```
  pip install PyMySQL
  ```
* [Conda](https://anaconda.org/conda-forge/pymysql) (📥 2.6M · ⏱️ 19.05.2026):
  ```
  conda install -c conda-forge pymysql
  ```

</details>
<details><summary><b><a href="https://github.com/Azure/azure-sdk-for-python">azure-storage-blob</a></b> (🥇33 ·  ⭐ 5.6K) - This repository is for active development of the Azure SDK.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/Azure/azure-sdk-for-python) ⭐ 5,601 | 🐛 1,099 | 🌐 Python | 📅 2026-09-18 (👨‍💻 1.3K · 🔀 3.4K · 📦 2.8K):

  ```
  git clone https://github.com/Azure/azure-sdk-for-python
  ```
* [PyPi](https://pypi.org/project/azure-storage-blob) (📥 100M / month · 📦 2.1K · ⏱️ 16.09.2026):
  ```
  pip install azure-storage-blob
  ```
* [Conda](https://anaconda.org/conda-forge/azure-storage-blob) (📥 2M · ⏱️ 27.08.2026):
  ```
  conda install -c conda-forge azure-storage-blob
  ```

</details>
<details><summary><b><a href="https://github.com/googleapis/google-cloud-python">google-cloud-storage</a></b> (🥇33 ·  ⭐ 5.4K) - Google Cloud Client Libraries for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/googleapis/google-cloud-python) ⭐ 5,385 | 🐛 568 | 🌐 Python | 📅 2026-09-18 (👨‍💻 1.1K · 🔀 1.8K · 📦 2.6K):

  ```
  git clone https://github.com/googleapis/google-cloud-python
  ```
* [PyPi](https://pypi.org/project/google-cloud-storage) (📥 170M / month · 📦 3.4K · ⏱️ 08.09.2026):
  ```
  pip install google-cloud-storage
  ```
* [Conda](https://anaconda.org/conda-forge/google-cloud-storage) (📥 7.8M · ⏱️ 09.09.2026):
  ```
  conda install -c conda-forge google-cloud-storage
  ```

</details>
<details><summary><b><a href="https://github.com/googleapis/python-bigquery-pandas">pandas-gbq</a></b> (🥇33 ·  ⭐ 5.4K) - This library has moved to https://github.com/googleapis/google-.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/googleapis/python-bigquery-pandas) ⚠️ Archived (👨‍💻 60 · 🔀 1.8K · 📥 720 · ⏱️ 13.03.2026):

  ```
  git clone https://github.com/googleapis/python-bigquery-pandas
  ```
* [PyPi](https://pypi.org/project/pandas-gbq) (📥 22M / month · 📦 300 · ⏱️ 25.08.2026):
  ```
  pip install pandas-gbq
  ```
* [Conda](https://anaconda.org/conda-forge/pandas-gbq) (📥 2.6M · ⏱️ 01.05.2025):
  ```
  conda install -c conda-forge pandas-gbq
  ```

</details>
<details><summary><b><a href="https://github.com/googleapis/python-bigtable">google-cloud-bigtable</a></b> (🥇33 ·  ⭐ 5.4K) - This library has moved to.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/googleapis/python-bigtable) ⚠️ Archived (👨‍💻 80 · 🔀 1.8K · ⏱️ 01.04.2026):

  ```
  git clone https://github.com/googleapis/python-bigtable
  ```
* [PyPi](https://pypi.org/project/google-cloud-bigtable) (📥 29M / month · 📦 73 · ⏱️ 14.09.2026):
  ```
  pip install google-cloud-bigtable
  ```
* [Conda](https://anaconda.org/conda-forge/google-cloud-bigtable) (📥 250K · ⏱️ 04.09.2026):
  ```
  conda install -c conda-forge google-cloud-bigtable
  ```

</details>
<details><summary><b><a href="https://github.com/elastic/elasticsearch-py">elasticsearch</a></b> (🥇33 ·  ⭐ 4.4K) - Official Python client for Elasticsearch. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/elastic/elasticsearch-py) ⭐ 4,387 | 🐛 67 | 🌐 Python | 📅 2026-09-14 (👨‍💻 230 · 🔀 1.2K · 📦 66K):

  ```
  git clone https://github.com/elastic/elasticsearch-py
  ```
* [PyPi](https://pypi.org/project/elasticsearch) (📥 40M / month · 📦 2.3K · ⏱️ 09.09.2026):
  ```
  pip install elasticsearch
  ```
* [Conda](https://anaconda.org/conda-forge/elasticsearch) (📥 2M · ⏱️ 10.09.2026):
  ```
  conda install -c conda-forge elasticsearch
  ```

</details>
<details><summary><b><a href="https://github.com/boto/s3transfer">s3transfer</a></b> (🥇33 ·  ⭐ 240) - Amazon S3 Transfer Manager for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/boto/s3transfer) ⭐ 240 | 🐛 74 | 🌐 Python | 📅 2026-09-17 (👨‍💻 49 · 🔀 160 · 📦 390K):

  ```
  git clone https://github.com/boto/s3transfer
  ```
* [PyPi](https://pypi.org/project/s3transfer) (📥 680M / month · 📦 940 · ⏱️ 22.07.2026):
  ```
  pip install s3transfer
  ```
* [Conda](https://anaconda.org/conda-forge/s3transfer) (📥 32M · ⏱️ 23.07.2026):
  ```
  conda install -c conda-forge s3transfer
  ```

</details>
<details><summary><b><a href="https://github.com/coleifer/peewee">peewee</a></b> (🥈32 ·  ⭐ 12K) - a small, expressive orm -- supports postgresql, mysql, sqlite, now with.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/coleifer/peewee) ⭐ 11,989 | 🐛 0 | 🌐 Python | 📅 2026-09-14 (👨‍💻 160 · 🔀 1.4K · 📦 24):

  ```
  git clone https://github.com/coleifer/peewee
  ```
* [PyPi](https://pypi.org/project/peewee) (📥 43M / month · 📦 1.4K · ⏱️ 08.09.2026):
  ```
  pip install peewee
  ```
* [Conda](https://anaconda.org/conda-forge/peewee) (📥 2.3M · ⏱️ 08.09.2026):
  ```
  conda install -c conda-forge peewee
  ```

</details>
<details><summary><b><a href="https://github.com/dpkp/kafka-python">kafka-python</a></b> (🥈32 ·  ⭐ 5.9K) - Python client for Apache Kafka. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/dpkp/kafka-python) ⭐ 5,903 | 🐛 20 | 🌐 Python | 📅 2026-09-14 (👨‍💻 240 · 🔀 1.5K · 📦 37K):

  ```
  git clone https://github.com/dpkp/kafka-python
  ```
* [PyPi](https://pypi.org/project/kafka-python) (📥 19M / month · 📦 1.1K · ⏱️ 16.08.2026):
  ```
  pip install kafka-python
  ```
* [Conda](https://anaconda.org/conda-forge/kafka-python) (📥 540K · ⏱️ 16.08.2026):
  ```
  conda install -c conda-forge kafka-python
  ```

</details>
<details><summary><b><a href="https://github.com/fsspec/s3fs">s3fs</a></b> (🥈32 ·  ⭐ 1K) - S3 Filesystem. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/fsspec/s3fs) ⭐ 1,045 | 🐛 174 | 🌐 Python | 📅 2026-09-17 (👨‍💻 160 · 🔀 300 · 📦 29K):

  ```
  git clone https://github.com/fsspec/s3fs
  ```
* [PyPi](https://pypi.org/project/s3fs) (📥 380M / month · 📦 2.1K · ⏱️ 28.07.2026):
  ```
  pip install s3fs
  ```
* [Conda](https://anaconda.org/conda-forge/s3fs) (📥 13M · ⏱️ 01.09.2026):
  ```
  conda install -c conda-forge s3fs
  ```

</details>
<details><summary><b><a href="https://github.com/prometheus/client_python">Prometheus Client</a></b> (🥈31 ·  ⭐ 4.3K) - Prometheus instrumentation library for Python.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/prometheus/client_python) ⭐ 4,375 | 🐛 150 | 🌐 Python | 📅 2026-09-15 (👨‍💻 180 · 🔀 870):

  ```
  git clone https://github.com/prometheus/client_python
  ```
* [PyPi](https://pypi.org/project/prometheus_client) (📥 140M / month · 📦 4.8K · ⏱️ 24.07.2026):
  ```
  pip install prometheus_client
  ```
* [Conda](https://anaconda.org/conda-forge/prometheus_client) (📥 29M · ⏱️ 25.07.2026):
  ```
  conda install -c conda-forge prometheus_client
  ```

</details>
<details><summary><b><a href="https://github.com/confluentinc/confluent-kafka-python">confluent-kafka-python</a></b> (🥈31 ·  ⭐ 510) - Confluents Kafka Python Client. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/confluentinc/confluent-kafka-python) ⭐ 509 | 🐛 219 | 🌐 Python | 📅 2026-09-18 (👨‍💻 150 · 🔀 960 · 📦 18K):

  ```
  git clone https://github.com/confluentinc/confluent-kafka-python
  ```
* [PyPi](https://pypi.org/project/confluent-kafka) (📥 46M / month · 📦 1.1K · ⏱️ 10.09.2026):
  ```
  pip install confluent-kafka
  ```
* [Conda](https://anaconda.org/conda-forge/python-confluent-kafka) (📥 2.2M · ⏱️ 10.09.2026):
  ```
  conda install -c conda-forge python-confluent-kafka
  ```

</details>
<details><summary><b><a href="https://github.com/fastapi/sqlmodel">sqlmodel</a></b> (🥈30 ·  ⭐ 18K) - SQLModel, SQL databases in Python, designed for simplicity,.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code> <code>pydantic</code></summary>

* [GitHub]() (👨‍💻 120 · 🔀 880 · 📦 33K):

  ```
  git clone https://github.com/tiangolo/sqlmodel
  ```
* [PyPi](https://pypi.org/project/sqlmodel) (📥 23M / month · 📦 2K · ⏱️ 28.08.2026):
  ```
  pip install sqlmodel
  ```
* [Conda](https://anaconda.org/conda-forge/sqlmodel) (📥 350K · ⏱️ 04.09.2026):
  ```
  conda install -c conda-forge sqlmodel
  ```

</details>
<details><summary><b><a href="https://github.com/tortoise/tortoise-orm">tortoise-orm</a></b> (🥈29 ·  ⭐ 5.6K) - Familiar asyncio ORM for python, built with relations in mind. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/tortoise/tortoise-orm) ⭐ 5,631 | 🐛 521 | 🌐 Python | 📅 2026-09-18 (👨‍💻 190 · 🔀 500 · 📦 11K):

  ```
  git clone https://github.com/tortoise/tortoise-orm
  ```
* [PyPi](https://pypi.org/project/tortoise-orm) (📥 480K / month · 📦 390 · ⏱️ 10.08.2026):
  ```
  pip install tortoise-orm
  ```
* [Conda](https://anaconda.org/conda-forge/tortoise-orm) (📥 74K · ⏱️ 10.08.2026):
  ```
  conda install -c conda-forge tortoise-orm
  ```

</details>
<details><summary><b><a href="https://github.com/mongodb/mongo-python-driver">pymongo</a></b> (🥈29 ·  ⭐ 4.4K · 📉) - PyMongo - the Official MongoDB Python driver. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/mongodb/mongo-python-driver) ⭐ 4,355 | 🐛 15 | 🌐 Python | 📅 2026-09-18 (👨‍💻 260 · 🔀 1.2K):

  ```
  git clone https://github.com/mongodb/mongo-python-driver
  ```
* [PyPi](https://pypi.org/project/pymongo) (📦 7.1K · ⏱️ 10.09.2026):
  ```
  pip install pymongo
  ```
* [Conda](https://anaconda.org/conda-forge/pymongo) (📥 7.8M · ⏱️ 10.09.2026):
  ```
  conda install -c conda-forge pymongo
  ```

</details>
<details><summary><b><a href="https://aws-sdk-pandas.readthedocs.io/">AWS Data Wrangler</a></b> (🥈29 ·  ⭐ 4.1K) - Pandas on AWS. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code></summary>

* [GitHub]() (👨‍💻 180 · 🔀 740 · 📦 2.6K):

  ```
  git clone https://github.com/awslabs/aws-data-wrangler
  ```
* [PyPi](https://pypi.org/project/awswrangler) (📥 85M / month · 📦 150 · ⏱️ 03.08.2026):
  ```
  pip install awswrangler
  ```
* [Conda](https://anaconda.org/conda-forge/awswrangler) (📥 930K · ⏱️ 04.08.2026):
  ```
  conda install -c conda-forge awswrangler
  ```

</details>
<details><summary><b><a href="https://github.com/PyMySQL/mysqlclient">mysqlclient</a></b> (🥈29 ·  ⭐ 2.5K) - MySQL/MariaDB connector for Python. <code><a href="http://bit.ly/2KucAZR">❗️GPL-2.0</a></code></summary>

* [GitHub](https://github.com/PyMySQL/mysqlclient) ⭐ 2,539 | 🐛 3 | 🌐 Python | 📅 2026-09-11 (👨‍💻 93 · 🔀 440 · 📦 210K):

  ```
  git clone https://github.com/PyMySQL/mysqlclient
  ```
* [PyPi](https://pypi.org/project/mysqlclient) (📥 17M / month · 📦 1.2K · ⏱️ 14.09.2026):
  ```
  pip install mysqlclient
  ```
* [Conda](https://anaconda.org/conda-forge/mysqlclient) (📥 1M · ⏱️ 03.03.2026):
  ```
  conda install -c conda-forge mysqlclient
  ```

</details>
<details><summary><b><a href="https://github.com/apache/cassandra-python-driver/">Cassandra Driver</a></b> (🥈29 ·  ⭐ 1.4K) - Apache Cassandra Python Driver. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub]() (👨‍💻 220 · 🔀 580 · 📦 11K):

  ```
  git clone https://github.com/datastax/python-driver
  ```
* [PyPi](https://pypi.org/project/cassandra-driver) (📥 4.4M / month · 📦 330 · ⏱️ 06.07.2026):
  ```
  pip install cassandra-driver
  ```
* [Conda](https://anaconda.org/conda-forge/cassandra-driver) (📥 700K · ⏱️ 11.07.2026):
  ```
  conda install -c conda-forge cassandra-driver
  ```

</details>
<details><summary><b><a href="https://github.com/ibis-project/ibis">Ibis</a></b> (🥉28 ·  ⭐ 6.5K) - the portable Python dataframe library. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/ibis-project/ibis) ⭐ 6,661 | 🐛 545 | 🌐 Python | 📅 2026-09-17 (👨‍💻 520 · 🔀 720 · 📦 5.4K):

  ```
  git clone https://github.com/ibis-project/ibis
  ```
* [PyPi](https://pypi.org/project/ibis-framework) (📥 3.7M / month · 📦 130 · ⏱️ 07.02.2026):
  ```
  pip install ibis-framework
  ```
* [Conda](https://anaconda.org/conda-forge/ibis-framework) (📥 1.1M · ⏱️ 01.06.2026):
  ```
  conda install -c conda-forge ibis-framework
  ```

</details>
<details><summary><b><a href="https://github.com/ponyorm/pony">Pony</a></b> (🥉28 ·  ⭐ 3.8K) - Pony Object Relational Mapper. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/ponyorm/pony) ⭐ 3,818 | 🐛 358 | 🌐 Python | 📅 2026-08-10 (👨‍💻 34 · 🔀 260 · 📦 4.7K):

  ```
  git clone https://github.com/ponyorm/pony
  ```
* [PyPi](https://pypi.org/project/pony) (📥 540K / month · 📦 190 · ⏱️ 09.08.2026):
  ```
  pip install pony
  ```
* [Conda](https://anaconda.org/conda-forge/pony) (📥 300K · ⏱️ 10.08.2026):
  ```
  conda install -c conda-forge pony
  ```

</details>
<details><summary><b><a href="https://github.com/kayak/pypika">PyPika</a></b> (🥉28 ·  ⭐ 2.9K · 💤) - PyPika is a python SQL query builder that exposes the full.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/kayak/pypika) ⭐ 2,926 | 🐛 233 | 🌐 Python | 📅 2026-02-04 (👨‍💻 120 · 🔀 330 · 📦 48K):

  ```
  git clone https://github.com/kayak/pypika
  ```
* [PyPi](https://pypi.org/project/pypika) (📥 9.3M / month · 📦 230 · ⏱️ 04.02.2026):
  ```
  pip install pypika
  ```
* [Conda](https://anaconda.org/conda-forge/pypika) (📥 120K · ⏱️ 02.02.2026):
  ```
  conda install -c conda-forge pypika
  ```

</details>
<details><summary><b><a href="https://github.com/pudo/dataset">dataset</a></b> (🥉27 ·  ⭐ 4.9K) - Easy-to-use data handling for SQL data stores with support for implicit.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/pudo/dataset) ⭐ 4,870 | 🐛 22 | 🌐 Python | 📅 2026-07-22 (👨‍💻 81 · 🔀 300 · 📦 5.2K):

  ```
  git clone https://github.com/pudo/dataset
  ```
* [PyPi](https://pypi.org/project/dataset) (📥 1.7M / month · 📦 150 · ⏱️ 12.04.2026):
  ```
  pip install dataset
  ```
* [Conda](https://anaconda.org/conda-forge/dataset) (📥 21K · ⏱️ 17.09.2025):
  ```
  conda install -c conda-forge dataset
  ```

</details>
<details><summary><b><a href="https://github.com/MongoEngine/mongoengine">MongoEngine</a></b> (🥉27 ·  ⭐ 4.3K) - A Python Object-Document-Mapper for working with MongoDB. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/MongoEngine/mongoengine) ⭐ 4,349 | 🐛 318 | 🌐 Python | 📅 2026-09-06 (👨‍💻 400 · 🔀 1.2K · 📦 28K):

  ```
  git clone https://github.com/MongoEngine/mongoengine
  ```
* [PyPi](https://pypi.org/project/mongoengine) (📦 450 · ⏱️ 10.03.2026):
  ```
  pip install mongoengine
  ```
* [Conda](https://anaconda.org/conda-forge/mongoengine) (📥 730K · ⏱️ 10.03.2026):
  ```
  conda install -c conda-forge mongoengine
  ```

</details>
<details><summary><b><a href="https://github.com/kennethreitz/records">Records</a></b> (🥉25 ·  ⭐ 7.2K) - SQL for Humans. <code><a href="http://bit.ly/3hkKRql">ISC</a></code></summary>

* [GitHub](https://github.com/kennethreitz/records) ⭐ 7,220 | 🐛 50 | 🌐 Python | 📅 2026-02-09 (👨‍💻 52 · 🔀 570 · 📦 1.3K):

  ```
  git clone https://github.com/kennethreitz/records
  ```
* [PyPi](https://pypi.org/project/records) (📥 340K / month · 📦 47 · ⏱️ 29.03.2024):
  ```
  pip install records
  ```
* [Conda](https://anaconda.org/conda-forge/records) (📥 120K · ⏱️ 01.09.2026):
  ```
  conda install -c conda-forge records
  ```

</details>
<details><summary><b><a href="https://github.com/pynamodb/PynamoDB">PynamoDB</a></b> (🥉25 ·  ⭐ 2.6K · 💤) - A pythonic interface to Amazons DynamoDB. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/pynamodb/PynamoDB) ⭐ 2,647 | 🐛 318 | 🌐 Python | 📅 2026-05-29 (👨‍💻 120 · 🔀 430 · 📦 2K):

  ```
  git clone https://github.com/pynamodb/PynamoDB
  ```
* [PyPi](https://pypi.org/project/pynamodb) (📥 3.1M / month · 📦 86 · ⏱️ 02.06.2025):
  ```
  pip install pynamodb
  ```
* [Conda](https://anaconda.org/conda-forge/pynamodb) (📥 4.2M · ⏱️ 02.06.2025):
  ```
  conda install -c conda-forge pynamodb
  ```

</details>
<details><summary><b><a href="https://github.com/apache/libcloud">libcloud</a></b> (🥉25 ·  ⭐ 2.1K) - Apache Libcloud is a Python library that hides differences between.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/apache/libcloud) ⭐ 2,124 | 🐛 94 | 🌐 Python | 📅 2026-09-14 (👨‍💻 500 · 🔀 930):

  ```
  git clone https://github.com/apache/libcloud
  ```
* [PyPi](https://pypi.org/project/apache-libcloud) (📥 730K / month · 📦 190 · ⏱️ 21.04.2026):
  ```
  pip install apache-libcloud
  ```

</details>
<details><summary><b><a href="https://github.com/piccolo-orm/piccolo">piccolos</a></b> (🥉25 ·  ⭐ 1.9K) - A fast, user friendly ORM and query builder which supports asyncio. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/piccolo-orm/piccolo) ⭐ 1,944 | 🐛 47 | 🌐 Python | 📅 2026-09-14 (👨‍💻 59 · 🔀 110 · 📦 550):

  ```
  git clone https://github.com/piccolo-orm/piccolo
  ```
* [PyPi](https://pypi.org/project/piccolo) (📥 740K / month · 📦 18 · ⏱️ 27.07.2026):
  ```
  pip install piccolo
  ```

</details>
<details><summary><b><a href="https://github.com/minio/minio-py">minio</a></b> (🥉24 ·  ⭐ 1K · 💤) - MinIO Client SDK for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/minio/minio-py) ⭐ 1,074 | 🐛 2 | 🌐 Python | 📅 2026-08-23 (👨‍💻 150 · 🔀 370):

  ```
  git clone https://github.com/minio/minio-py
  ```
* [PyPi](https://pypi.org/project/minio) (📥 9.5M / month · 📦 820 · ⏱️ 27.11.2025):
  ```
  pip install minio
  ```
* [Conda](https://anaconda.org/conda-forge/minio) (📥 400K · ⏱️ 27.11.2025):
  ```
  conda install -c conda-forge minio
  ```

</details>
<details><summary><b><a href="https://github.com/oracle/python-cx_Oracle">cx-Oracle</a></b> (🥉24 ·  ⭐ 900 · 💤) - Obsolete Python interface to Oracle Database, now superseded by.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/oracle/python-cx_Oracle) ⭐ 886 | 🐛 19 | 🌐 C | 📅 2025-05-26 (👨‍💻 17 · 🔀 360 · 📦 9.2K):

  ```
  git clone https://github.com/oracle/python-cx_Oracle
  ```
* [PyPi](https://pypi.org/project/cx-Oracle) (📥 2.6M / month · 📦 430 · ⏱️ 20.07.2023):
  ```
  pip install cx-Oracle
  ```
* [Conda](https://anaconda.org/conda-forge/cx_oracle) (📥 1.1M · ⏱️ 15.01.2026):
  ```
  conda install -c conda-forge cx_oracle
  ```

</details>
<details><summary><b><a href="https://github.com/django-haystack/pysolr">pysolr</a></b> (🥉24 ·  ⭐ 700) - Pysolr Python Solr client. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/django-haystack/pysolr) ⭐ 702 | 🐛 19 | 🌐 Python | 📅 2026-09-18 (👨‍💻 78 · 🔀 340 · 📦 4K):

  ```
  git clone https://github.com/django-haystack/pysolr
  ```
* [PyPi](https://pypi.org/project/pysolr) (📥 330K / month · 📦 87 · ⏱️ 18.11.2025):
  ```
  pip install pysolr
  ```
* [Conda](https://anaconda.org/conda-forge/pysolr) (📥 120K · ⏱️ 18.11.2025):
  ```
  conda install -c conda-forge pysolr
  ```

</details>
<details><summary><b><a href="https://github.com/art049/odmantic">ODMantic</a></b> (🥉23 ·  ⭐ 1.2K) - Sync and Async ODM (Object Document Mapper) for MongoDB based on python.. <code><a href="http://bit.ly/3hkKRql">ISC</a></code></summary>

* [GitHub](https://github.com/art049/odmantic) ⭐ 1,174 | 🐛 118 | 🌐 Python | 📅 2026-05-19 (👨‍💻 23 · 🔀 97 · 📦 3.6K):

  ```
  git clone https://github.com/art049/odmantic
  ```
* [PyPi](https://pypi.org/project/odmantic) (📥 63K / month · 📦 24 · ⏱️ 24.01.2026):
  ```
  pip install odmantic
  ```

</details>
<details><summary><b><a href="https://github.com/neo4j/neo4j-python-driver">neo4j-driver</a></b> (🥉23 ·  ⭐ 1K) - Neo4j Bolt driver for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/neo4j/neo4j-python-driver) ⭐ 1,049 | 🐛 7 | 🌐 Python | 📅 2026-09-17 (👨‍💻 45 · 🔀 210):

  ```
  git clone https://github.com/neo4j/neo4j-python-driver
  ```
* [PyPi](https://pypi.org/project/neo4j-driver) (📥 98K / month · 📦 65 · ⏱️ 15.09.2026):
  ```
  pip install neo4j-driver
  ```
* [Conda](https://anaconda.org/conda-forge/neo4j-python-driver) (📥 530K · ⏱️ 15.09.2026):
  ```
  conda install -c conda-forge neo4j-python-driver
  ```

</details>
<details><summary><b><a href="https://github.com/amol-/depot">filedepot</a></b> (🥉23 ·  ⭐ 160) - Toolkit for storing files and attachments in web applications. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/amol-/depot) ⭐ 166 | 🐛 9 | 🌐 Python | 📅 2026-07-28 (👨‍💻 21 · 🔀 41 · 📦 1.5K):

  ```
  git clone https://github.com/amol-/depot
  ```
* [PyPi](https://pypi.org/project/filedepot) (📥 16K / month · 📦 15 · ⏱️ 22.07.2026):
  ```
  pip install filedepot
  ```

</details>
<details><summary><b><a href="https://github.com/python-happybase/happybase">HappyBase</a></b> (🥉22 ·  ⭐ 610 · 💤) - [UNMAINTAINED] A developer-friendly Python library to interact with.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/python-happybase/happybase) ⭐ 609 | 🐛 34 | 🌐 Python | 📅 2026-03-16 (👨‍💻 22 · 🔀 160 · 📦 1K):

  ```
  git clone https://github.com/python-happybase/happybase
  ```
* [PyPi](https://pypi.org/project/happybase) (📥 56K / month · 📦 44 · ⏱️ 26.09.2025):
  ```
  pip install happybase
  ```
* [Conda](https://anaconda.org/conda-forge/happybase) (📥 390K · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge happybase
  ```

</details>
<details><summary><b><a href="https://github.com/cloudant/python-cloudant">cloudant</a></b> (🥉19 ·  ⭐ 160) - A Python library for Cloudant and CouchDB. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/cloudant/python-cloudant) ⚠️ Archived (👨‍💻 33 · 🔀 55):

  ```
  git clone https://github.com/cloudant/python-cloudant
  ```
* [PyPi](https://pypi.org/project/cloudant) (📥 240K / month · 📦 34 · ⏱️ 26.08.2021):
  ```
  pip install cloudant
  ```
* [Conda](https://anaconda.org/conda-forge/cloudant) (📥 430K · ⏱️ 29.04.2026):
  ```
  conda install -c conda-forge cloudant
  ```

</details>
<details><summary><b><a href="https://github.com/psycopg/psycopg">psycopg3</a></b> (🥉18 ·  ⭐ 2.5K) - New generation PostgreSQL database adapter for the Python.. <code><a href="http://bit.ly/37RvQcA">❗️LGPL-3.0</a></code></summary>

* [GitHub](https://github.com/psycopg/psycopg) ⭐ 2,501 | 🐛 56 | 🌐 Python | 📅 2026-09-18 (👨‍💻 110 · 🔀 270):

  ```
  git clone https://github.com/psycopg/psycopg
  ```

</details>
<details><summary><b><a href="https://github.com/mcfunley/pugsql">PugSQL</a></b> (🥉18 ·  ⭐ 760 · 💤) - A HugSQL-inspired database library for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/mcfunley/pugsql) ⭐ 777 | 🐛 26 | 🌐 Python | 📅 2025-12-04 (👨‍💻 14 · 🔀 27 · 📦 84):

  ```
  git clone https://github.com/mcfunley/pugsql
  ```
* [PyPi](https://pypi.org/project/pugsql) (📥 26K / month · 📦 3 · ⏱️ 04.12.2025):
  ```
  pip install pugsql
  ```

</details>
<details><summary><b><a href="https://github.com/Scille/umongo">umongo</a></b> (🥉18 ·  ⭐ 450 · 📉) - sync/async MongoDB ODM, yes. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/Scille/umongo) ⭐ 447 | 🐛 50 | 🌐 Python | 📅 2026-09-07 (👨‍💻 19 · 🔀 68):

  ```
  git clone https://github.com/Scille/umongo
  ```
* [PyPi](https://pypi.org/project/umongo) (📥 23K / month · 📦 10 · ⏱️ 01.03.2026):
  ```
  pip install umongo
  ```
* [Conda](https://anaconda.org/conda-forge/umongo) (📥 20K · ⏱️ 20.08.2026):
  ```
  conda install -c conda-forge umongo
  ```

</details>
<details><summary>Show 21 hidden projects...</summary>

* <b>[Databases](https://github.com/encode/databases) ⚠️ Archived</b> (🥉27 ·  ⭐ 4K · 💀) - Async database support for Python. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[Elasticsearch DSL](https://github.com/elastic/elasticsearch-dsl-py) ⚠️ Archived</b> (🥇34 ·  ⭐ 3.9K · 💀) - High level Python client for Elasticsearch. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[psycopg2](https://github.com/psycopg/psycopg2) ⭐ 3,654 | 🐛 16 | 🌐 C | 📅 2026-09-10</b> (🥈32 ·  ⭐ 3.7K) - PostgreSQL database adapter for the Python.. <code><a href="https://tldrlegal.com/search?q=BSD-3-Clause-Attribution">❗️BSD-3-Clause-Attribution</a></code>
* <b>[pyodbc](https://github.com/mkleehammer/pyodbc) ⭐ 3,081 | 🐛 60 | 🌐 C++ | 📅 2026-06-06</b> (🥈29 ·  ⭐ 3.1K) - Python ODBC bridge. <code><a href="https://tldrlegal.com/search?q=MIT-0">❗️MIT-0</a></code>
* <b>[gino](https://github.com/python-gino/gino) ⭐ 2,790 | 🐛 54 | 🌐 Python | 📅 2022-02-12</b> (🥉23 ·  ⭐ 2.7K · 💀) - GINO Is Not ORM - a Python asyncio ORM on SQLAlchemy core. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[Motor](https://github.com/mongodb/motor) ⭐ 2,525 | 🐛 1 | 🌐 Python | 📅 2026-09-18</b> (🥈29 ·  ⭐ 2.5K · 💀) - Motor - the async Python driver for MongoDB and Tornado or asyncio. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[prisma](https://github.com/RobertCraigie/prisma-client-py) ⚠️ Archived</b> (🥉24 ·  ⭐ 2.1K · 💀) - Prisma Client Python is an auto-generated and fully type-safe.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[mongo-connector](https://github.com/yougov/mongo-connector) ⭐ 1,872 | 🐛 262 | 🌐 Python | 📅 2024-03-27</b> (🥉22 ·  ⭐ 1.9K · 💀) - MongoDB data stream pipeline tools by YouGov (adopted.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[influxdb](https://github.com/influxdata/influxdb-python) ⚠️ Archived</b> (🥉24 ·  ⭐ 1.7K · 💀) - Python client for InfluxDB. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[pygsheets](https://github.com/nithinmurali/pygsheets) ⭐ 1,544 | 🐛 68 | 🌐 Python | 📅 2025-06-10</b> (🥉25 ·  ⭐ 1.5K · 💀) - Google Sheets Python API v4. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[SQLAlchemy-Utils](https://github.com/kvesteri/sqlalchemy-utils) ⭐ 1,345 | 🐛 231 | 🌐 Python | 📅 2026-07-08</b> (🥉27 ·  ⭐ 1.3K) - Various utility functions and datatypes for SQLAlchemy. <code>❗Unlicensed</code>
* <b>[db.py](https://github.com/yhat/db.py) ⚠️ Archived</b> (🥉15 ·  ⭐ 1.2K · 💀) - db.py is an easier way to interact with your databases. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code>
* <b>[redis-py-cluster](https://github.com/grokzen/redis-py-cluster) ⚠️ Archived</b> (🥉24 ·  ⭐ 1.1K · 💀) - Python cluster client for the official redis cluster... <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[SuperSQLite](https://github.com/plasticityai/supersqlite) ⭐ 708 | 🐛 8 | 🌐 Python | 📅 2020-05-09</b> (🥉13 ·  ⭐ 720 · 💀) - A supercharged SQLite library for Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[lazydata](https://github.com/rstojnic/lazydata) ⚠️ Archived</b> (🥉13 ·  ⭐ 620 · 💀) - Lazydata: Scalable data dependencies for Python projects. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[PyMODM](https://github.com/mongodb/pymodm) ⚠️ Archived</b> (🥉17 ·  ⭐ 350 · 💀) - A Pythonic, object-oriented interface for working with MongoDB. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[pyhdb](https://github.com/SAP-archive/PyHDB) ⚠️ Archived</b> (🥉20 ·  ⭐ 320 · 💀) - SAP HANA Connector in pure Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[Queries](https://github.com/gmr/queries) ⭐ 254 | 🐛 5 | 🌐 Python | 📅 2021-11-16</b> (🥉20 ·  ⭐ 260 · 💀) - PostgreSQL database access simplified. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[gsheets-db-api](https://github.com/betodealmeida/gsheets-db-api) ⭐ 222 | 🐛 15 | 🌐 Python | 📅 2022-12-08</b> (🥉13 ·  ⭐ 210 · 💀) - A Python DB-API and SQLAlchemy dialect to Google Spreasheets. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[aioprometheus](https://github.com/claws/aioprometheus) ⭐ 190 | 🐛 20 | 🌐 Python | 📅 2024-08-21</b> (🥉15 ·  ⭐ 180 · 💀) - A Prometheus Python client library for asyncio-based.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b><a href="https://py2neo.org/">py2neo</a></b> (🥉15 · 💀) - Python client library and toolkit for Neo4j. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>

</details>
<br>

## Data Loading & Extraction

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

*Libraries for loading, collecting, and extracting data from a variety of data sources and formats.*

<details><summary><b><a href="https://github.com/huggingface/datasets">Datasets</a></b> (🥇35 ·  ⭐ 22K) - The largest hub of ready-to-use datasets for AI models with fast,.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/huggingface/datasets) ⭐ 21,988 | 🐛 1,386 | 🌐 Python | 📅 2026-09-17 (👨‍💻 700 · 🔀 3.3K · 📦 130K):

  ```
  git clone https://github.com/huggingface/datasets
  ```
* [PyPi](https://pypi.org/project/datasets) (📥 98M / month · 📦 7.1K · ⏱️ 28.07.2026):
  ```
  pip install datasets
  ```
* [Conda](https://anaconda.org/conda-forge/datasets) (📥 3.5M · ⏱️ 29.07.2026):
  ```
  conda install -c conda-forge datasets
  ```

</details>
<details><summary><b><a href="https://github.com/joke2k/faker">Faker</a></b> (🥇31 ·  ⭐ 19K · 📉) - Faker is a Python package that generates fake data for you. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/joke2k/faker) ⭐ 19,403 | 🐛 25 | 🌐 Python | 📅 2026-09-15 (👨‍💻 680 · 🔀 2.1K · 📦 300K):

  ```
  git clone https://github.com/joke2k/faker
  ```
* [PyPi](https://pypi.org/project/Faker) (📦 3.6K · ⏱️ 14.09.2026):
  ```
  pip install Faker
  ```
* [Conda](https://anaconda.org/conda-forge/faker) (📥 2.2M · ⏱️ 02.09.2026):
  ```
  conda install -c conda-forge faker
  ```

</details>
<details><summary><b><a href="https://github.com/jazzband/tablib">Tablib</a></b> (🥇31 ·  ⭐ 4.8K · 📈) - Python Module for Tabular Datasets in XLS, CSV, JSON, YAML, &c. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/jazzband/tablib) ⭐ 4,757 | 🐛 54 | 🌐 Python | 📅 2026-09-11 (👨‍💻 140 · 🔀 620 · 📦 130K):

  ```
  git clone https://github.com/jazzband/tablib
  ```
* [PyPi](https://pypi.org/project/tablib) (📥 4.9M / month · 📦 230 · ⏱️ 31.07.2026):
  ```
  pip install tablib
  ```
* [Conda](https://anaconda.org/conda-forge/tablib) (📥 390K · ⏱️ 31.07.2026):
  ```
  conda install -c conda-forge tablib
  ```

</details>
<details><summary><b><a href="https://github.com/ahupp/python-magic">python-magic</a></b> (🥇31 ·  ⭐ 2.9K) - A python wrapper for libmagic. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/ahupp/python-magic) ⭐ 2,919 | 🐛 27 | 🌐 Python | 📅 2026-07-20 (👨‍💻 67 · 🔀 300 · 📦 87K):

  ```
  git clone https://github.com/ahupp/python-magic
  ```
* [PyPi](https://pypi.org/project/python-magic) (📥 30M / month · 📦 1.5K · ⏱️ 07.06.2022):
  ```
  pip install python-magic
  ```
* [Conda](https://anaconda.org/conda-forge/python-magic) (📥 980K · ⏱️ 19.06.2026):
  ```
  conda install -c conda-forge python-magic
  ```

</details>
<details><summary><b><a href="https://github.com/martinblech/xmltodict">xmltodict</a></b> (🥈30 ·  ⭐ 5.7K · 💤) - Python module that makes working with XML feel like you are.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/martinblech/xmltodict) ⭐ 5,756 | 🐛 5 | 🌐 Python | 📅 2026-08-19 (👨‍💻 60 · 🔀 470 · 📦 89K):

  ```
  git clone https://github.com/martinblech/xmltodict
  ```
* [PyPi](https://pypi.org/project/xmltodict) (📥 100M / month · 📦 3.9K · ⏱️ 22.02.2026):
  ```
  pip install xmltodict
  ```
* [Conda](https://anaconda.org/conda-forge/xmltodict) (📥 7.6M · ⏱️ 22.02.2026):
  ```
  conda install -c conda-forge xmltodict
  ```

</details>
<details><summary><b><a href="https://github.com/wkentaro/gdown">gdown</a></b> (🥈29 ·  ⭐ 5.4K) - Google Drive public file downloader when curl/wget fails. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/wkentaro/gdown) ⭐ 5,430 | 🐛 0 | 🌐 Python | 📅 2026-09-18 (👨‍💻 27 · 🔀 430 · 📦 60K):

  ```
  git clone https://github.com/wkentaro/gdown
  ```
* [PyPi](https://pypi.org/project/gdown) (📥 3M / month · 📦 1.6K · ⏱️ 17.09.2026):
  ```
  pip install gdown
  ```
* [Conda](https://anaconda.org/conda-forge/gdown) (📥 580K · ⏱️ 06.09.2026):
  ```
  conda install -c conda-forge gdown
  ```

</details>
<details><summary><b><a href="https://github.com/xlwings/xlwings">xlwings</a></b> (🥈29 ·  ⭐ 3.4K) - xlwings is a Python library that makes it easy to call Python from.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/xlwings/xlwings) ⭐ 3,407 | 🐛 385 | 🌐 Python | 📅 2026-09-18 (👨‍💻 69 · 🔀 540 · 📦 34K):

  ```
  git clone https://github.com/xlwings/xlwings
  ```
* [PyPi](https://pypi.org/project/xlwings) (📥 750K / month · 📦 310 · ⏱️ 16.09.2026):
  ```
  pip install xlwings
  ```
* [Conda](https://anaconda.org/conda-forge/xlwings) (📥 2.1M · ⏱️ 01.09.2026):
  ```
  conda install -c conda-forge xlwings
  ```

</details>
<details><summary><b><a href="https://github.com/deanmalmgren/textract">textract</a></b> (🥈27 ·  ⭐ 4.7K) - extract text from any document. no muss. no fuss. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/deanmalmgren/textract) ⭐ 4,716 | 🐛 57 | 🌐 HTML | 📅 2026-09-01 (👨‍💻 48 · 🔀 720 · 📦 4.9K):

  ```
  git clone https://github.com/deanmalmgren/textract
  ```
* [PyPi](https://pypi.org/project/textract) (📥 290K / month · 📦 97 · ⏱️ 01.09.2026):
  ```
  pip install textract
  ```
* [Conda](https://anaconda.org/conda-forge/textract) (📥 150K · ⏱️ 01.09.2026):
  ```
  conda install -c conda-forge textract
  ```

</details>
<details><summary><b><a href="https://github.com/piskvorky/smart_open">smart-open</a></b> (🥈27 ·  ⭐ 3.4K) - Utils for streaming large files (S3, HDFS, GCS, SFTP, Azure Blob.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 130 · 🔀 390):

  ```
  git clone https://github.com/RaRe-Technologies/smart_open
  ```
* [PyPi](https://pypi.org/project/smart-open) (📥 61M / month · 📦 700 · ⏱️ 15.07.2026):
  ```
  pip install smart-open
  ```
* [Conda](https://anaconda.org/conda-forge/smart_open) (📥 5.2M · ⏱️ 15.07.2026):
  ```
  conda install -c conda-forge smart_open
  ```

</details>
<details><summary><b><a href="https://github.com/wireservice/csvkit">csvkit</a></b> (🥈24 ·  ⭐ 6.4K) - A suite of utilities for converting to and working with CSV, the king of.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/wireservice/csvkit) ⭐ 6,408 | 🐛 44 | 🌐 Python | 📅 2026-09-17 (👨‍💻 120 · 🔀 670 · 📦 2K):

  ```
  git clone https://github.com/wireservice/csvkit
  ```
* [PyPi](https://pypi.org/project/csvkit) (📥 250K / month · 📦 49 · ⏱️ 15.12.2025):
  ```
  pip install csvkit
  ```
* [Conda](https://anaconda.org/conda-forge/csvkit) (📥 380K · ⏱️ 26.04.2026):
  ```
  conda install -c conda-forge csvkit
  ```

</details>
<details><summary><b><a href="https://github.com/intake/intake">Intake</a></b> (🥈24 ·  ⭐ 1.1K) - Intake is a lightweight package for finding, investigating, loading and.. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/intake/intake) ⭐ 1,084 | 🐛 114 | 🌐 Python | 📅 2026-09-11 (👨‍💻 97 · 🔀 150 · 📦 3.7K):

  ```
  git clone https://github.com/intake/intake
  ```
* [PyPi](https://pypi.org/project/intake) (📥 74K / month · 📦 230 · ⏱️ 17.06.2026):
  ```
  pip install intake
  ```
* [Conda](https://anaconda.org/conda-forge/intake) (📥 960K · ⏱️ 09.03.2026):
  ```
  conda install -c conda-forge intake
  ```

</details>
<details><summary><b><a href="https://github.com/rom1504/img2dataset">img2dataset</a></b> (🥉23 ·  ⭐ 4.2K · 💤) - Easily turn large sets of image urls to an image dataset. Can.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/rom1504/img2dataset) ⭐ 4,452 | 🐛 145 | 🌐 Python | 📅 2025-10-19 (👨‍💻 36 · 🔀 360 · 📦 290):

  ```
  git clone https://github.com/rom1504/img2dataset
  ```
* [PyPi](https://pypi.org/project/img2dataset) (📥 68K / month · 📦 6 · ⏱️ 09.08.2025):
  ```
  pip install img2dataset
  ```

</details>
<details><summary><b><a href="https://github.com/camelot-dev/camelot">camelot</a></b> (🥉23 ·  ⭐ 3.8K) - A Python library to extract tabular data from PDFs. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/camelot-dev/camelot) ⭐ 3,822 | 🐛 55 | 🌐 Python | 📅 2026-09-18 (👨‍💻 80 · 🔀 550 · 📦 3.7K):

  ```
  git clone https://github.com/camelot-dev/camelot
  ```
* [PyPi](https://pypi.org/project/camelot) (📥 2.8K / month · 📦 6 · ⏱️ 15.12.2021):
  ```
  pip install camelot
  ```

</details>
<details><summary><b><a href="https://github.com/snorkel-team/snorkel">snorkel</a></b> (🥉22 ·  ⭐ 6K) - A system for quickly generating training data with weak supervision. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/snorkel-team/snorkel) ⭐ 6,010 | 🐛 18 | 🌐 Python | 📅 2026-09-14 (👨‍💻 81 · 🔀 850 · 📦 680):

  ```
  git clone https://github.com/snorkel-team/snorkel
  ```
* [PyPi](https://pypi.org/project/snorkel) (📥 37K / month · 📦 22 · ⏱️ 27.02.2024):
  ```
  pip install snorkel
  ```
* [Conda](https://anaconda.org/conda-forge/snorkel) (📥 76K · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge snorkel
  ```

</details>
<details><summary><b><a href="https://github.com/singer-io/getting-started">Singer</a></b> (🥉22 ·  ⭐ 1.3K · 💤) - Standard for moving data between databases, web APIs, files,.. <code><a href="http://bit.ly/3pwmjO5">❗️AGPL-3.0</a></code></summary>

* [GitHub](https://github.com/singer-io/getting-started) ⭐ 1,348 | 🐛 29 | 🌐 Makefile | 📅 2025-08-08 (👨‍💻 33 · 🔀 150):

  ```
  git clone https://github.com/singer-io/getting-started
  ```
* [PyPi](https://pypi.org/project/singer-python) (📥 550K / month · 📦 310 · ⏱️ 27.02.2026):
  ```
  pip install singer-python
  ```

</details>
<details><summary><b><a href="https://github.com/upgini/upgini">Upgini</a></b> (🥉19 ·  ⭐ 360) - Data search & enrichment library for Machine Learning Easily find and add.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/upgini/upgini) ⭐ 357 | 🐛 1 | 🌐 Python | 📅 2026-09-16 (👨‍💻 14 · 🔀 26 · 📦 11):

  ```
  git clone https://github.com/upgini/upgini
  ```
* [PyPi](https://pypi.org/project/upgini) (📥 18K / month · ⏱️ 17.09.2026):
  ```
  pip install upgini
  ```

</details>
<details><summary><b><a href="https://github.com/turicas/rows">rows</a></b> (🥉17 ·  ⭐ 890) - A common, beautiful interface to tabular data, no matter the format. <code><a href="http://bit.ly/37RvQcA">❗️LGPL-3.0</a></code></summary>

* [GitHub](https://github.com/turicas/rows) ⭐ 886 | 🐛 162 | 🌐 Python | 📅 2026-08-08 (👨‍💻 32 · 🔀 140 · 📦 180):

  ```
  git clone https://github.com/turicas/rows
  ```
* [PyPi](https://pypi.org/project/rows) (📥 5.5K / month · 📦 6 · ⏱️ 15.12.2021):
  ```
  pip install rows
  ```

</details>
<details><summary>Show 15 hidden projects...</summary>

* <b>[PDFMiner](https://github.com/euske/pdfminer) ⚠️ Archived</b> (🥈25 ·  ⭐ 5.3K · 💀) - Python PDF Parser (Not actively maintained). Check out pdfminer.six. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[borb](https://github.com/jorisschellekens/ptext-release) ⭐ 3,572 | 🐛 10 | 🌐 Python | 📅 2026-08-26</b> (🥉17 ·  ⭐ 3.6K) - pText is a library for reading, creating and manipulating PDF files.. <code>❗Unlicensed</code>
* <b>[SDV](https://github.com/sdv-dev/SDV) ⭐ 3,559 | 🐛 166 | 🌐 Python | 📅 2026-09-18</b> (🥈24 ·  ⭐ 3.6K) - Synthetic data generation for tabular data. <code>❗Unlicensed</code>
* <b>[pandas-datareader](https://github.com/pydata/pandas-datareader) ⭐ 3,269 | 🐛 147 | 🌐 Python | 📅 2026-07-21</b> (🥈26 ·  ⭐ 3.2K) - Extract data from a wide range of Internet sources.. <code>❗Unlicensed</code>
* <b>[xlrd](https://github.com/python-excel/xlrd) ⭐ 2,206 | 🐛 3 | 🌐 Python | 📅 2026-07-15</b> (🥈30 ·  ⭐ 2.2K · 💀) - Please use openpyxl where you can... <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[excalibur](https://github.com/camelot-dev/excalibur) ⭐ 1,817 | 🐛 117 | 🌐 Python | 📅 2026-05-20</b> (🥉18 ·  ⭐ 1.7K · 💀) - A web interface to extract tabular data from PDFs. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[csvs-to-sqlite](https://github.com/simonw/csvs-to-sqlite) ⭐ 937 | 🐛 42 | 🌐 Python | 📅 2026-07-15</b> (🥉13 ·  ⭐ 900 · 💀) - Convert CSV files into a SQLite database. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[messytables](https://github.com/okfn/messytables) ⭐ 394 | 🐛 37 | 🌐 HTML | 📅 2023-05-22</b> (🥉20 ·  ⭐ 390 · 💀) - Tools for parsing messy tabular data. This is now superseded by.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[datatest](https://github.com/shawnbrown/datatest) ⭐ 295 | 🐛 15 | 🌐 Python | 📅 2021-12-05</b> (🥉18 ·  ⭐ 290 · 💀) - Tools for test driven data-wrangling and data validation. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[deepdish](https://github.com/uchicago-cs/deepdish) ⭐ 274 | 🐛 23 | 🌐 Python | 📅 2024-05-29</b> (🥉19 ·  ⭐ 270 · 💀) - Flexible HDF5 saving/loading and other data science tools from the.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[tabulator-py](https://github.com/frictionlessdata/tabulator-py) ⚠️ Archived</b> (🥉23 ·  ⭐ 240 · 💀) - Python library for reading and writing tabular data via streams. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[pyexcel-xlsx](https://github.com/pyexcel/pyexcel-xlsx) ⭐ 121 | 🐛 3 | 🌐 Python | 📅 2026-07-01</b> (🥉21 ·  ⭐ 120 · 💀) - A wrapper library to read, manipulate and write data in xlsx.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[pdfmux](https://github.com/NameetP/pdfmux) ⭐ 82 | 🐛 4 | 🌐 Python | 📅 2026-09-12</b> (🥉15 ·  ⭐ 77 · 🐣) - PDF extraction that audits its own output and certifies.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code> <code>python</code> <code>pdf</code> <code>markdown</code>
* <b>[Wayback-Archive](https://github.com/GeiserX/Wayback-Archive) ⭐ 35 | 🐛 1 | 🌐 Python | 📅 2026-08-24</b> (🥉6 ·  ⭐ 27 · 🐣) - Download complete websites from the Wayback Machine with.. <code><a href="http://bit.ly/2M0xdwT">❗️GPL-3.0</a></code>
* <b>[Squirrel](https://github.com/merantix-momentum/squirrel-core)</b> (🥉12 ·  ⭐ 280 · 💀) - A Python library that enables ML teams to share, load, and.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>

</details>
<br>

## Data Pipelines & Streaming

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

*Libraries for data batch- and stream-processing, workflow automation, job scheduling, and other data pipeline tasks.*

<details><summary><b><a href="https://github.com/apache/airflow">Airflow</a></b> (🥇35 ·  ⭐ 48K) - Platform to programmatically author, schedule, and monitor workflows. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/apache/airflow) ⭐ 46,886 | 🐛 2,138 | 🌐 Python | 📅 2026-09-18 (👨‍💻 4.6K · 🔀 18K · 📦 20K):

  ```
  git clone https://github.com/apache/airflow
  ```
* [PyPi](https://pypi.org/project/apache-airflow) (📥 10M / month · 📦 1K · ⏱️ 17.09.2026):
  ```
  pip install apache-airflow
  ```
* [Conda](https://anaconda.org/conda-forge/airflow) (📥 2.2M · ⏱️ 20.08.2026):
  ```
  conda install -c conda-forge airflow
  ```
* [Docker Hub](https://hub.docker.com/r/apache/airflow) (📥 1.6B · ⭐ 640 · ⏱️ 17.09.2026):
  ```
  docker pull apache/airflow
  ```

</details>
<details><summary><b><a href="https://github.com/celery/celery">Celery</a></b> (🥇35 ·  ⭐ 29K) - Asynchronous task queue/job queue based on distributed message passing. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/celery/celery) ⭐ 28,897 | 🐛 731 | 🌐 Python | 📅 2026-09-18 (👨‍💻 1.6K · 🔀 5.1K · 📦 190K):

  ```
  git clone https://github.com/celery/celery
  ```
* [PyPi](https://pypi.org/project/celery) (📥 46M / month · 📦 2.6K · ⏱️ 26.03.2026):
  ```
  pip install celery
  ```
* [Conda](https://anaconda.org/conda-forge/celery) (📥 3.2M · ⏱️ 26.07.2026):
  ```
  conda install -c conda-forge celery
  ```

</details>
<details><summary><b><a href="https://github.com/joblib/joblib">joblib</a></b> (🥇35 ·  ⭐ 4.4K) - Computing with Python functions. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/joblib/joblib) ⭐ 4,394 | 🐛 436 | 🌐 Python | 📅 2026-09-15 (👨‍💻 160 · 🔀 470 · 📦 750K):

  ```
  git clone https://github.com/joblib/joblib
  ```
* [PyPi](https://pypi.org/project/joblib) (📥 180M / month · 📦 12K · ⏱️ 31.08.2026):
  ```
  pip install joblib
  ```
* [Conda](https://anaconda.org/conda-forge/joblib) (📥 53M · ⏱️ 31.08.2026):
  ```
  conda install -c conda-forge joblib
  ```

</details>
<details><summary><b><a href="https://github.com/rq/rq">rq</a></b> (🥇33 ·  ⭐ 11K) - Simple job queues for Python. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/rq/rq) ⭐ 10,687 | 🐛 257 | 🌐 Python | 📅 2026-09-17 (👨‍💻 360 · 🔀 1.5K · 📦 22K):

  ```
  git clone https://github.com/rq/rq
  ```
* [PyPi](https://pypi.org/project/rq) (📥 11M / month · 📦 440 · ⏱️ 30.08.2026):
  ```
  pip install rq
  ```
* [Conda](https://anaconda.org/conda-forge/rq) (📥 340K · ⏱️ 01.09.2026):
  ```
  conda install -c conda-forge rq
  ```

</details>
<details><summary><b><a href="https://github.com/apache/beam">Beam</a></b> (🥇33 ·  ⭐ 8.7K) - Unified programming model to define and execute data processing.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/apache/beam) ⭐ 8,668 | 🐛 3,955 | 🌐 Java | 📅 2026-09-18 (👨‍💻 2K · 🔀 4.7K · 📦 9.8K):

  ```
  git clone https://github.com/apache/beam
  ```
* [PyPi](https://pypi.org/project/apache-beam) (📥 3.9M / month · 📦 190 · ⏱️ 31.08.2026):
  ```
  pip install apache-beam
  ```
* [Conda](https://anaconda.org/conda-forge/apache-beam-with-aws) (📥 360K · ⏱️ 11.07.2026):
  ```
  conda install -c conda-forge apache-beam-with-aws
  ```

</details>
<details><summary><b><a href="https://github.com/PrefectHQ/prefect">Prefect</a></b> (🥈32 ·  ⭐ 24K) - Prefect is a workflow orchestration framework for building resilient.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/PrefectHQ/prefect) ⭐ 23,862 | 🐛 856 | 🌐 Python | 📅 2026-09-18 (👨‍💻 760 · 🔀 2.5K · 📦 8.5K):

  ```
  git clone https://github.com/PrefectHQ/prefect
  ```
* [PyPi](https://pypi.org/project/prefect) (📥 8.1M / month · 📦 580 · ⏱️ 16.09.2026):
  ```
  pip install prefect
  ```
* [Conda](https://anaconda.org/conda-forge/prefect) (📥 1.3M · ⏱️ 04.09.2026):
  ```
  conda install -c conda-forge prefect
  ```

</details>
<details><summary><b><a href="https://github.com/dagster-io/dagster">Dagster</a></b> (🥈32 ·  ⭐ 16K) - An orchestration platform for the development, production, and.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/dagster-io/dagster) ⭐ 16,167 | 🐛 2,577 | 🌐 Python | 📅 2026-09-18 (👨‍💻 700 · 🔀 2.3K · 📦 4.9K):

  ```
  git clone https://github.com/dagster-io/dagster
  ```
* [PyPi](https://pypi.org/project/dagster) (📥 8.7M / month · 📦 440 · ⏱️ 16.09.2026):
  ```
  pip install dagster
  ```
* [Conda](https://anaconda.org/conda-forge/dagster) (📥 3.3M · ⏱️ 17.09.2026):
  ```
  conda install -c conda-forge dagster
  ```

</details>
<details><summary><b><a href="https://github.com/spotify/luigi">luigi</a></b> (🥈30 ·  ⭐ 19K) - Luigi is a Python module that helps you build complex pipelines of batch.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/spotify/luigi) ⭐ 18,775 | 🐛 177 | 🌐 Python | 📅 2026-07-18 (👨‍💻 640 · 🔀 2.5K · 📦 2.9K):

  ```
  git clone https://github.com/spotify/luigi
  ```
* [PyPi](https://pypi.org/project/luigi) (📥 970K / month · 📦 180 · ⏱️ 07.05.2026):
  ```
  pip install luigi
  ```
* [Conda](https://anaconda.org/anaconda/luigi) (📥 27K · 📦 3 · ⏱️ 02.09.2026):
  ```
  conda install -c anaconda luigi
  ```

</details>
<details><summary><b><a href="https://greatexpectations.io">Great Expectations</a></b> (🥈29 ·  ⭐ 12K) - Always know what to expect from your data. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub]() (👨‍💻 500 · 🔀 1.8K · 📦 4):

  ```
  git clone https://github.com/great-expectations/great_expectations
  ```
* [PyPi](https://pypi.org/project/great_expectations) (📥 21M / month · 📦 190 · ⏱️ 10.09.2026):
  ```
  pip install great_expectations
  ```
* [Conda](https://anaconda.org/conda-forge/great-expectations) (📥 1.7M · ⏱️ 11.09.2026):
  ```
  conda install -c conda-forge great-expectations
  ```

</details>
<details><summary><b><a href="https://github.com/kedro-org/kedro">Kedro</a></b> (🥈29 ·  ⭐ 11K) - Kedro is a toolbox for production-ready data science. It uses software.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/kedro-org/kedro) ⭐ 11,005 | 🐛 140 | 🌐 Python | 📅 2026-09-18 (👨‍💻 300 · 🔀 1.1K · 📦 3.3K):

  ```
  git clone https://github.com/kedro-org/kedro
  ```
* [PyPi](https://pypi.org/project/kedro) (📥 1.2M / month · 📦 170 · ⏱️ 09.09.2026):
  ```
  pip install kedro
  ```

</details>
<details><summary><b><a href="https://getdbt.com">dbt</a></b> (🥈28 ·  ⭐ 14K) - Build analytics the way engineers build applications. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub]() (👨‍💻 450 · 🔀 2.6K · 📦 1.3K):

  ```
  git clone https://github.com/dbt-labs/dbt-core
  ```
* [PyPi](https://pypi.org/project/dbt) (📥 130K / month · 📦 36 · ⏱️ 16.09.2026):
  ```
  pip install dbt
  ```
* [Conda](https://anaconda.org/conda-forge/dbt) (📥 860K · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge dbt
  ```

</details>
<details><summary><b><a href="https://github.com/petl-developers/petl">petl</a></b> (🥈28 ·  ⭐ 1.3K) - Python Extract Transform and Load Tables of Data. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/petl-developers/petl) ⭐ 1,317 | 🐛 87 | 🌐 Python | 📅 2026-09-16 (👨‍💻 75 · 🔀 210 · 📦 4.4K):

  ```
  git clone https://github.com/petl-developers/petl
  ```
* [PyPi](https://pypi.org/project/petl) (📥 770K / month · 📦 57 · ⏱️ 09.09.2026):
  ```
  pip install petl
  ```
* [Conda](https://anaconda.org/conda-forge/petl) (📥 540K · ⏱️ 09.09.2026):
  ```
  conda install -c conda-forge petl
  ```

</details>
<details><summary><b><a href="https://github.com/zenml-io/zenml">zenml</a></b> (🥈27 ·  ⭐ 5.6K) - ZenML : One AI Platform from Pipelines to Agents. https://zenml.io. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/zenml-io/zenml) ⭐ 5,583 | 🐛 168 | 🌐 Python | 📅 2026-09-18 (👨‍💻 160 · 🔀 650 · 📦 1.5K):

  ```
  git clone https://github.com/zenml-io/zenml
  ```
* [PyPi](https://pypi.org/project/zenml) (📥 140K / month · 📦 15 · ⏱️ 04.09.2026):
  ```
  pip install zenml
  ```

</details>
<details><summary><b><a href="https://github.com/python-arq/arq">arq</a></b> (🥈27 ·  ⭐ 2.9K) - Job queues in python with asyncio and redis. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 67 · 🔀 210 · 📦 1.5K):

  ```
  git clone https://github.com/samuelcolvin/arq
  ```
* [PyPi](https://pypi.org/project/arq) (📥 4M / month · 📦 82 · ⏱️ 16.04.2026):
  ```
  pip install arq
  ```
* [Conda](https://anaconda.org/conda-forge/arq) (📥 28K · ⏱️ 03.08.2026):
  ```
  conda install -c conda-forge arq
  ```

</details>
<details><summary><b><a href="https://github.com/coleifer/huey">huey</a></b> (🥈26 ·  ⭐ 6K) - a little task queue for python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/coleifer/huey) ⭐ 6,034 | 🐛 0 | 🌐 Python | 📅 2026-09-15 (👨‍💻 72 · 🔀 400):

  ```
  git clone https://github.com/coleifer/huey
  ```
* [PyPi](https://pypi.org/project/huey) (📥 17M / month · 📦 160 · ⏱️ 04.09.2026):
  ```
  pip install huey
  ```
* [Conda](https://anaconda.org/conda-forge/huey) (📥 240K · ⏱️ 04.09.2026):
  ```
  conda install -c conda-forge huey
  ```

</details>
<details><summary><b><a href="https://github.com/combust/mleap">mleap</a></b> (🥈26 ·  ⭐ 1.5K) - MLeap: Deploy ML Pipelines to Production. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/combust/mleap) ⭐ 1,543 | 🐛 111 | 🌐 Scala | 📅 2026-09-14 (👨‍💻 87 · 🔀 320 · 📦 250):

  ```
  git clone https://github.com/combust/mleap
  ```
* [PyPi](https://pypi.org/project/mleap) (📥 1.8M / month · 📦 11 · ⏱️ 21.07.2026):
  ```
  pip install mleap
  ```
* [Conda](https://anaconda.org/conda-forge/mleap) (📥 120K · ⏱️ 22.07.2026):
  ```
  conda install -c conda-forge mleap
  ```

</details>
<details><summary><b><a href="https://github.com/cocoindex-io/cocoindex">CocoIndex</a></b> (🥈24 ·  ⭐ 6.1K) - Data transformation framework for AI. Ultra performant, with.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/cocoindex-io/cocoindex) ⭐ 11,576 | 🐛 86 | 🌐 Rust | 📅 2026-09-17 (👨‍💻 93 · 🔀 450 · 📦 72):

  ```
  git clone https://github.com/cocoindex-io/cocoindex
  ```
* [PyPi](https://pypi.org/project/cocoindex) (📥 70K / month · 📦 20 · ⏱️ 13.09.2026):
  ```
  pip install cocoindex
  ```

</details>
<details><summary><b><a href="https://github.com/closeio/tasktiger">TaskTiger</a></b> (🥉21 ·  ⭐ 1.5K) - Python task queue using Redis. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/closeio/tasktiger) ⭐ 1,466 | 🐛 50 | 🌐 Python | 📅 2026-08-31 (👨‍💻 35 · 🔀 95 · 📦 33):

  ```
  git clone https://github.com/closeio/tasktiger
  ```
* [PyPi](https://pypi.org/project/tasktiger) (📥 9.9K / month · 📦 2 · ⏱️ 31.08.2026):
  ```
  pip install tasktiger
  ```

</details>
<details><summary><b><a href="https://github.com/nerevu/riko">riko</a></b> (🥉19 ·  ⭐ 1.6K) - A Python stream processing engine modeled after Yahoo! Pipes. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/nerevu/riko) ⭐ 1,607 | 🐛 25 | 🌐 Python | 📅 2026-09-18 (👨‍💻 25 · 🔀 75 · 📦 14):

  ```
  git clone https://github.com/nerevu/riko
  ```
* [PyPi](https://pypi.org/project/riko) (📥 970 / month · ⏱️ 01.09.2026):
  ```
  pip install riko
  ```

</details>
<details><summary><b><a href="https://github.com/databand-ai/dbnd">dbnd</a></b> (🥉19 ·  ⭐ 270) - DBND is an agile pipeline framework that helps data engineering teams.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/databand-ai/dbnd) ⭐ 267 | 🐛 27 | 🌐 Python | 📅 2026-03-04 (👨‍💻 93 · 🔀 33 · 📦 42):

  ```
  git clone https://github.com/databand-ai/dbnd
  ```
* [PyPi](https://pypi.org/project/dbnd) (📥 18K / month · 📦 27 · ⏱️ 05.03.2026):
  ```
  pip install dbnd
  ```

</details>
<details><summary><b><a href="https://github.com/pdpipe/pdpipe">pdpipe</a></b> (🥉17 ·  ⭐ 720) - Easy pipelines for pandas DataFrames. <code><a href="http://bit.ly/34MBwT8">MIT</a></code> <code><img src="https://git.io/JLy1S" style="display:inline;" width="13" height="13"></code></summary>

* [GitHub](https://github.com/pdpipe/pdpipe) ⭐ 731 | 🐛 10 | 🌐 Jupyter Notebook | 📅 2026-08-01 (👨‍💻 14 · 🔀 46 · 📦 67):

  ```
  git clone https://github.com/pdpipe/pdpipe
  ```
* [PyPi](https://pypi.org/project/pdpipe) (📥 1.3K / month · 📦 14 · ⏱️ 06.03.2026):
  ```
  pip install pdpipe
  ```
* [Conda](https://anaconda.org/conda-forge/pdpipe) (📥 67K · ⏱️ 06.03.2026):
  ```
  conda install -c conda-forge pdpipe
  ```

</details>
<details><summary><b><a href="https://github.com/analysiscenter/batchflow">BatchFlow</a></b> (🥉17 ·  ⭐ 200) - BatchFlow helps you conveniently work with random or sequential.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/analysiscenter/batchflow) ⭐ 205 | 🐛 44 | 🌐 Python | 📅 2026-07-10 (👨‍💻 40 · 🔀 45 · 📦 15):

  ```
  git clone https://github.com/analysiscenter/batchflow
  ```
* [PyPi](https://pypi.org/project/batchflow) (📥 730 / month · ⏱️ 16.04.2026):
  ```
  pip install batchflow
  ```

</details>
<details><summary><b><a href="https://github.com/kubeflow/kale">kale</a></b> (🥉13 ·  ⭐ 700) - Convert JupyterNotebooks to Kubeflow Pipelines deployments. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code> <code>jupyter</code></summary>

* [GitHub]() (👨‍💻 29 · 🔀 160):

  ```
  git clone https://github.com/kubeflow-kale/kale
  ```
* [PyPi](https://pypi.org/project/kubeflow-kale) (⏱️ 07.08.2026):
  ```
  pip install kubeflow-kale
  ```

</details>
<details><summary>Show 21 hidden projects...</summary>

* <b>[faust](https://github.com/robinhood/faust) ⭐ 6,824 | 🐛 280 | 🌐 Python | 📅 2024-07-27</b> (🥉22 ·  ⭐ 6.8K · 💀) - Python Stream Processing. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[ploomber](https://github.com/ploomber/ploomber) ⚠️ Archived</b> (🥈24 ·  ⭐ 3.6K · 💀) - The fastest way to build data pipelines. Develop iteratively,.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[whylogs](https://github.com/whylabs/whylogs) ⭐ 2,833 | 🐛 3 | 🌐 Jupyter Notebook | 📅 2025-01-10</b> (🥉22 ·  ⭐ 2.8K · 💀) - Open standard for end-to-end data and ML monitoring for any.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[dpark](https://github.com/douban/dpark) ⚠️ Archived</b> (🥉15 ·  ⭐ 2.7K · 💀) - Python clone of Spark, a MapReduce alike framework in Python. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code> <code>spark</code>
* <b>[mrjob](https://github.com/Yelp/mrjob) ⭐ 2,613 | 🐛 217 | 🌐 Python | 📅 2026-04-02</b> (🥈24 ·  ⭐ 2.6K · 💀) - Run MapReduce jobs on Hadoop or Amazon Web Services. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[PyFunctional](https://github.com/EntilZha/PyFunctional) ⭐ 2,485 | 🐛 12 | 🌐 Python | 📅 2025-03-13</b> (🥉23 ·  ⭐ 2.3K · 💀) - Python library for creating data pipelines with chain.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[Mara Pipelines](https://github.com/mara/mara-pipelines) ⭐ 2,091 | 🐛 26 | 🌐 Python | 📅 2023-12-15</b> (🥉13 ·  ⭐ 2.1K · 💀) - A lightweight opinionated ETL framework, halfway between.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[spark-deep-learning](https://github.com/databricks/spark-deep-learning) ⭐ 1,988 | 🐛 87 | 🌐 Python | 📅 2023-03-30</b> (🥉14 ·  ⭐ 2K · 💀) - Deep Learning Pipelines for Apache Spark. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code> <code>spark</code>
* <b>[bonobo](https://github.com/python-bonobo/bonobo) ⭐ 1,612 | 🐛 107 | 🌐 Python | 📅 2023-05-12</b> (🥉20 ·  ⭐ 1.6K · 💀) - Extract Transform Load for Python 3.5+. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[Pypeline](https://github.com/cgarciae/pypeln) ⭐ 1,597 | 🐛 35 | 🌐 Python | 📅 2023-07-20</b> (🥉20 ·  ⭐ 1.4K · 💀) - Concurrent data pipelines in Python . <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[Optimus](https://github.com/hi-primus/optimus) ⭐ 1,536 | 🐛 30 | 🌐 Python | 📅 2024-12-02</b> (🥉17 ·  ⭐ 1.5K · 💀) - Agile Data Preparation Workflows madeeasy with Pandas,.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code> <code>spark</code>
* <b>[streamparse](https://github.com/Parsely/streamparse) ⭐ 1,503 | 🐛 61 | 🌐 Python | 📅 2026-04-22</b> (🥉17 ·  ⭐ 1.5K · 💀) - streamparse lets you run Python code against real-time.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[Botflow](https://github.com/kkyon/botflow) ⭐ 1,194 | 🐛 4 | 🌐 Python | 📅 2026-02-03</b> (🥉13 ·  ⭐ 740 · 💀) - Python Fast Data driven programming framework for Data pipeline.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[Databolt Flow](https://github.com/d6t/d6tflow) ⭐ 947 | 🐛 11 | 🌐 Python | 📅 2026-06-28</b> (🥉15 ·  ⭐ 950 · 💀) - Python library for building highly effective data science.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[mrq](https://github.com/pricingassistant/mrq) ⭐ 892 | 🐛 63 | 🌐 Python | 📅 2023-06-13</b> (🥉14 ·  ⭐ 880 · 💀) - Mr. Queue - A distributed worker task queue in Python using Redis & gevent. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[bodywork-core](https://github.com/bodywork-ml/bodywork-core) ⚠️ Archived</b> (🥉12 ·  ⭐ 430 · 💀) - ML pipeline orchestration and model deployments on.. <code><a href="http://bit.ly/3pwmjO5">❗️AGPL-3.0</a></code>
* <b>[pysparkling](https://github.com/svenkreiss/pysparkling) ⭐ 270 | 🐛 11 | 🌐 Python | 📅 2024-09-03</b> (🥉19 ·  ⭐ 260 · 💀) - A pure Python implementation of Apache Sparks RDD and DStream.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[RasgoQL](https://github.com/rasgointelligence/RasgoQL) ⭐ 269 | 🐛 6 | 🌐 Jupyter Notebook | 📅 2022-07-05</b> (🥉10 ·  ⭐ 270 · 💀) - Write python locally, execute SQL in your data warehouse. <code><a href="http://bit.ly/3pwmjO5">❗️AGPL-3.0</a></code>
* <b>[flupy](https://github.com/olirice/flupy) ⭐ 196 | 🐛 0 | 🌐 Python | 📅 2026-07-26</b> (🥉21 ·  ⭐ 200) - Fluent data pipelines for python and your shell. <code>❗Unlicensed</code>
* <b>[datajob](https://github.com/vincentclaes/datajob) ⭐ 111 | 🐛 19 | 🌐 Python | 📅 2023-02-08</b> (🥉11 ·  ⭐ 100 · 💀) - Build and deploy a serverless data pipeline on AWS with no effort. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b><a href="https://www.github.com/activeloopai/deeplake">Activeloop</a></b> (🥉21 ·  ⭐ 9.2K · 💀) - Activeloop Deep Lake. <code><a href="http://bit.ly/3postzC">MPL-2.0</a></code>

</details>
<br>

## File Formats

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/yaml/pyyaml">PyYAML</a></b> (🥇33 ·  ⭐ 2.9K · 💤) - Canonical source repository for PyYAML. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/yaml/pyyaml) ⭐ 2,945 | 🐛 369 | 🌐 Python | 📅 2026-06-17 (👨‍💻 40 · 🔀 570 · 📦 1.9M):

  ```
  git clone https://github.com/yaml/pyyaml
  ```
* [PyPi](https://pypi.org/project/pyyaml) (📥 950M / month · 📦 55K · ⏱️ 25.09.2025):
  ```
  pip install pyyaml
  ```
* [Conda](https://anaconda.org/conda-forge/pyyaml) (📥 96M · ⏱️ 04.02.2026):
  ```
  conda install -c conda-forge pyyaml
  ```

</details>
<details><summary><b><a href="https://github.com/jmcnamara/XlsxWriter">XlsxWriter</a></b> (🥉30 ·  ⭐ 3.9K) - A Python module for creating Excel XLSX files. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/jmcnamara/XlsxWriter) ⭐ 3,973 | 🐛 30 | 🌐 Python | 📅 2026-08-04 (👨‍💻 58 · 🔀 660 · 📦 110K):

  ```
  git clone https://github.com/jmcnamara/XlsxWriter
  ```
* [PyPi](https://pypi.org/project/xlsxwriter) (📥 87M / month · 📦 470 · ⏱️ 16.09.2025):
  ```
  pip install xlsxwriter
  ```
* [Conda](https://anaconda.org/conda-forge/xlsxwriter) (📥 5.4M · ⏱️ 12.10.2025):
  ```
  conda install -c conda-forge xlsxwriter
  ```

</details>
<details><summary><b><a href="https://github.com/jmespath/jmespath.py">jmespath</a></b> (🥉25 ·  ⭐ 2.4K · 💤) - JMESPath is a query language for JSON. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/jmespath/jmespath.py) ⭐ 2,450 | 🐛 76 | 🌐 Python | 📅 2026-04-20 (👨‍💻 32 · 🔀 190):

  ```
  git clone https://github.com/jmespath/jmespath.py
  ```
* [PyPi](https://pypi.org/project/jmespath) (📦 2K · ⏱️ 22.01.2026):
  ```
  pip install jmespath
  ```
* [Conda](https://anaconda.org/conda-forge/jmespath) (📥 36M · ⏱️ 23.01.2026):
  ```
  conda install -c conda-forge jmespath
  ```

</details>
<br>

## Code Inspection

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://zepworks.com/deepdiff/">deepdiff</a></b> (🥇29 ·  ⭐ 2.5K) - Deep Difference and Search of any Python object/data. Recreate objects.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub]() (👨‍💻 110 · 🔀 270 · 📦 22K):

  ```
  git clone https://github.com/seperman/deepdiff
  ```
* [PyPi](https://pypi.org/project/deepdiff) (📥 59M / month · 📦 1.5K · ⏱️ 15.05.2026):
  ```
  pip install deepdiff
  ```
* [Conda](https://anaconda.org/conda-forge/deepdiff) (📥 990K · ⏱️ 16.05.2026):
  ```
  conda install -c conda-forge deepdiff
  ```

</details>
<details><summary>Show 3 hidden projects...</summary>

* <b>[typing\_inspect](https://github.com/ilevkivskyi/typing_inspect) ⭐ 375 | 🐛 24 | 🌐 Python | 📅 2026-01-18</b> (🥈24 ·  ⭐ 370 · 💀) - Runtime inspection utilities for Python typing module. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[entrypoints](https://github.com/takluyver/entrypoints) ⭐ 77 | 🐛 4 | 🌐 Python | 📅 2026-08-27</b> (🥉23 ·  ⭐ 74 · 💀) - Discover and load entry points from installed packages. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[importlib-resources](https://github.com/python/importlib_resources) ⭐ 74 | 🐛 9 | 🌐 Python | 📅 2026-04-13</b> (🥈24 ·  ⭐ 74) - Backport of the importlib.resources module. <code>❗Unlicensed</code>

</details>
<br>

## General Utilities

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/more-itertools/more-itertools">more-itertools</a></b> (🥇34 ·  ⭐ 4.1K · 📈) - More routines for operating on iterables, beyond itertools. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/more-itertools/more-itertools) ⭐ 4,092 | 🐛 11 | 🌐 Python | 📅 2026-09-18 (👨‍💻 160 · 🔀 360 · 📦 300K):

  ```
  git clone https://github.com/more-itertools/more-itertools
  ```
* [PyPi](https://pypi.org/project/more-itertools) (📥 240M / month · 📦 5K · ⏱️ 22.05.2026):
  ```
  pip install more-itertools
  ```
* [Conda](https://anaconda.org/conda-forge/more-itertools) (📥 36M · ⏱️ 22.05.2026):
  ```
  conda install -c conda-forge more-itertools
  ```

</details>
<details><summary><b><a href="https://github.com/jd/tenacity">tenacity</a></b> (🥇33 ·  ⭐ 8.6K) - Retrying library for Python. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/jd/tenacity) ⭐ 8,789 | 🐛 61 | 🌐 Python | 📅 2026-09-01 (👨‍💻 120 · 🔀 320 · 📦 410K):

  ```
  git clone https://github.com/jd/tenacity
  ```
* [PyPi](https://pypi.org/project/tenacity) (📥 360M / month · 📦 6.3K · ⏱️ 07.02.2026):
  ```
  pip install tenacity
  ```
* [Conda](https://anaconda.org/conda-forge/tenacity) (📥 16M · ⏱️ 08.02.2026):
  ```
  conda install -c conda-forge tenacity
  ```

</details>
<details><summary><b><a href="https://github.com/python-attrs/attrs">attrs</a></b> (🥇33 ·  ⭐ 5.8K) - Python Classes Without Boilerplate. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/python-attrs/attrs) ⭐ 5,843 | 🐛 157 | 🌐 Python | 📅 2026-09-01 (👨‍💻 190 · 🔀 420 · 📦 1.5M):

  ```
  git clone https://github.com/python-attrs/attrs
  ```
* [PyPi](https://pypi.org/project/attrs) (📥 730M / month · 📦 11K · ⏱️ 19.03.2026):
  ```
  pip install attrs
  ```
* [Conda](https://anaconda.org/conda-forge/attrs) (📥 77M · ⏱️ 19.03.2026):
  ```
  conda install -c conda-forge attrs
  ```

</details>
<details><summary><b><a href="https://github.com/mahmoud/boltons">boltons</a></b> (🥈29 ·  ⭐ 6.9K) - Like builtins, but boltons. 250+ constructs, recipes, and snippets.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/mahmoud/boltons) ⭐ 6,924 | 🐛 89 | 🌐 Python | 📅 2026-09-08 (👨‍💻 120 · 🔀 430 · 📦 12K):

  ```
  git clone https://github.com/mahmoud/boltons
  ```
* [PyPi](https://pypi.org/project/boltons) (📥 23M / month · 📦 460 · ⏱️ 07.09.2026):
  ```
  pip install boltons
  ```
* [Conda](https://anaconda.org/conda-forge/boltons) (📥 18M · ⏱️ 08.09.2026):
  ```
  conda install -c conda-forge boltons
  ```

</details>
<details><summary><b><a href="https://github.com/ets-labs/python-dependency-injector">python-dependency-injector</a></b> (🥈29 ·  ⭐ 4.9K) - Dependency injection framework for Python. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/ets-labs/python-dependency-injector) ⭐ 4,913 | 🐛 211 | 🌐 Python | 📅 2026-09-01 (👨‍💻 49 · 🔀 350 · 📦 5.9K):

  ```
  git clone https://github.com/ets-labs/python-dependency-injector
  ```
* [PyPi](https://pypi.org/project/dependency-injector) (📥 8M / month · 📦 420 · ⏱️ 18.06.2026):
  ```
  pip install dependency-injector
  ```
* [Conda](https://anaconda.org/conda-forge/dependency_injector) (📥 150K · ⏱️ 13.09.2026):
  ```
  conda install -c conda-forge dependency_injector
  ```

</details>
<details><summary><b><a href="https://github.com/Suor/funcy">funcy</a></b> (🥈28 ·  ⭐ 3.5K) - A fancy and practical functional tools. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/Suor/funcy) ⭐ 3,510 | 🐛 15 | 🌐 Python | 📅 2026-09-16 (👨‍💻 40 · 🔀 160 · 📦 16K):

  ```
  git clone https://github.com/Suor/funcy
  ```
* [PyPi](https://pypi.org/project/funcy) (📥 6.4M / month · 📦 450 · ⏱️ 10.09.2026):
  ```
  pip install funcy
  ```
* [Conda](https://anaconda.org/conda-forge/funcy) (📥 1.1M · ⏱️ 10.09.2026):
  ```
  conda install -c conda-forge funcy
  ```

</details>
<details><summary><b><a href="https://github.com/dry-python/returns">returns</a></b> (🥉27 ·  ⭐ 4.3K) - Make your functions return something meaningful, typed, and safe!. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/dry-python/returns) ⭐ 4,365 | 🐛 81 | 🌐 Python | 📅 2026-09-18 (👨‍💻 74 · 🔀 150 · 📦 1.1K):

  ```
  git clone https://github.com/dry-python/returns
  ```
* [PyPi](https://pypi.org/project/returns) (📥 1.3M / month · 📦 240 · ⏱️ 02.08.2026):
  ```
  pip install returns
  ```
* [Conda](https://anaconda.org/conda-forge/returns) (📥 100K · ⏱️ 03.08.2026):
  ```
  conda install -c conda-forge returns
  ```

</details>
<details><summary><b><a href="https://github.com/SethMMorton/natsort">natsort</a></b> (🥉26 ·  ⭐ 910 · 💤) - Simple yet flexible natural sorting in Python. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/SethMMorton/natsort) ⭐ 1,012 | 🐛 10 | 🌐 Python | 📅 2026-09-10 (👨‍💻 26 · 🔀 52 · 📦 50K):

  ```
  git clone https://github.com/SethMMorton/natsort
  ```
* [PyPi](https://pypi.org/project/natsort) (📥 18M / month · 📦 1.5K · ⏱️ 20.06.2023):
  ```
  pip install natsort
  ```
* [Conda](https://anaconda.org/conda-forge/natsort) (📥 3.2M · ⏱️ 12.02.2026):
  ```
  conda install -c conda-forge natsort
  ```

</details>
<details><summary><b><a href="https://github.com/Erotemic/ubelt">ubelt</a></b> (🥉20 ·  ⭐ 740) - A Python utility library with a stdlib like feel and extra batteries... <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub](https://github.com/Erotemic/ubelt) ⭐ 740 | 🐛 11 | 🌐 Python | 📅 2026-07-31 (👨‍💻 6 · 🔀 51):

  ```
  git clone https://github.com/Erotemic/ubelt
  ```
* [PyPi](https://pypi.org/project/ubelt) (📥 38K / month · 📦 140 · ⏱️ 31.07.2026):
  ```
  pip install ubelt
  ```
* [Conda](https://anaconda.org/conda-forge/ubelt) (📥 460K · ⏱️ 06.09.2026):
  ```
  conda install -c conda-forge ubelt
  ```

</details>
<details><summary>Show 6 hidden projects...</summary>

* <b>[toolz](https://github.com/pytoolz/toolz) ⭐ 5,157 | 🐛 133 | 🌐 Python | 📅 2026-09-18</b> (🥈28 ·  ⭐ 5.1K · 💤) - A functional standard library for Python. <code>❗Unlicensed</code>
* <b>[pampy](https://github.com/santinic/pampy) ⭐ 3,522 | 🐛 23 | 🌐 Python | 📅 2025-01-16</b> (🥉20 ·  ⭐ 3.5K · 💀) - Pampy: The Pattern Matching for Python you always dreamed of. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[retrying](https://github.com/rholder/retrying) ⭐ 1,923 | 🐛 50 | 🌐 Python | 📅 2021-08-15</b> (🥉26 ·  ⭐ 1.9K · 💀) - Retrying is an Apache 2.0 licensed general-purpose retrying.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[CommonRegex](https://github.com/madisonmay/CommonRegex) ⭐ 1,583 | 🐛 22 | 🌐 Python | 📅 2023-04-20</b> (🥉20 ·  ⭐ 1.6K · 💀) - A collection of common regular expressions bundled with an easy.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[pinject](https://github.com/google/pinject) ⚠️ Archived</b> (🥉20 ·  ⭐ 1.3K · 💀) - A pythonic dependency injection library. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[ratelimit](https://github.com/tomasbasham/ratelimit) ⭐ 833 | 🐛 32 | 🌐 Python | 📅 2026-04-13</b> (🥉25 ·  ⭐ 760 · 💀) - API Rate Limit Decorator. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>

</details>
<br>

## Python Implementations

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/python/cpython">cpython</a></b> (🥇34 ·  ⭐ 76K) - The Python programming language. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/python/cpython) ⭐ 77,207 | 🐛 9,688 | 🌐 Python | 📅 2026-09-18 (👨‍💻 4K · 🔀 35K · 📦 630K):

  ```
  git clone https://github.com/python/cpython
  ```
* [PyPi](https://pypi.org/project/cpython) (📥 11 / month · 📦 21 · ⏱️ 22.07.2020):
  ```
  pip install cpython
  ```
* [Conda](https://anaconda.org/conda-forge/typing) (📥 4.5M · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge typing
  ```
* [npm](https://www.npmjs.com/package/@buckpkg/python) (📦 6 · ⏱️ 03.08.2017):
  ```
  npm install @buckpkg/python
  ```

</details>
<details><summary><b><a href="https://github.com/micropython/micropython">micropython</a></b> (🥈21 ·  ⭐ 22K) - MicroPython - a lean and efficient Python implementation for.. <code><a href="http://bit.ly/35wkF7y">Python-2.0</a></code></summary>

* [GitHub](https://github.com/micropython/micropython) ⭐ 22,073 | 🐛 1,534 | 🌐 C | 📅 2026-09-16 (👨‍💻 770 · 🔀 8.9K · 📦 21):

  ```
  git clone https://github.com/micropython/micropython
  ```
* [PyPi](https://pypi.org/project/micropython-_markupbase) (⏱️ 10.10.2016):
  ```
  pip install micropython-_markupbase
  ```

</details>
<details><summary>Show 4 hidden projects...</summary>

* <b>[grumpy](https://github.com/google/grumpy) ⚠️ Archived</b> (🥈17 ·  ⭐ 10K · 💀) - Grumpy is a Python to Go source code transcompiler and runtime. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[pyston](https://github.com/pyston/pyston) ⭐ 2,493 | 🐛 51 | 🌐 Python | 📅 2024-08-12</b> (🥉12 ·  ⭐ 2.5K · 💀) - (No longer maintained) A faster and highly-compatible.. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code>
* <b>[stackless](https://github.com/stackless-dev/stackless) ⚠️ Archived</b> (🥉15 ·  ⭐ 1.1K · 💀) - The Stackless Python programming language. <code>❗Unlicensed</code>
* <b>[cl-python](https://github.com/metawilm/cl-python) ⭐ 396 | 🐛 2 | 🌐 Common Lisp | 📅 2024-06-13</b> (🥉7 ·  ⭐ 240 · 💀) - An implementation of Python in Common Lisp. <code>❗Unlicensed</code>

</details>
<br>

## Others

<a href="#contents"><img align="right" width="15" height="15" src="https://git.io/JtehR" alt="Back to top"></a>

<details><summary><b><a href="https://github.com/pygments/pygments">Pygments</a></b> (🥇35 ·  ⭐ 2.2K) - Pygments is a generic syntax highlighter written in Python. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/pygments/pygments) ⭐ 2,207 | 🐛 663 | 🌐 Python | 📅 2026-08-17 (👨‍💻 950 · 🔀 870 · 📦 1.2M):

  ```
  git clone https://github.com/pygments/pygments
  ```
* [PyPi](https://pypi.org/project/pygments) (📥 990M / month · 📦 12K · ⏱️ 17.08.2026):
  ```
  pip install pygments
  ```
* [Conda](https://anaconda.org/conda-forge/pygments) (📥 73M · ⏱️ 17.08.2026):
  ```
  conda install -c conda-forge pygments
  ```

</details>
<details><summary><b><a href="https://github.com/GrahamDumpleton/wrapt">wrapt</a></b> (🥇34 ·  ⭐ 2.3K) - A Python module for decorators, wrappers and monkey patching. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/GrahamDumpleton/wrapt) ⭐ 2,307 | 🐛 1 | 🌐 Python | 📅 2026-09-10 (👨‍💻 40 · 🔀 250 · 📦 640K):

  ```
  git clone https://github.com/GrahamDumpleton/wrapt
  ```
* [PyPi](https://pypi.org/project/wrapt) (📥 400M / month · 📦 3.4K · ⏱️ 10.09.2026):
  ```
  pip install wrapt
  ```
* [Conda](https://anaconda.org/conda-forge/wrapt) (📥 42M · ⏱️ 11.09.2026):
  ```
  conda install -c conda-forge wrapt
  ```

</details>
<details><summary><b><a href="https://github.com/pyparsing/pyparsing">pyparsing</a></b> (🥇33 ·  ⭐ 2.5K) - Python library for creating PEG parsers. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/pyparsing/pyparsing) ⭐ 2,491 | 🐛 46 | 🌐 Python | 📅 2026-08-18 (👨‍💻 84 · 🔀 320 · 📦 1.3M):

  ```
  git clone https://github.com/pyparsing/pyparsing
  ```
* [PyPi](https://pypi.org/project/pyparsing) (📥 330M / month · 📦 6.3K · ⏱️ 21.01.2026):
  ```
  pip install pyparsing
  ```
* [Conda](https://anaconda.org/conda-forge/pyparsing) (📥 79M · ⏱️ 21.01.2026):
  ```
  conda install -c conda-forge pyparsing
  ```

</details>
<details><summary><b><a href="https://github.com/cookiecutter/cookiecutter">cookiecutter</a></b> (🥈31 ·  ⭐ 25K) - A cross-platform command-line utility that creates projects from.. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/cookiecutter/cookiecutter) ⭐ 25,093 | 🐛 313 | 🌐 Python | 📅 2026-04-01 (👨‍💻 330 · 🔀 2.2K · 📦 38K):

  ```
  git clone https://github.com/cookiecutter/cookiecutter
  ```
* [PyPi](https://pypi.org/project/cookiecutter) (📥 7.8M / month · 📦 1.2K · ⏱️ 04.03.2026):
  ```
  pip install cookiecutter
  ```
* [Conda](https://anaconda.org/conda-forge/cookiecutter) (📥 2.3M · ⏱️ 04.03.2026):
  ```
  conda install -c conda-forge cookiecutter
  ```

</details>
<details><summary><b><a href="https://github.com/pytest-dev/pluggy">pluggy</a></b> (🥈31 ·  ⭐ 1.6K) - A minimalist production ready plugin system. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/pytest-dev/pluggy) ⭐ 1,692 | 🐛 49 | 🌐 Python | 📅 2026-09-18 (👨‍💻 64 · 🔀 150 · 📦 830K):

  ```
  git clone https://github.com/pytest-dev/pluggy
  ```
* [PyPi](https://pypi.org/project/pluggy) (📥 970M / month · 📦 2.6K · ⏱️ 15.05.2025):
  ```
  pip install pluggy
  ```
* [Conda](https://anaconda.org/conda-forge/pluggy) (📥 60M · ⏱️ 05.12.2025):
  ```
  conda install -c conda-forge pluggy
  ```

</details>
<details><summary><b><a href="https://github.com/micheles/decorator">decorator</a></b> (🥈30 ·  ⭐ 910) - Decorators for Humans. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code></summary>

* [GitHub](https://github.com/micheles/decorator) ⭐ 904 | 🐛 5 | 🌐 Python | 📅 2026-07-09 (👨‍💻 33 · 🔀 120 · 📦 770K):

  ```
  git clone https://github.com/micheles/decorator
  ```
* [PyPi](https://pypi.org/project/decorator) (📥 150M / month · 📦 2.9K · ⏱️ 18.05.2026):
  ```
  pip install decorator
  ```
* [Conda](https://anaconda.org/conda-forge/decorator) (📥 52M · ⏱️ 18.05.2026):
  ```
  conda install -c conda-forge decorator
  ```

</details>
<details><summary><b><a href="http://fastcore.fast.ai">fastcore</a></b> (🥈29 ·  ⭐ 1.1K) - Python supercharged for fastai development. <code><a href="http://bit.ly/3nYMfla">Apache-2</a></code></summary>

* [GitHub]() (👨‍💻 84 · 🔀 300 · 📦 14K):

  ```
  git clone https://github.com/fastai/fastcore
  ```
* [PyPi](https://pypi.org/project/fastcore) (📥 49M / month · 📦 1.3K · ⏱️ 15.09.2026):
  ```
  pip install fastcore
  ```
* [Conda](https://anaconda.org/conda-forge/fastcore) (📥 390K · ⏱️ 13.09.2026):
  ```
  conda install -c conda-forge fastcore
  ```

</details>
<details><summary><b><a href="https://github.com/mingrammer/diagrams">diagrams</a></b> (🥈28 ·  ⭐ 42K) - Diagram as Code for prototyping cloud system architectures. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/mingrammer/diagrams) ⭐ 42,625 | 🐛 390 | 🌐 Python | 📅 2026-09-10 (👨‍💻 180 · 🔀 2.7K · 📦 2.5K):

  ```
  git clone https://github.com/mingrammer/diagrams
  ```
* [PyPi](https://pypi.org/project/diagrams) (📥 2.2M / month · 📦 140 · ⏱️ 22.11.2025):
  ```
  pip install diagrams
  ```
* [Conda](https://anaconda.org/conda-forge/diagrams) (📥 380K · ⏱️ 24.11.2025):
  ```
  conda install -c conda-forge diagrams
  ```

</details>
<details><summary><b><a href="https://github.com/boppreh/keyboard">keyboard</a></b> (🥈28 ·  ⭐ 3.8K) - Hook and simulate global keyboard events on Windows and Linux. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/boppreh/keyboard) ⭐ 3,967 | 🐛 414 | 🌐 Python | 📅 2026-07-10 (👨‍💻 41 · 🔀 430 · 📦 27K):

  ```
  git clone https://github.com/boppreh/keyboard
  ```
* [PyPi](https://pypi.org/project/keyboard) (📥 530K / month · 📦 720 · ⏱️ 23.03.2020):
  ```
  pip install keyboard
  ```
* [Conda](https://anaconda.org/conda-forge/keyboard) (📥 87K · ⏱️ 08.09.2026):
  ```
  conda install -c conda-forge keyboard
  ```

</details>
<details><summary><b><a href="https://github.com/copier-org/copier">Copier</a></b> (🥉27 ·  ⭐ 3.6K) - Library and command-line utility for rendering projects templates. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/copier-org/copier) ⭐ 3,581 | 🐛 143 | 🌐 Python | 📅 2026-09-18 (👨‍💻 130 · 🔀 270 · 📦 2.1K):

  ```
  git clone https://github.com/copier-org/copier
  ```
* [PyPi](https://pypi.org/project/copier) (📥 1.5M / month · 📦 420 · ⏱️ 07.09.2026):
  ```
  pip install copier
  ```

</details>
<details><summary><b><a href="https://github.com/arsenetar/send2trash">Send2Trash</a></b> (🥉27 ·  ⭐ 300 · 💤) - Python library to natively send files to Trash (or Recycle bin).. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code></summary>

* [GitHub](https://github.com/arsenetar/send2trash) ⭐ 312 | 🐛 41 | 🌐 Python | 📅 2026-06-16 (👨‍💻 20 · 🔀 47 · 📦 320K):

  ```
  git clone https://github.com/arsenetar/send2trash
  ```
* [PyPi](https://pypi.org/project/send2trash) (📥 34M / month · 📦 710 · ⏱️ 14.01.2026):
  ```
  pip install send2trash
  ```
* [Conda](https://anaconda.org/conda-forge/send2trash) (📥 23M · ⏱️ 12.02.2026):
  ```
  conda install -c conda-forge send2trash
  ```

</details>
<details><summary><b><a href="https://github.com/pyscaffold/pyscaffold">pyscaffold</a></b> (🥉24 ·  ⭐ 2.3K) - Python project template generator with batteries included. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/pyscaffold/pyscaffold) ⭐ 2,260 | 🐛 37 | 🌐 Python | 📅 2026-08-31 (👨‍💻 59 · 🔀 190):

  ```
  git clone https://github.com/pyscaffold/pyscaffold
  ```
* [PyPi](https://pypi.org/project/pyscaffold) (📥 2.4M / month · 📦 46 · ⏱️ 23.09.2024):
  ```
  pip install pyscaffold
  ```
* [Conda](https://anaconda.org/conda-forge/pyscaffold) (📥 780K · ⏱️ 22.04.2025):
  ```
  conda install -c conda-forge pyscaffold
  ```

</details>
<details><summary><b><a href="https://github.com/BoboTiG/python-mss">python-mss</a></b> (🥉23 ·  ⭐ 1.3K) - An ultra fast cross-platform multiple screenshots module in pure.. <code><a href="http://bit.ly/34MBwT8">MIT</a></code></summary>

* [GitHub](https://github.com/BoboTiG/python-mss) ⭐ 1,282 | 🐛 43 | 🌐 Python | 📅 2026-09-18 (👨‍💻 31 · 🔀 110 · 📦 14K):

  ```
  git clone https://github.com/BoboTiG/python-mss
  ```
* [PyPi](https://pypi.org/project/mss) (📦 650 · ⏱️ 23.04.2026):
  ```
  pip install mss
  ```
* [Conda](https://anaconda.org/conda-forge/python-mss) (📥 100K · ⏱️ 23.04.2026):
  ```
  conda install -c conda-forge python-mss
  ```

</details>
<details><summary>Show 8 hidden projects...</summary>

* <b>[powerline-shell](https://github.com/b-ryan/powerline-shell) ⭐ 6,288 | 🐛 131 | 🌐 Python | 📅 2024-03-19</b> (🥉21 ·  ⭐ 5.9K · 💀) - A beautiful and useful prompt for your shell. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[pycparser](https://github.com/eliben/pycparser) ⭐ 3,569 | 🐛 30 | 🌐 Python | 📅 2026-09-15</b> (🥈30 ·  ⭐ 3.6K) - Complete C99 parser in pure Python. <code>❗Unlicensed</code>
* <b>[macropy](https://github.com/lihaoyi/macropy) ⭐ 3,305 | 🐛 28 | 🌐 TSQL | 📅 2023-06-10</b> (🥉15 ·  ⭐ 2.5K · 💀) - Macros in Python: quasiquotes, case classes, LINQ and more!. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b>[pluginbase](https://github.com/mitsuhiko/pluginbase) ⭐ 1,139 | 🐛 5 | 🌐 Python | 📅 2021-05-16</b> (🥉22 ·  ⭐ 1.1K · 💀) - A simple but flexible plugin system for Python. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b>[pyscreenshot](https://github.com/ponty/pyscreenshot) ⭐ 512 | 🐛 4 | 🌐 Python | 📅 2023-11-04</b> (🥉23 ·  ⭐ 500 · 💀) - Python screenshot library, replacement for the Pillow\.. <code><a href="http://bit.ly/3rqEWVr">BSD-2</a></code>
* <b>[catalogue](https://github.com/explosion/catalogue) ⭐ 183 | 🐛 6 | 🌐 Python | 📅 2026-03-27</b> (🥉25 ·  ⭐ 180 · 💀) - Super lightweight function registries for your library. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>
* <b><a href="https://www.py4j.org/">py4j</a></b> (🥈28 ·  ⭐ 1.3K · 💀) - Enables Python programs to dynamically access arbitrary Java objects. <code><a href="http://bit.ly/3aKzpTv">BSD-3</a></code>
* <b><a href="https://openpyxl.readthedocs.io">openpyxl</a></b> (🥉25 ·  ⭐ 13) - A Python library to read/write Excel 2010 xlsx/xlsm files. <code><a href="http://bit.ly/34MBwT8">MIT</a></code>

</details>

***

## Related Resources

* [**awesome-python**](https://github.com/vinta/awesome-python) ⭐ 321,492 | 🐛 23 | 🌐 Python | 📅 2026-09-15: A curated list of awesome Python frameworks, libraries, software and resources.
* [**best-of-ml-python**](https://github.com/ml-tooling/best-of-ml-python) ⭐ 23,811 | 🐛 57 | 📅 2026-09-17: A ranked list of awesome machine learning Python libraries.
* [**best-of-web-python**](https://github.com/ml-tooling/best-of-web-python) ⭐ 2,759 | 🐛 19 | 📅 2026-09-17: A ranked list of awesome Python libraries for web development.
* [**best-of-python-dev**](https://github.com/ml-tooling/best-of-python-dev) ⭐ 1,302 | 🐛 33 | 🌐 Python | 📅 2026-09-17: A ranked list of awesome Python developer tools and libraries.
* [**Best-of lists**](https://best-of.org): Discover other best-of lists with awesome open-source projects on all kinds of topics.

## Contribution

Contributions are encouraged and always welcome! If you like to add or update projects, choose one of the following ways:

* Open an issue by selecting one of the provided categories from the [issue page](https://github.com/ml-tooling/best-of-python/issues/new/choose) ⭐ 4,604 | 🐛 49 | 📅 2026-09-17 and fill in the requested information.
* Modify the [projects.yaml](https://github.com/ml-tooling/best-of-python/blob/main/projects.yaml) ⭐ 4,604 | 🐛 49 | 📅 2026-09-17 with your additions or changes, and submit a pull request. This can also be done directly via the [Github UI](https://github.com/ml-tooling/best-of-python/edit/main/projects.yaml) ⭐ 4,604 | 🐛 49 | 📅 2026-09-17.

If you like to contribute to or share suggestions regarding the project metadata collection or markdown generation, please refer to the [best-of-generator](https://github.com/best-of-lists/best-of-generator) ⭐ 110 | 🐛 15 | 🌐 Python | 📅 2025-08-31 repository. If you like to create your own best-of list, we recommend to follow [this guide](https://github.com/best-of-lists/best-of/blob/main/create-best-of-list.md) ⭐ 1,898 | 🐛 24 | 📅 2026-09-17.

For more information on how to add or update projects, please read the [contribution guidelines](https://github.com/ml-tooling/best-of-python/blob/main/CONTRIBUTING.md) ⭐ 4,604 | 🐛 49 | 📅 2026-09-17. By participating in this project, you agree to abide by its [Code of Conduct](https://github.com/ml-tooling/best-of-python/blob/main/.github/CODE_OF_CONDUCT.md) ⭐ 4,604 | 🐛 49 | 📅 2026-09-17.

## License

[![CC0](https://mirrors.creativecommons.org/presskit/buttons/88x31/svg/by-sa.svg)](https://creativecommons.org/licenses/by-sa/4.0/)

***

> _Enhansomed by [enhansome](https://github.com/enhansome) on 2026-09-18._
